...
function startMaster() {
  ...
  cluster.on('exit', function restartDeadWorker(deadWorker) {
    ...
    if (config.notify && config.notify.onCrash) {
      util.sendMail({}, function(err, result) {
        if (err) logErr('Mailer failed with error', err.stack||err)
        if (config.log > 1) log('Crash mailer result', result)
      })
    }
...
function sendServiceStartedMail() {
  ...
  util.sendMail(message, function(err, result) {
    if (err) logErr('Notification Mailer Failed ' + util.nowFormatted(), err.stack||err)
    else log('Notification mail sent ' + util.nowFormatted())
    if (config.log > 1) log('send mail result', result)
  })
}
...
function handleUncaughtError(err) {
  ...
  if (config.notify && config.notify.onCrash) {
    ...
    mail.subject = (cluster.isMaster)
      ? config.service.name + ' Master process crashed on ' + util.nowFormatted()
      : config.service.name + ' Worker process ' + cluster.worker.id + ' crashed on ' + util.nowFormatted()
    log('Sending crash mail')
    util.sendMail(mail, function(err, result) {
      if (err) logErr('Mailer failed with error', err.stack||err)
      if (config.log > 1) log('Crash mailer result', result)
      writeCrashFile()
    })
