...
// Add magic utils such as _, type, and log to the global namespace
require('./global')
...
function startRecurringTasks() {
  util.db.tasks.find().toArray(function(err, taskDocs) {
    if (err) return logErr(err)
    taskDocs.forEach(function(taskDoc) {
      err = util.db.tasks.start(taskDoc)
      if (tipe.error(err)) logErr('Error ' + err + ' starting task:', taskDoc)
    })
    log('Started ' + taskDocs.length + ' recurring tasks')
  })
}
