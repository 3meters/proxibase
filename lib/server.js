...
// Useful when running in a container where the external URL is supplied by the 
// container orchistrator
if (cli.url) {
  config.service.urlExternal = cli.url
}
...
function startMaster() {
  ...
  cluster.on("listening", function readyWorker(worker) {
    ...
    if (cStarted === statics.workers) {
      log("\n" + config.service.name + " listening on " + config.service.uri)
      log("\nPublic url: " + config.service.urlExternal)
...
function sendServiceStartedMail() {
  util.sendMail({
    ...
    text: "\nService: " + config.service.uri + "\n\n" +
     "Version: " + version + "\n\n" +
     "Public url: " + config.service.urlExternal + "\n\n" +
     "Sign in: " + config.service.urlExternal + "/v1/signin\n\n" +
...
function handleUncaughtError(err) {
  ...
  if (config.notify && config.notify.onCrash) {
    var mail = {to: config.notify.to, text: stack}
