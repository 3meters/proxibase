...
function startMaster() {
  ...
  cluster.on('listening', function readyWorker(worker) {
    ...
    // Start recurring tasks on one worker
    if (!statics.taskWorkerId) {
      statics.taskWorkerId = worker.id
      setTimeout(function() {
        if (config.ignoreTasks) log('\nNot starting recurring tasks')
        else {
          log('\nAsking worker ' + worker.id + ' to run recurring tasks\n')
          worker.send({startTasks: true})
        }
      }, 250)
    }
    // Say hello
    if (!statics.helloSent) {
      statics.helloSent = true
      setTimeout(function() {
