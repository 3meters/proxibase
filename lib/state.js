...
function init(configIn, cb) {
  ...
  function initStateVar(key, next) {
    ...
    db.documents.safeFindOne({_id: doc._id}, dbOps, function(err, foundDoc) {
      if (err) return cb(err)
      if (foundDoc) return next()  // values in the db override those pass in on config
      else db.documents.safeInsert(doc, dbOps, function(err, savedDoc) {
        if (err) return cb(err)
  ...
  function getStateFromDb(err) {
    ...
    db.documents.safeFind({type: 'state'}, dbOps, function(err, docs) {
      if (err) return cb(err)
      if (cluster.isMaster) log()

      docs.forEach(function(doc) {
        var key = doc._id.slice(3)  // slice off the do. from the documents collection _id
        config[key] = doc.data
