...
function init(configIn, cb) {
  ...
  function initStateVar(key, next) {
    ...
    db.sysvars.safeFindOne({_id: doc._id}, dbOps, function(err, foundDoc) {
      if (err) return cb(err)
      if (foundDoc) return next()  // values in the db override those pass in on config
      else db.sysvars.safeInsert(doc, dbOps, function(err, savedDoc) {
        if (err) return cb(err)
  ...
  function getStateFromDb(err) {
    ...
    db.sysvars.safeFind({type: 'state'}, dbOps, function(err, docs) {
      if (err) return cb(err)
...
function set(key, val, cb) {
  ...
  db.sysvars.safeUpsert(doc, dbOps, function(err, savedDoc) {
    if (err) return cb(err)
...
function _get(key, forceRefresh, cb) {
  ...
  delete config[key]
  db.sysvars.safeFindOne({_id: 'sy.' + key}, dbOps, function(err, foundDoc) {
    if (err) return cb(err)
...
function remove(key, cb) {
  cb = cb || log
  delete config[key]
  db.sysvars.safeRemove({_id: 'sy.' + key, type: 'state'}, dbOps, function(err, meta) {
