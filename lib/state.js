...
function _get(key, forceRefresh, cb) {
  if (!forceRefresh && tipe.isDefined(target[key])) {
    if (tipe.isString(key)) return cb(null, target[key])
    if (tipe.isRegex(key)) {
      results = []
      for (var k in target) {
  ...
  db.documents.safeFindOne({_id: 'do.' + key}, dbOps, function(err, foundDoc) {
    if (err) return cb(err)
    if (!foundDoc) return cb(perr.notFound)
    target[key] = foundDoc.data
    cb(null, target[key])
  })
}
...
process.on('message', function msgGetState(msg) {
  ...
  if (msg.worker === cluster.worker.id) return  // sent from me
