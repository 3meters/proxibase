...
module.exports = {
  ...
  // Logs requests in playbackable format
  requestLogger: function() {
    return function(req, res, next) {
      if (!util.config.requestLog) return next()
      var smallReq = {method: req.method, url: req.url}
      // TODO: do a two-way hash of passwords with a shared secret with the playback module
      if (req.method === 'post') smallReq.body = req.body
