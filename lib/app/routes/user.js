...
create = function(req, res, cb) {
  ...
  function checkSecret(secret) {
    if (config.service.newAccountSecret === body.secret) return null
    else return res.error(perr.notHuman())
  }

  var body = req.body
  var err = util.check(_body, body)
  ...
  checkEmail(body.data.email, function(err) {
    ...
    var options = {
      user: util.adminUser,
      viaApi: true,
      skipEmailValidation: util.truthy(body.skipEmailValidation) // Optional skip email address validation
    }
    ...
    users.safeInsert(body.data, options, function(err, savedUser) {
      ...
      var validateEmailUrl
      if (!body.skipEmailValidation) {
        validateEmailUrl = users.genValidationLink(savedUser._id, savedUser.email)
      }
...
function checkEmail(email, cb) {
  ...
  util.db.documents.findOne({
    type: 'validUser', data: {email: email}
  }, function(err, doc) {
    if (err) return cb(err)
  ...
  users.findOne({_id: req.body.user._id}, function(err, user) {
    ...
    users.reqValidate(user, user, {user: req.user}, function(err) {
      ...
      users.update({id: user._id},
        {$set: {validationNotifyDate: user.validationNotifiyDate}},
        function(err) {
