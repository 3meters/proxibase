...
create = function(req, res, next) {
  ...
  if (!(body && body.data && body.data.email &&
      body.data.password && body.secret)) {
    return res.error(perr.missingParam('data.email, data.password, secret'))
  }

  if (config.service.newAccountSecret !== body.secret) {
    return res.error(perr.notHuman())
  }
...
changePassword = function(req, res, next) {
  ...
  if (!(req.body && req.body.user && req.body.user._id &&
    ...
    return next(perr.missingParam('user._id, user.newPassword'))
  }
...
reqResetPassword = function(req, res, next) {
  ...
  users.findOne({email: req.query.email}, function(err, user) {
    ...
    if (!user.validationDate) {
      // TODO: make a new perror
      return res.error(perr.badRequest(
