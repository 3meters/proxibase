...
function run(options, cb) {
  ...
  // Normalize a candidate source, returns null on failure
  function normCandidate(candidate, cb) {
    normalize(candidate, function(source) {
      if (source) {
  ...
  var grovelWebPage = function(uri, cb) {
    ...
    superagent.get(uri).end(function(err, sres) {
      ...
      async.forEach(candidates, normCandidate, function() {
  ...
  // Normalize
  async.forEach(sources, normalize, queueGetters) // normalize never returns errors
