...
function run(options, cb) {
  ...
  var getters = async.queue(function(task, cb) {
    if (task && type.isFunction(task.fn)) {
      task.options = {
        query: task.query,
        getters: this,           // enable getters to add new getters
    ...
    sources.forEach(function(source) {
      switch (source.type) {
        case 'website':
          ...
          break
        case 'foursquare':
          getters.push({fn: factual.getFactualByPartnerId, query: {
          ...
          break
        case '_factual':
          getters.push({fn: factual.getFactualByFactualId, query: {
            factualId: source.id
          }})
          break
        default:
