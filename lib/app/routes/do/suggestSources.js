...
function run(options, cb) {
  ...
  function normalize(candidate, cb) {
    util.sources.normalize(candidate, function() {
      if (candidate.type) {
        if (!util.statics.sources[candidate.type]) candidate.hidden = true
  ...
  function queueGetters() {
    sources = _.compact(sources) // non-normalized sources have been set to null
    dedupe(sources)
  ...
  // Dedupe sources in place
  function dedupe(sources) {
    sources = _.uniq(sources, false, function(source) {
