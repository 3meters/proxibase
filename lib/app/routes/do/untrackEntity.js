      ...
      if (doc && doc.primary) {
        ...
        data.find(req, function(err, results) {
          ...
          if (plusCount - minusCount <= 0) {
            clearProximityLinks(req, entityId, beaconIds)
          }
        })
      }              
    })
...
function clearProximityLinks(req, entityId, beaconIds) {
  ...
  db.links.find({ _from:entityId, _to:{ $in:beaconIds }, type:'proximity' }).toArray(function(err, links) {
    ...
    function finish(err) {
      if (err) {
        util.logErr('Deleting proximity links failed in untrackEntity', err)
      }
    }
  })
}