  ...
  function mergePlaceEntities(places, cb) {
    places.forEach(function(place) {
      if (place.place && place.place.location) {
        place.place.distance = haversine(
          req.body.latitude,
          req.body.longitude,
    ...
    for (var p = places.length; p--;) {
      if (places[p].place && places[p].place.distance) {
        radius = places[p].place.distance
        break
      }
    }
      ...
      .toArray(function(err, entities) {
        ...
        for (var e = entities.length; e--;) {
          if (entities[e].place && entities[e].place.location &&
            ...
            userCreatedPlaces++
            entities[e].place.distance = haversine(
              req.body.latitude,
              req.body.longitude,
        ...
        if (userCreatedPlaces) {
          entities.forEach(function(entity) {
            if (entity.place && 'user' === entity.place.provider) {
              places.push(entity)
            }
          })
        }
        ...
        places.sort(function(a, b) {
          var result = null
          try { result = a.place.distance - b.place.distance }
          catch (e) { }
          if (type.isNumber(result)) return result
          // The rest should rarely if ever run.  Just paranoid.
          if (!a.place && !b.place) return 0
          if (a.place && !b.place) return -1
          if (!a.place && b.place) return 1
          if (!type.isNumber(a.place.distance) && !type.isNumber(b.place.distance)) return 0
          if (type.isNumber(a.place.distance) && !type.isNumber(b.place.distance)) return -1
          if (!type.isNumber(a.place.distance) && type.isNumber(b.place.distance)) return 1
          return 0
        })
        cb(null, places)
