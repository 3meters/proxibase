...
var main = function(req, res) {
  ...
  function mergePlaceEntities(places, cb) {
    ...
    db.entities
      .find(query, {'_id':1})
      .limit(req.body.limit)
      .toArray(function(err, entIdsUncut) {
        if (err) return cb(err)
        log('debug entIdsUncut', entIdsUncut)
        var entIds = []
        entIdsUncut.forEach(function(id) {
          if (!exclude(id._id)) entIds.push(id._id)
        })
        log('debug entIds', entIds)
        var r = util.clone(req)
        log('debug r', r)
        getEntities(r, {entityIds: entIds}, function(err, entities) {
