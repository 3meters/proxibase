...
function suggestSources(req, res) {
  ...
  if (!(entity.sources && req.body.suggestSources)) {
...
function doInsertEntity(req, res) {
  ...
  db.entities.safeInsert(doc, options, function (err, savedDoc) {
    ...
    methods.logAction(savedDoc._id, 'aircandi', actionType, req.user._id,
        req.body.observation ? req.body.observation : null, req)
...
function insertLinks(req, res) {
  if (req.body.parentId) {
    ...
    function insertLink(beacon, next) {
      ...
      var link = {_from:req.insertedEntity._id, _to:beacon._id, primary:primary,
          signal:beacon.level, type:'browse'} 
      var options = userOptions(req.user)
      db.links.safeInsert(link, options, function(err, savedDoc) {
        if (err) return next(err)
        if (savedDoc && primary) {
          log('Logging action for place entity primary link: ' + savedDoc._id)
          methods.logAction(savedDoc._id, 'aircandi', "link_browse", req.user._id,
              req.body.observation ? req.body.observation : null, req)
        }
        next()
      })
