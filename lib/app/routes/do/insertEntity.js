...
function suggestSources(req, res) {
  var sources = _.clone(req.body.entity.sources)
  ...
  suggest(options, function(err, newSources) {
    if (err) logErr(err)
      ...
      sources.forEach(function(source) {
        if (!_sources[source.type]) return // not one of our known sources
...
function doInsertEntity(req, res) {
  ...
  if (doc.place && doc.place.provider) {
    if (doc.place.provider !== 'aircandi' && doc.place.provider !== 'user') {
      actionType += '_linked'
      options = {user:req.user, adminOwns:true}
    }
    else if (doc.place.provider == 'user') {
      actionType += '_custom'
    }
  }
