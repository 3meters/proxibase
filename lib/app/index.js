...
function addPreRoutes(app) {
  ...
  // Kill bad intalls by sending back an expected parameter with a diffent type
  app.post('/do/registerInstall', function(req, res, next) {
    var install = req.body.install
    if (!install) return next()
    if ((install.clientVersionCode === 120 &&
         install.clientPackageName === "com.patchr.android") ||
        (install.clientVersionCode === 214 &&
         install.clientPackageName === "com.aircandi.catalina")) {
      log('Delivering poison pill for /do/registerInstall', {qs: req.query, body: req.body})
      return res.send({count: 'Please reinstall Patchr from the Google Play store.'})
    }
    next()
  })
  ...
  app.all('*', function(req, res, next) {
    var dbOps = {ip: req.ip, tag: req.tag}
    if (req.user) dbOps.user = req.user
    if (req.user && req.user._id === util.adminId) dbOps.asAdmin = true
    req.dbOps = dbOps
    next()
  })
  ...
  function checkContentType(req, res, next) {
    ...
    if (!/^application\/json/.test(contentType)) {
      if (!/^\/signin/.test(req.path) && !/^\/admin\/client/.test(req.path)) {  // HACK: our 2 html forms
