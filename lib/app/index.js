...
app.configure(function() {
  app.set('views', viewDir)         // for explorer
  app.set('view engine', 'jade')    // for explorer
  app.use(favicon(path.join(__dirname, '../../assets/favicon.ico')))
  app.use(middleware.tagger())
  app.use(compression())
  app.use(express.static(path.join(__dirname, '../../assets')))
  // These two express3 methods are replaced by the express4-style explicit
  // module on the next line
...
function addPreRoutes(app) {
  ...
  // accept and explict route for static assest
  app.get('/assets/?*', serveStatic(path.join(__dirname, '../../assets')))
