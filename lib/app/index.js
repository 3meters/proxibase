...
function addPreRoutes(app) {
  ...
  app.all('*', function(req, res, next) {
    ...
    if (req.query.installId && !req.query.install) req.query.install = req.query.installId

    if (!(req.query.install && (req.query.location || req.query.beacons))) return next()

    var ops = {
      installId: req.query.install,
      userId: req.user ? req.user._id : util.annonId,
