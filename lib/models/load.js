...
function loadModels(mdb) {
  ...
  // users model is required by the base schema
  mdb.model('users', require('./users').getSchema());
  mdb.modelNames.push('users');
  log("Loaded model users");

  var modelDir = path.join(__dirname, mdb.name);
