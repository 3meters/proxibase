...
Users.pre('save', function(next) {
  if (this.isNew) {
    if (!(this.password || this.oauthId)) {
      return next(new HttpError(httpErrors.badUserAuthParams))
    }
    if (!this.email) {
      return next(new HttpError(httpErrors.missingParams, 'user.email'))
    }
    if (this.password) {
      if (!ensurePasswordStrength(this.password, this.name)) {
        return next(new HttpError(httpErrors.badUserPassword))
      }
...
Users.pre('save', function(next) {
  if (!this.isNew && this.isModified('password')) {
    if (!(this.__fromApi && (this.__fromApi === hashApiSecret(this._id)))) {
      return next(new HttpError(httpErrors.mustChangePasswordViaApi))
    }
    else {
      // allow the save
...
Users.methods.changePassword = function(oldPass, newPass, next) {
  if (!this.verifyPassword(oldPass)) {
    return next(new HttpError(httpErrors.badAuthCred))
  }
