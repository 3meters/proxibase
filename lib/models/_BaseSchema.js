...
// Public statics methods on all models
baseSchema.statics = {
...
/*
 * Id is the schema number.  It is intended to be overridden by each model that relies 
 * on the base schema to generate IDs, it should be a int between 0 and 999 that will be 
 * padded to 3 digits in the id.  It should be unique per schema per database
 */
 ...
 *
 *    clusterId.collectionId.date.secondsSinceMidnight.milliseconds.randomNumber
 *
 *  in the following format with a fixed number of characters for each element:
 *
 *    cls.col.yymmdd.scnds.mil.random
 *
 */
function genId() {
