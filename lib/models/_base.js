...
var Schema = exports.Schema = function(schemaId) {
  ...
  this.add({
    ...
    changed:    { type: Number, default: getTimeUTC, set: getTimeUTC}
  });

  this.pre('validate', function(next) {
    log('validate fired');
    if (!this._id) this._id = _genId(schemaId, this.changed);
    next();
  });
 ...
 *
 */
function _genId(schemaId, timeUTC) {

  assert(schemaId && timeUTC, "Invalid call to _genId");
...
// hang an array of reference columns, aka foreign keys, on the models schema object
function loadRefs() {
  this.schema.refs = [];
  var tree = this.schema.tree; // schema.tree is a private mongoose object
  for (path in tree) {
    if (tree[path].ref) this.schema.refs.push(path); 
  }
}
