...
// Create and open the performance log file
if (config.log.perf) {
  perfLog = fs.createWriteStream(config.log.perf)
  perfLog.write('RequestTag,Time\n')
}
...
// Override res.send with a version that logs response times
app.all('*', function(req, res, next) {
  res._send = res.send   // Stash express's send method
  res.send = function(arg1, arg2, arg3) {
    if (config.log && config.log.level && config.log.level > 1) {
      log('==== Request ' + req.tag + ' intermediate time: ' + util.getElapsedTime(req.start))
    }
    res._send(arg1, arg2, arg3)
    /*
     * res.send is often called a second time intnerally by Express, the first call
     * converts the body object to JSON, the second call actually sends the
     * response.  This code inpects the function signiture to differentiate
     * between these cases. This may break across Express upgrades.
     */
    if (arg1 && arg3 && typeof arg1 === 'string' && typeof arg3 === 'number') {
      req.time = util.getElapsedTime(req.start)
      log('==== Request ' + req.tag + ' total time: ' + req.time)
      // Log the request time in CSV format
      if (config.log.perf) {
        perfLog.write(req.tag + ',' + req.time + '\n')
      }
    }
  }
  next()
...
app.all('*', function(req, res, next) {
  if (config.log && config.log.level) {
    req.tag = Math.floor(Math.random() * 10000000).toString()
    req.start = new Date() // start request timer
    log('\n==== Request ' + req.tag + ' ' + req.start +
      ' (' + req.start.getMilliseconds() + ')')
