...
// Create and configure the public express app instance
var app = module.exports.app = express.createServer()
app
  .use(express.static(__dirname + '/public'))
  .use(express.bodyParser())
  .use(exputil.errorTrap())
  .use(exputil.tagger())
if (config.log && config.log.level) app.use(exputil.logger())
app.use(app.router)
...
app.get('/signin/:service', authService)


// Sign out
app.get('/signout', authService.signout)


// Check req headers
app.post('*', function(req, res, next) {
  if (!req.headers['content-type']) {
    return res.error(new Error('Missing header content-type'))
  }
  var contentType = req.headers['content-type'].toLowerCase()
  if (contentType.indexOf('application/json') != 0) {
    return res.error(new Error('Invalid content-type: ' + contentType +
...
app.post('/__do/*', function(req, res) {
  ...
  if (req.paths.length != 1)
    return res.error(new Error("Expected /__do/methodname"))
    ...
    if (children[0] === '*') {
      ...
      for (var i = children.length; i--;) {
        ...
        if (!gdb.models[req.modelName].schema.refChildren[child])
          return res.error(new Error("Table " + req.modelName + " does not have child " + child))
      }
    }
...
app.post('*', function(req, res) {
  ...
  if (!(req.body && req.body.data)) {
    return res.error(new Error('request.body must contain { "data": {...} }'))
  }
  if (req.body.data instanceof Array && req.body.data.length != 1) {
    return res.error(new Error('request.body.data[] may contain only one element'))
  }
  if (req.qry.ids) {
...
/*
 * Parse query options passed in on the URL.  Unknown parameters are ignored
 */
  ...
  var parseQueryOptions = {
    ...
    __find: function(s) {
      try {
        ...
        return res.error('Could not parse __find criteria as valid JSON')
      }
