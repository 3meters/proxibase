...
var methods = require('./methods/main');
var log = require('./log');
...
app.post('*', function(req, res, next) {
  if (!req.headers['content-type']) {
    return res.send({ Error: 'Missing header content-type'}, 400);
  }
  var contentType = req.headers['content-type'].toLowerCase()
  if (contentType.indexOf('application/json') != 0) {
    return res.send({ Error: 'Invalid content-type: ' + contentType +
      ' Expected: application/json'}, 400);
  }
  next();
});
