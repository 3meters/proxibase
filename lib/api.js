...
app.all('*', function(req, res, next) {
  log('modelName3', req.modelName);
  if (!req.modelName)
    return res.send(notFound, 412);
  next();
});
...
app.get('*', function(req, res) {
  // only valid path element left is __info
  if (req.paths.length) {
...
app.delete('*', function(req, res) {
  log('delete req.paths', req.paths);
  if (req.paths.length)
    return res.send(notFound, 404);
  if (req.docIds)
