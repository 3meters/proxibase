...
app.get('/' + __info, function(req, res) {
  ...
  for (var modelName in mdb.models) {
    idMap[mdb.models[modelName].tableId] = modelName
  }
...
app.get('*', function(req, res) {
  ...
  if (req.paths.length) {
    ...
    if (req.paths[0] === __info) {
      var doc = mdb.models[req.modelName].schema.tree // tree is not doced and may be private
      delete doc.id
      return res.send(doc)
    }
    ...
    if (req.paths[0].toLowerCase() === '__genid') 
      return res.send({id: require('./util').genId(mdb.models[req.modelName].tableId)})
