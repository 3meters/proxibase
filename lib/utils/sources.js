...
function makeSourceArray() {
  ...
  keys.forEach(function(key) {
    ...
    var statics = _sources[key].statics
    for (var sKey in statics) {
      if (sKey.indexOf('_') !== 0) source[sKey] = statics[sKey]
    }
    sources.push(source)
  })
  util.statics.sources = sources
}

// Remove system properties from the public map
function makeSourceMap() {
  util.statics.sourceMap =  _sources
}
