...
// TODO:  create a meta schema for schema, and check
//  the incoming schema against it.
...
function check(value, schema, options) {
  ...
  var err = checkObject(value, schema, options)
  ...
  switch (type(value)) {
    case 'object':
      ...
      for (var key in schema) {
        options.key = key
        var err = checkObject(value[key], schema[key], options)
        if (err) return err
        // Set defaults
...
function checkScalar(value, schema, options) {
  ...
  if (schema.type && !isString(schema.type)) {
    var msg = 'Invalid schema.' + options.key + '.type must be a string'
  ...
  return null // scalar value passes
}
