...
function checkObject(value, schema, options) {
  ...
  if (isArray(value)) {
    log('debug value is array.  schema:', schema)
    value.forEach(function(elm) {
      log('debug calling check on array')
      log('elm', elm)
      log('schema.value', schema.value)
  ...
  // Check an object
    ...
    if (beStrict) {
      for (var key in value) {
        if (!schema[key]) {
          log('debug strict check failed on key: ' + key)
          //log('value', value)
          //log('schema', schema)
          //log('key', key)
    ...
    for (var key in value) {
      options.key = key
      // BUG:  this won't work for arrays
