...
// Template strings can be delimited by | for enum-like matching
function stringMatch(str, template) {
  if (template.indexOf('|') < 0) {
    return (str === template)
  }
  else return template.split('|').some(function(tStr) {
    return (str === tStr)
  })
}
...
function check(object, template, options) {
  ...
  // If the method doesn't accept empty, pass in empty to generate a quick api doc
  ...
  for (var key in template) {
    ...
    if (isObject(template[key].value) && isObject(object[key])) { // recurse
    ...
    if (template[key].required &&
        (isUndefined(object[key]) || isNull(object[key]))) {
    ...
    if (template[key].type && !isString(template[key].type)) {
    ...
    // Check type
    if (template[key].type && !isUndefined(object[key]) &&
        !stringMatch(type(object[key]), template[key].type)) {
    ...
    switch (type(template[key].value)) {
      ...
      case 'function':
        ...
        break
      case 'string':
        if (!stringMatch(object[key], template[key].value)) {
