...
var _options = {
  ...
  value: {
    ...
    timeout:  {type: 'number', default: locUtil.statics.timeout},
    log:      {type: 'boolean'}
  },
  init: function(v) {
...
exports.factual = function(options, cb) {
  ...
  if (locUtil.config.service.mode === 'test') {
    fact.cred = fact.credtest
  }
  var factual = new Factual(fact.cred.key, fact.cred.secret)
  if (locUtil.config.log > 1) factual.startDebug()
  locUtil.log('factual call count: ' + services.factual.callCount++)
...
exports.yelp = function(options, cb) {
  ...
  function fn(cb) {
    yelp.business(options.path, cb)
  }
  callService('yelp', fn, options, cb)
}
...
function prep(options) {
  return scrub(options, _options, {returnValue: true})
}
...
function callService(name, fn, options, cb) {
  // locUtil.debug('callService ' + name + ' options', options)
  if (tipe.isError(fn)) return cb(fn)
  if (locUtil.config.log > 1) options.log = true
  if (options.log) {
    var timer = locUtil.timer()
    var tag = String(Math.floor(Math.random() * 1000000))
    options.basePath = options.basePath || ''
    locUtil.log('Partner ' + name + ' req ' + tag + ' ' + options.basePath)
  }

  locUtil.timeLimit(fn, options.timeout, function(err, res, body) {
    if (options.log) locUtil.log('Partner ' + name + ' res ' + tag + ' ' + timer.read())
    cb(err, res, body)
  })
}
