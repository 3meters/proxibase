...
exports.factual = function(options, cb) {
  ...
  factual.get(options.path, options.query, function(err, res) {
    ...
    if (err) {
      debug('callService factual err', err)
      debug('callService factual res', res)
      var realErr = err
...
function call(source, options, cb) {
  ...
  if (options.log) {
    ...
    log('External service ' + tag + ' called: ' + source.service + req.req.path)
  }
  ...
  function finish(err, res, body) {
    if (options.log) {
      log('External service ' + tag + ' returned after ', timer.read())
    }
    cb(err, res, body)
  }
}
