...
var services = exports.services = {
  ...
  accountKit: {
    web: 'https://developers.facebook.com/docs/accountkit',
    host: 'https://graph.accountkit.com/v1.0',
    cred: {
      access_token: apiKeys.accountKit,
    },
  },
}
...
exports.accountKit = function(options, cb) {
  var fn = buildReq(services.accountKit, options)
  callService('accountKit', fn, options, cb)
}
...
function buildReq(partner, options) {
  ...
  options = util.scrub(options, _options, {returnValue: true})  // can coerce a string into an object
  ...
  var req = request[options.method](uri)
