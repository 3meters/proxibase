...
var _options = {
  ...
  value: {
    ...
    timeout:  {type: 'number', default: util.statics.timeout},
    log:      {type: 'boolean'}
  },
  init: function(v) {
...
function callService(name, fn, options, cb) {
  debug('callService ' + name + ' options', options)
  ...
  if (options.log) {
    ...
    log('Partner ' + name + ' req ' + tag + ' ' + options.basePath)
  }

  util.timeLimit(fn, options.timeout, function(err, res, body) {
    if (options.log) log('Partner ' + name + ' res ' + tag + ' ' + timer.read())
    cb(err, res, body)
  })
}
