...
function setConfig(configFile) {
  ...
  config.service.uri_external = config.service.protocol + '://' + config.service.host_external + port
  config.service.docsUri = "https://github.com/3meters/proxibase"
  if (config.proxy && config.proxy.enabled) {
    port = (config.proxy.port === 80 || config.proxy.port === 443)
      ? ''
      : ':' + config.proxy.port
    config.proxy.uri = config.proxy.protocol + '://' + config.proxy.host + port
  } else {
    // Not using a load balancer, set the proxy uri to the service uri
    config.proxy = {uri: config.service.uri}
  }
