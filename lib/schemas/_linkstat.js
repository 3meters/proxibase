  ...
  function startMapReduce(firstLinkId, lastLinkId) {
    ...
    var reduce = function(key, values) {
      ...
      values.forEach(function(value, i) {
        ...
        else print('Error in prox _linkstat mapreduce: expected number, got ' + typeof(value))
      })
      return count
    }
      ...
      function nextDoc(err) {
        ...
        cursor.nextObject(function(err, doc) {
          ...
          db[clName].findOne({_id: id}, function(err, joinedDoc) {
            ...
            // add new fields, but only after the Id and value fields, otherwise
            // the incrementatl reduce step fails.  Thus rather than user $set
            // to update only the new properties, we replace the entire document
            // with a new one with the new properties appended to the end
            //
            ...
            clStats.update({_id: doc._id}, doc, {w: 1}, function(err, count) {
              if (err) return cb(err)
              clStats.findOne({_id: doc._id}, function(err, updatedStat) {
