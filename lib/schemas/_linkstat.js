...
function _rebuild(options, cb) {
  ...
  clStats.remove({}, function(err, cRemoved) {
    if (err) return cb(err)
      ...
      db.sysvars.safeRemove({_id: lastStatDocId}, options, function(err) {
      if (err) return cb(err)
...
// Lock document
var lockDoc = {
  _id: 'sy.refreshLinkStatInProgress',
  type: 'lock',
}
...
function _refresh(options, cb) {
  ...
  // Check for a lock document to prevent running multiple
  // refreshes at the same time.
  db.sysvars.safeFindOne({_id: lockDoc._id}, dbOps, function(err, inProgress) {
    if (err) return cb(err)
    if (inProgress) {
      log('linkStat refresh already in progress -- ignoring')
      return cb()
    }
    db.sysvars.safeInsert(lockDoc, dbOps, function(err, inProgress) {
      if (err) return cb(err)
      if (!inProgress) return cb(perr.serverError('could not save lockDoc', lockDoc))
      start()
    })
  })
  ...
  function start() {
    ...
    db.sysvars.findOne({_id: lastStatDocId}, function(err, lastStatDoc) {
      ...
      db.links.safeFind({}, findOps, function(err, lastLinks) {
        if (err) return done(err)
        if (!(lastLinks && lastLinks.length)) return done(perr.serverError('no links found'))
    ...
    function startMapReduce(firstLinkId, lastLinkId) {
      ...
      db.links.mapReduce(map, reduce, mrOps, function(err, cl, mrStats) {
        ...
        function finish(err, mrStats) {
          ...
          db.sysvars.safeUpsert(lastStatDoc, dbOps, function(err) {
            if (err) return done(perr.serverError('stats updated but statDoc update failed', err))
  ...
  // Clear our lock and call back
  function done(err, out) {
    db.sysvars.safeRemove({_id: lockDoc._id}, dbOps, function(lockErr) {
      if (err && lockErr) err.lockErr = lockErr
      cb(err || lockErr, out)
    })
  }
}
