...
// Lock document
var lockDoc = {
  _id: 'do.refreshLinkStatInProgress',
  type: 'lock',
}
...
function _refresh(options, cb) {
  ...
  db.documents.safeFindOne({_id: lockDoc._id}, dbOps, function(err, inProgress) {
    if (err) return cb(err)
    if (inProgress) {
      log('linkStat refresh already in progress -- ignoring')
      return cb()
    }
    db.documents.safeInsert(lockDoc, dbOps, function(err, inProgress) {
      if (err) return cb(err)
  ...
  function done(err, out) {
    db.documents.safeRemove({_id: lockDoc._id}, dbOps, function(lockErr) {
