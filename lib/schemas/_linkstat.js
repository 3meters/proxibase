...
function _refresh(options, cb) {
  ...
  db.sys.safeFindOne({_id: lockDoc._id}, dbOps, function(err, inProgress) {
    if (err) return cb(err)
    if (inProgress) {
      log('linkStat refresh already in progress -- ignoring')
      return cb()
    }
    db.sys.safeInsert(lockDoc, dbOps, function(err, inProgress) {
      if (err) return cb(err)
  ...
  function done(err, out) {
    db.sys.safeRemove({_id: lockDoc._id}, dbOps, function(lockErr) {
