...
var link = {
  ...
  validators: {
    insert: [findFromDoc, findToDoc, checkInsert, setOwner],
    update: [extendFromPrevious, findFromDoc, findToDoc, checkUpdate, setOwner],
...
// Strong links are owned by the owner of the _to so that he or she can 
// unlink them if needed
function setOwner(doc, previous, options, next) {
  if (util.strongLink(doc)) doc._owner = options.docTo._owner
  return next()
}
