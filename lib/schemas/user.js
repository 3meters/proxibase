...
var users = {
  ...
  before: {
    ...
    insert: [ scrubNew, lowerCaseEmail, reqValidate, onlyAdminsCanSetDeveloper,
              mustSetFieldsViaApi],
    update: [ lowerCaseEmail, checkChangeRoles, checkChangeOwner,
              onlyAdminsCanSetDeveloper, mustSetFieldsViaApi,
...
// Check change owner
function checkChangeOwner(doc, previous, options, cb) {
  if (!tipe.isDefined(doc.owner)) return cb()
  if (doc._owner === previous._owner) return cb()
  if (tipe.isNull(doc._owner)) return cb(perr.badValue('Cannot set _owner to null'))
  this.db.users.safeFindOne({_id: doc._owner}, _.cloneDeep(options), function(err, newOwner) {
    if (err) return cb(err)
    if (!newOwner) return cb(perr.badValue('User ' + doc._id + ' not found.'))
    cb()
  })
