...
function filterFields(query, options, cb) {
  debug('filterFields called with query', query)
  ...
  if (!options.fields) options.fields = publicFields
  else {
    // safeFind accepts many field formats, call it to convert to object map
