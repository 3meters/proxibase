...
var users = {
  ...
  validators: {
    ...
    insert: [ scrubNew, lowerCaseEmail, parseOauthId, ensureAuthSource,
              reqValidate, onlyAdminsCanSetDeveloper ],
    update: [ lowerCaseEmail, parseOauthId, ensureAuthSource,
              onlyAdminsCanChangeRoles, onlyAdminsCanSetDeveloper,
...
function filterFields(query, options, cb) {
  ...
  if (query._id && options && options.user
      && (query._id === options.user._id
          || (query._id.$in && query._id.$in[0] === options.user._id))) {
    return cb()
  }
...
function authByPassword(credentials, cb) {
  ...
  self.findOne({email: credentials.email}, function(err, user) {
    ...
    if (user._id === util.anonId) return cb(proxErr.badAuthCred())
    ...
    self.safeUpdate(updateDoc, {user: user, asAdmin: true}, cb)
  })
}
...
function genValidationLink(userId, userEmail) {
  return util.config.service.url +
...
function reqValidate(doc, previous, options, cb) {
  ...
  var text = 'Thanks for trying out Candipatch. To complete ' +
      'your signup please click:\n\n    ' + link +
      '\n\nEnjoy!\n\n-The Candipatch Team'

  /* Turned off pending a bug fix
  util.sendMail({
    to: doc.name + ' <' + doc.email + '>',
    subject: 'Complete your Candipatch signup',
    text: text
  })
*/
  cb()
