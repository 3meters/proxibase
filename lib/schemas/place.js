...
var place = {
  ...
  methods: {
    ...
    userCanReadPlaceContent: userCanReadPlaceContent,
  },
}
...
function upsertPlace(place, options, cb) {
  ...
  if (place.name && place.postalCode) {
    conditions.push({$and: [
      {namelc: place.name.toLowerCase()},
      {postalCode: place.postalCode.toLowerCase()},
    ]})
  }
  ...
  for (key in place.provider) {
    ...
    if ('aircandi' === key) continue

    // googleRef is a query token, not a unique Id
    if ('googleRef' === key) continue
...
// async check if user can read content linked to a place
function userCanReadPlaceContent(userId, cb) {

  var place = this

  if (!tipe.isFunction(cb)) cb = logErr
  if (!tipe.isString(userId)) return cb(perr.serverErr('Invalid call: expected string userId'))

  // Public place
  if (!place.restricted) return cb(null, true)

  // User owns place
  if (place._owner === userId) return cb(null, true)

  // Look for a watch link between user and place
  var linkQry = {
    _from: userId,
    _to: place._id,
    type: 'watch',
