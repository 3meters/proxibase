...
function refresh(placeId, user, cb) {
  applinks.get({placeId: placeId, user: user, save: true}, cb)
}


// Find the next place and refresh it.  util.nextId cycles
// through all documents in the collections.  Private API
// ment to be called by the task runner
function refreshNext(cb) {
  util.nextId('places', function(err, placeId) {
    if (err) return cb(err)
    refresh(placeId, util.adminUser, cb)
  })
}
