...
var place = {
  ...
  indexes: [
    ...
    { index: 'provider.google1',     options: { sparse: true, unique: true }},
...
function upsertPlace(place, options, cb) {
  ...
  var err = scrub(place, this.schema.fields, {strict: true})
  if (err) return cb(err)
  debug('place.js place', place)
  ...
  // Google uses a two-part key for a place, separated by a |.  The first
  // part is guarnateed to uniquely identify google's current idea of a
  // place.  The second part is guaranteed to always resolve to a place.
  // I'm not sure whether this is overcomplicated or brilliant.
  if (place.provider && place.provider.google) {
    var idParts = place.provider.google.split('|')
    place.provider.google1 = idParts[0]
    place.provider.google2 = idParts[1]
  }
  ...
  for (key in place.provider) {
    ...
    // don't dedupe custom places
    if (key === 'aircandi') continue

    // google1 holds googles idea of a current place
    if (('google' === key) || ('google2' === key)) continue
  ...
  self.safeFind(query, findOps, function(err, foundPlaces) {
    ...
    for (var key in place.provider) {
      foundPlaces = dupes.filter(providerMatch)
      if (foundPlaces.length) return update(foundPlaces[0], place)
    }
    function providerMatch(pl) {
...
function refreshNext(cb) {
  util.nextDoc('places', function(err, place) {
    ...
    if (!place.applinkDate) return refreshNext(cb)
...
// Does not persist
function merge(place1, place2, options) {
  ...
  // Accept place1's id if it exists
  if (place1._id) delete place2._id

  // Accept the best location fix, favoring google, then foursqure
  var updateLocation = false
  if (place2.location && !place2.provider.yelp) {
    if (place2.provider.google) {
      updateLocation = true
    }
    else {
      if (!place1.provider.google) {
        updateLocation = true
      }
    }
  }
  if (updateLocation) place1.location = place2.location
