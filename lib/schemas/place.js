...
// Override the default safeInsert, safeUpdate, and safeUpsert commands
// with a custom one with special dupe and update semantics
function upsert(place, options, cb) {
  ...
  log('upsert place', place)
  log('upsert options', options)

  options.asAdmin = true
  options.sort = options.sort || [{_id: 1}]

  if (place._id) query = {_id: place._id}
  else {
  ...
  self.safeFind(query, options, function(err, foundPlaces) {
    ...
    if (!calledAsAdmin && foundPlace._owner !== util.adminId && foundPlace._owner !== user._id) {
      return cb(perr.badAuth())
    }

    // We let users update admin-owned photos and merge provider info but nothing else
    if (!calledAsAdmin && (foundPlace._owner === util.adminId)) {
      place = {
        photo: place.photo,
        _modifier: options.user._id,
        provider: place.provider,
      }
      foundPlace = merge(place, foundPlace)
    }
    else foundPlace = merge(foundPlace, place)
    update(foundPlace)
  })

  function update(place) {
    var _safeUpdate = mongo.Collection.prototype.safeUpdate
    _safeUpdate.call(self, place, options, cb)
  }
