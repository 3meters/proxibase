...
function upsertPlace(place, options, cb) {
  ...
  self.safeFind(query, findOps, function(err, foundPlaces) {
    ...
    // Check for _id match
    if (place._id) {
      foundPlaces = dupes.filter(function(pa) {
        if (pa._id === place._id) return pa
      })
    ...
    for (var key in place.provider) {
      foundPlaces = dupes.filter(function(pa) {
        if (!(place.provider && pa.provider)) return
        if (place.provider[key] === pa.provider[key]) return pa
      })  // jshint ignore:line
      if (foundPlaces.length) return update(foundPlaces[0], place)
    }
    ...
    var dupeNames = dupes.map(function(pa) { return pa.name })
    var placeScores = {}
    var winner = 0
    var topScore = 0
...
function isDupe(place1, place2) {
  ...
  var place2KnownToPlace = false
  for (var key in place1.provider) {
    if (place2.provider[key]) place2KnownToPlace = true  // by-product of the walk
