...
function safeInsert(place, options, cb) {
  ...
  self.findOne({$or: conditions}, function(err, foundPlace) {
    if (err) return cb(err)
    if (!foundPlace) return insert()
    foundPlace = merge(foundPlace, place)
    options.asAdmin = true
    return self.safeUpdate(foundPlace, options, cb)
  })
