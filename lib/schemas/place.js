...
function safeInsert(place, options, cb) {
  ...
  var logme = (place.name.match(/^Revel/))
      || place.name.match(/^Quo/)
  if (logme) debug('safe insert of place', place)
  ...
  if (logme) debug('conditions', {$or: conditions})
  ...
  self.safeFind({$or: conditions}, {asAdmin: true}, function(err, results) {
    ...
    if (logme) debug('before merge', foundPlaces[0])
    foundPlaces[0] = merge(foundPlaces[0], place)
    if (logme) debug('after merge', foundPlaces[0])
...
function merge(place1, place2) {
  ...
  place1.provider = _.extend(place1.provider, place2.provider)
  for (key in place2) {
    if (!place1[key]) place1[key] = place2[key]
  }

  // Accept the location with the smallest reported accuracy
  // Consider always accepting googles
  if (place2.location) {
    if (!place1.location) place1.location = place2.location
    else {
