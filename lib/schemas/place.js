...
function upsert(place, options, cb) {
  ...
  self.safeFind(query, findOps, function(err, foundPlaces) {
    ...
    if (foundPlaces.length > 1) {
      ...
      dupes.push({
        query: util.inspect(query, false, 20), // BUG: query is now out of scope
...
function merge(place1, place2, user) {
  ...
  // Prefer foursquare names and categorys, as there are often
  // more foursquare records per physical place than yelp or google
