...
function getUpstreamEntities(doc, previous, options, next) {
  ...
  if ('remove' === options.method) newActivityDate = util.now()
    ...
    if ('update' === options.method
        && doc.activityDate && previous.activityDate
        && doc.activityDate > previous.activityDate) {
      return next()  // exists and is unchanged
    }
    else newActivityDate = doc.activityDate
  }
  // DEBUG: not finished:  this missess set activity date with unset previous activity date
  // newActivity date is never applied to ents to be updated
