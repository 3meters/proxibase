...
function updateActions(doc, previous, options, cb) {
  ...
  db.actions.safeRemove(query, {user: admin}, logDeleteAction)
...
function removeLinks(doc, previous, options, cb) {
  ...
  this.db.links.safeRemove(query, {user: admin}, cb)
}


// Remove the link count records in the stats collection, issue 239
function removeStats(doc, previous, options, cb) {
  debug('removeStats', doc)
  var db = this.db
  db.tos.safeRemove({'_id._to': doc._id}, {user: admin}, function(err, count) {
    debug('tos removed:', count)
    if (err) return cb(err)
    db.froms.safeRemove({'_id._from': doc._id}, {user: admin}, function(err, count) {
      debug('froms removed:', count)
      cb(err)
    })
  })
}
