...
var entity = {
  ...
  validators: {
    update: [getUpstreamStrongLinkedEntities],
    remove: [getUpstreamStrongLinkedEntities],
    afterUpdate: [setUpstreamStrongLinkedEntities],
    afterRemove: [setUpstreamStrongLinkedEntities],
  }
}


function getUpstreamStrongLinkedEntities(doc, previous, options, next) {
...
function setUpstreamStrongLinkedEntities(doc, previous, options, next) {

  var db = this.db

  if (!options.toUpdate) return next()

  async.each(options.toUpdate, updateActivityDate, finish)
