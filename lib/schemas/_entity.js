...
var entity = {
  ...
  validators: {
    update: [getLinkedEntities, preserveModified],
    remove: [getLinkedEntities, removeActions, removeChildren],
  },
...
function getLinkedEntities(doc, previous, options, next) {
...
// When updating the activity date from a child don't change the modified markers
...
function removeChildren(doc, previous, options, cb) {
  ...
  function remove(child, next) {
    ...
    schema.collection.safeRemove({_id: child._id}, {user: util.adminUser}, next)
  }
}
