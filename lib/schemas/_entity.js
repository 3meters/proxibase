...
function addVisibility(query, options, next) {
  ...
  debug('/schemas/_entity options.fields', options.fields)
  // TODO:  uncommenting the following line fails test 2 without the 
  // preceding lines debug statement in the logs.  Curious.   
...
function checkPermissions(err, state, cb) {
  ...
  function checkDoc(doc, nextDoc) {
    safeDocs.push(doc)
    return nextDoc()
    var visibility = doc.visibility
    if (options.deleteVisibility) delete doc.visibility
    if ('public' === visibility) safeDocs.push(doc)
    else {
      // TODO: implement private and hidden
      safeDocs.push(doc)
    }
    nextDoc()
  }
  ...
  function finish(err) {
    if (err) return cb(err)
    if (state.docs) return cb(null, safeDocs, {count: safeDocs.length, more: state.more})
