...
var entity = {
  ...
  before: {
    ...
    remove: [getLinked, removeLinks, removeStats],
  },
...
function getLinked(doc, previous, options, next) {
  ...
  var linkedOps = adminOps(options)
  linkedOps.linked = [
...
function updateLinkedActivityDates(state, cb) {
  ...
  function updateLinkedEnt(ent, nextEnt) {
    ...
    this.db[clName].updateActivityDate(ent._id, ent.activityDate, actOps, nextEnt)
  }
...
function updateActivityDate(_id, newDate, options, cb) {
  // debug('updating actdt for ' + _id + ' to ' + newDate)
  var update = {
    _id: _id,
    activityDate: newDate
  }
  this.safeUpdate(update, adminOps(options), cb)
}


// Utility to set db options for asAdmin calls
function adminOps(options) {
  return _.assign({asAdmin: true}, _.pick(options, ['tag', 'test', 'log']))
}
