...
function _refresh(options, cb) {
  ...
  function startMapReduce(firstLinkId, lastLinkId) {
    ...
    db.links.mapReduce(map, reduce, mrOps, function(err, mrStats) {
      if (err) return cb(err)

      // debug('mrStats', mrStats)
      mrStats = {}
      ...
      function finish(err, results) {
        ...
        db.documents.safeUpsert(lastStatDoc, {user: util.adminUser}, function(err, savedStatDoc) {
