...
var link = {
  ...
  before: {
    insert: [checkInsertPermissions, setEnabled],
    update: [],
...
function checkInsertPermissions(doc, previous, options, next) {
...
function setEnabled(doc, previous, options, next) {
  ...
  doc.enabled = false
  // TODO: check for an outstanding share invite to the place, and set
  // enabled to true if one exits.
  next()
}
