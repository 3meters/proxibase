...
function canLinkTo(doc, previous, options, next) {
  ...
  if (docTo.restricted && !options.asReader
    ...
    // options.clTo may be a public, yet restricted place, look for explicity watch link
...
function decrementStats(doc, previous, options, next) {
  ...
  function decrementTos() {
    db.tos.findOne(toQuery, function(err, doc) {
      ...
      if (tipe.isNumber(doc.value) && (doc.value >= 1)) {
        ...
        db.tos.update(toQuery, {$set: {value: doc.value - 1}}, function(err) {
          if (err) return fail(err, toQuery)
          decrementFroms()
        })
      }
      else {
        logErr('decrementTos: Unexpected value from toQuery', toQuery)
        logErr('tos doc:', doc)
        decrementFroms()
      }
    })
  ...
  function decrementFroms() {
    db.froms.findOne(fromQuery, function(err, doc) {
      ...
      if (!doc) return next()
      if (tipe.isNumber(doc.value) && (doc.value >= 1)) {
        db.froms.update(fromQuery, {$set: {value: doc.value - 1}}, function(err) {
          if (err) return fail(err, fromQuery)
          next()
        })
      }
      else {
        logErr('decrementFroms: Unexpected value from fromQuery', fromQuery)
        logErr('froms doc:', doc)
        next()
      }
    })
  }
