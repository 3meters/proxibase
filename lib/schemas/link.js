...
function canReadTo(link, previous, options, next) {
  ...
  this.db[clTo].safeFindOne({_id: link._to}, options, function(err, docTo, updatedOps) {
...
function canLinkTo(doc, previous, options, next) {
  ...
  if (docTo.restricted && !options.asReader
    ...
    this.db[options.clTo].userIsWatching(options.user._id, options.docTo, function(err, isWatching) {
      if (err) return next(err)
...
function after(err, state, cb) {
  ...
  function setActivityDate(next) {

    if (!docTo) return next()  // many reasons this can be ok
  ...
  function setFromAcl(next) {
    ...
    db[clFrom].safeUpdate(update, {user: options.user}, function(err, savedFrom) {
      if (err) return next(err)
      if (savedFrom) docFrom = savedFrom
      next()
    })
  }
  ...
  // log actions
  function logActions(next) {

    // Can be passed in by the client or constructed
    ...
    db.actions.safeInsert(action, actionOps, function(err, savedAction, newMeta) {
      if (err) {
      ...
      if (newMeta && newMeta.notifications) meta.notifications = newMeta.notifications
      return next()
    })
  }

  // Construct the action event on the fly
  function buildActionEvent() {
    switch (state.method) {

      case 'insert': {
        if (link.type === 'like') return "like_entity_" + link.toSchema
        if (link.type === 'watch' && link.toSchema === 'patch') {
          if (link.enabled) return "watch_entity_patch"
          else return "request_watch_entity"  // should be patch, but backward compat
        }
        break
      }

      case 'update': {
        if (link.enabled && !state.previous.enabled) {
      ...
      case 'remove': {
        if (link.type === 'watch') return "unwatch_entity_" + link.toSchema
        if (link.type === 'content') return "unlink_entity_" + link.toSchema
        break
      }
    }
  }
...
function isStrong(link) {
  ...
  else if (statics.typeLike === link.type) return true
  else return false
}
