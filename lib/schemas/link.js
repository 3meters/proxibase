...
function canReadTo(link, previous, options, next) {
  ...
  var ops = _.clone(options)
  ops.refs = 'name,schema,photo'  // for notifications
...
function after(err, state, cb) {
  ...
  function sendNotifications(next) {
    ...
    var sendOps = {
      event:      actionEvent,
      triggers:   ['own_to', 'watch_to'],  // nearby is handled for android client by do/insertEntity
      toId:       link._to,
      to:         docTo,
      fromId:     link._from,
      from:       docFrom,
  ...
  function buildActionEvent() {
    switch (state.method) {
      ...
      case 'insert': {
        ...
        if (link.type === 'content' && link.toSchema === 'patch') return "insert_entity_" + link.fromSchema + '_content'
        if (link.type === 'share') return "insert_entity_" + link.fromSchema + '_share'
        break
      }
