...
function setOwnership(doc, previous, options, next) {
  ...
  if (doc.provider && !doc.provider['aircandi']) {
...
function safeInsert(place, options, cb) {
  ...
  self.findOne({$or: conditions}, function(err, splace) {
    ...
    /*
    if (options.user && options.user._id && 'user' === options.user.role) {
      var tempOps = util.clone(options)
      tempOps.asAdmin = true
    }
    */
