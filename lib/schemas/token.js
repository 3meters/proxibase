...
function gen(email, ops, cb) {
  ...
  db.users.safeFind({email: email}, ops, function(err, users) {
    ...
    if (!users.length) return cb(perr.notFound(email))
    ...
    if (user._id === util.adminId || user.role === 'admin') {
      ...
      return cb(perr.notFound(email))  // Same return as a missing email address
    }
    ...
    db.tokens.safeFind({_user: user._id}, findOps, function(err, tokens){
      ...
      var hashData = [user._id, user.email, serverKey, 'ksiiwks']
...
function spend(token, ops, cb) {
  ...
  db.tokens.safeFind({
    ...
    if (err) return cb(err)
    if (!tokens.length) return cb(perr.notFound())
