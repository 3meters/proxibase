...
function spend(token, ops, cb) {
  ...
  db.tokens.safeFind({
    ...
    var token = tokens[0]
    // Remove, not safe remove, to remove all tokens created by this
    // user so that they don't count against their five token limit
    db.tokens.remove({_user: token._user}, function(err) {
      if (err) return cb(err)
      cb(null, token._user)
    })
  })
