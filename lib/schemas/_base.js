  ...
  if (!previous) {  // insert
    ...
    if (!doc._id) {
      if (doc.type === util.statics.typeBeacon) {
        if (!(doc.beacon && doc.beacon.bssid)) return next(proxErr.missingParam('bssid'))
        doc._id = util.statics.collectionIds.entities + '.' + doc.beacon.bssid
      }
      else {
      ...
      var id = util.parseId(doc._id)
      if (id.collection.id !== collectionId) {
