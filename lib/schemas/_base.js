...
function setSystemFields(doc, previous, options, next) {
  ...
  if (this.schema.version) doc.schemaVersion = this.schema.version
  ...
  if (options.adminOwns && options.user.doNotTrack) {
    userId = anonId  // the anonymous user: for shy persons creating admin-owned objects
  }
  ...
  if (!previous) {  // insert
    ...
    if (!doc._id) {
      doc._id = this.genId(schemaId, doc)
...
function genId(schemaId, doc) {
  return util.genId(schemaId, doc.createdDate)
}
...
// safeguard, this routine logs the activity
function logAnonymousWrites(doc, previous, options, next) {
  var anonId = util.anonUser._id
  if (!((anonId === doc._creator) || (anonId === doc._modifier))) return next()
