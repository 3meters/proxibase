...
var base = {
  ...
  fields: {
    ...
    schema:         { type: 'string' },
    _acl:           { type: 'string' },
    _place:         { type: 'string' },  // deprecated, use _acl
...
function readWithAcl(cl, _id, options, cb) {
  ...
  cl.safeFindOne({_id: _id}, elevated, function(err, doc) {
    ...
    function checkAcl(err, acl) {
      ...
      // Public place
      if (acl.schema === 'place' && acl.visibility === 'public') return isReader()

      // User owns acl
      if (acl._owner === userId) return isReader()

      userIsWatching(userId, doc, function(err, isWatching) {
        if (err) return cb(err)
        if (isWatching) return isReader()

        userIsSharing(userId, doc, function(err, isSharing) {
          if (err) return cb(err)
          if (isSharing) return isReader()
          cb() // user cannot read
        })
      })

      function isReader() {
        options.isReader = true
        cb(null, doc, options)
      }
    }
  })
