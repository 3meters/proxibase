...
function setCanEdit(err, scope, cb) {  // jshint ignore:line
  ...
  if (scope.options.method === 'findOne' && !scope.options.skipCanEdit) {
    scope.options.canEdit = userCanEdit(scope.options.user, scope.results)
  }
  cb(err, scope.results, scope.options)
}
...
function ownerAccess(query, options, next) {
  ...
  readWithAcl(this, query, options, function(err, doc) {
...
function readWithAcl(cl, query, options, cb) {
  ...
  var elevated = _.cloneDeep(options)
  elevated.asAdmin = true
  elevated.skipCanEdit = true  // don't set the canEdit flag using elevated permissions
  delete elevated.refs
  delete elevated.linked
  delete elevated.links

  var userId = elevated.user._id

  cl.safeFindOne(query, elevated, function(err, doc) {
    if (err) return cb(err)
    if (!doc) return cb(null, doc)  // may be null
    ...
    function checkAcl(err, acl) {
      ...
      // Public patch
      if (clName === 'patches' && acl.visibility === 'public') return cb(null, doc)

      // User owns acl
      if (acl._owner === userId) return cb(null, doc)

      userIsWatching(userId, doc, options, function(err, isWatching) {
        if (err) return cb(err)
        if (isWatching) return cb(null, doc)

        userIsSharing(userId, doc, options, function(err, isSharing) {
          if (err) return cb(err)
...
function userIsWatching(_user, doc, options, cb) {
  ...
  this.db.links.safeFindOne(linkQry, ops, function(err, watchLink) {
    if (err) return cb(err)
...
function userIsSharing(_user, doc, options, cb) {
  ...
  this.db.links.safeFindOne(linkQry, ops, function(err, shareLink) {
    if (err) return cb(err)
