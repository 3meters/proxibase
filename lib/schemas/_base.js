...
function setCanEdit(err, scope, cb) {  // jshint ignore:line
  ...
  if (scope.docs) return cb(null, scope.docs, scope.options)
  // safeFindOne, set canEdit flag
  scope.options.canEdit = userCanEdit(scope.options.user, scope.doc)
  cb(err, scope.doc, scope.options)
}
...
function ownerAccess(query, options, next) {
  ...
  if (options.user && options.method !== 'findOne') {
  ...
  readWithAcl(this, query._id, options, function(err, doc, modifiedOps) {
    ...
    // right answer. Setting options.result tells safeFind to
    // send that result without bothering to requery the db
