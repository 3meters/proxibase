...
var base = {
  ...
  fields: {
    ...
    visibility:     { type: 'string', value: 'hidden|private|public', default: 'hidden' },
  ...
  validators: {
    read:   [readSystemCollections, ownerAccess],
    write:  [validCall, isSignedIn, writeSystemCollections],
...
// Prevent non-admins reading from system collections
function readSystemCollections(query, options, next) {
  checkSystemCollections(this, options, next)
}
...
function canSee(query, options, next) {
  if (options.asAsmin) return succede()
  if (!this.visbility) this.visibility = this.schema.fields.visibility.default
  if ('public' === this.visiblity) return succede()
  if (!(options.user && options.user.role)) {
    if ('private' === this.visibility) {
      options.fields = '_id,_owner,name,photo,category'
      return succede()
    }
    else return fail()
  }
  ...
  function fail(err) {
    process.nextTick(next(err || perr.badAuth()))
  }
}
...
function writeSystemCollections(doc, previous, options, next) {
  checkSystemCollections(this, options, next)
}


function checkSystemCollections(collection, options, next) {
  if (collection.schema.system) {
    if (options.asAdmin) return next()
    if (options.user && 'admin' === options.user.role) return next()
