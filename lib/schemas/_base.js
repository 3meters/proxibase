...
var base = {
  ...
  fields: {
    ...
    locked:       { type: 'boolean', default: false },    // updates and link to's restricted to owner
    enabled:      { type: 'boolean', default: true },     // visible/invisible system wide
    system:       { type: 'boolean' },                    // only used by the system
    data:         { type: 'object' },                     // map used to store opaque data for the service
...
function setSystemFields(doc, previous, options, next) {
  ...
  if (!(userIsAdmin && doc._modifier)) doc._modifier = options.user._id
