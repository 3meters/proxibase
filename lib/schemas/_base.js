...
function readWithAcl(cl, _id, options, cb) {
  ...
  cl.safeFindOne({_id: _id}, elevated, function(err, doc) {
    ...
    var aclId = util.parseId(doc._acl)
    if (!aclId.collectionName) return cb()

    db[aclId.collectionName].safeFindOne({_id: doc._acl}, elevated, checkAcl)
    ...
    function checkAcl(err, acl) {
      ...
      // Public place
      if (acl.schema === 'place' && acl.visiblity === 'public') return cb(null, doc)

      // User owns acl
      if (acl._owner === userId) return cb(null, doc)

      // Look for a watch link
      var linkQry = {
        _from: userId,
        _to: acl._id,
        type: 'watch',
      ...
      db.links.safeFindOne(linkQry, {asAdmin: true}, function(err, watchLink) {
        if (err) return cb(err)
        if (watchLink) return cb(null, doc)

        // Look for a share link to a message
        linkQry = {
          fromSchema: 'message',
          _from: acl._id,
          _to: userId,
        ...
        db.links.safeFindOne(linkQry, {asAdmin: true}, function(err, shareLink) {
          if (err) return cb(err)
          if (shareLink) return cb(null, doc)
...
function checkAcl(doc, previous, options, next) {
  ...
  debug('parsedId', parsedId)
  doc.aclSchema = parsedId.schemaName
  var clAcl = db[parsedId.collectionName]

  clAcl.safeFindOne({_id: doc._acl}, {user: options.user}, function(err, acl) {
    if (err) return next(err)

    if (!acl) return next(perr.badAuth('User cannot read ' + doc.aclSchema + ' ' + doc._acl))
    next()
  })
}
