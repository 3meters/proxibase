...
function ownerAccess(query, options, next) {
  ...
  if (this.schema.public) return next()
  ...
  readWithAcl(this, query, options, function(err, doc, canReadLinked) {
    ...
    if (canReadLinked) options.asReader = true  // means can read docs children, risky
...
function readWithAcl(cl, query, options, cb) {
  ...
  cl.safeFindOne(query, elevated, function(err, doc) {
    ...
    function checkReadAcl(err, acl) {
      ...
      // Public patch
      if (acl.schema === 'patch' && acl.visibility === 'public') return cb(null, doc, true)

      // User owns acl
      if (acl._owner === userId) return cb(null, doc, true)

      userIsWatching.call(cl, userId, doc, options, function(err, isWatching) {
        if (err) return cb(err)
        if (isWatching) return cb(null, doc, true)

        userIsSharing.call(cl, userId, doc, options, function(err, isSharing) {
          if (err) return cb(err)
          if (isSharing) return cb(null, doc, false)  // do not have right to read linked
          ...
          if (sch.public && !sch.system) return cb(null, doc, false)
