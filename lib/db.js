...
mongo.initDb = function(config, cb) {
  ...
  var statDir = path.join(__dirname, './stats')

  db.open(function(err) {
    if (err) return cb(err)
    loadSchemas(schemaDir, function(err) {
      if (err) return cb(err)
      loadSchemas(statDir, function(err) {
        if (err) return cb(err)
        if (config.log > 1) log('Schemas:', db.safeSchemas)
        if (config.db.ensureUsers) ensureDefaultUsers()
        else finish()
      })
    })
  })
  ...
  function loadSchemas(dir, cb) {
    ...
    var ensureIndexes = config.db.ensureIndexes
    ...
    function next(err) {
      if (err) return cb(err)
      if (ensureIndexes) log('Ensured indexes')
      cb()
    }
  }
