...
Collection.safeInsert = function(docs, options, callback) {
  var doc = docs
...
Collection.safeUpdate = function(query, update, options, callback) {
  if (options && (options.upsert || options.multi)) {
    if (options.skipValidation) {
      ...
      callback = callback || console.error
      return callback(error('update with multi or upsert does not support ' +
...
Collection.safeRemove = function(query, options, callback) {
...
function validateAndRun(collection, method, query, docs, options, callback) {
  ...
  if (query && 'insert' !== method) {
    collection.find(query, options, function(err, found) {
      ...
      if (found.length > 1) return errorCb('MongoSafe can only affect one record at a time')
  ...
  function executeMongoCommand(err, doc) {
    ...
    switch(method) {
      case 'insert':
        ...
        if (err) return errorCb(err)
        return Collection.insert.call(collection, doc, options, callback)
        break
      case 'update':
        var err = checkFields(doc)
        if (err) return errorCb(err)
        doc = prepareUpdate(doc)
        return Collection.update.call(collection, query, doc, options, handleUpdates)
        break
      case 'remove':
        return Collection.remove.call(collection, query, options, callback)
        break
      default:
