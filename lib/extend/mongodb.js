...
function validateAndRun(collection, method, query, doc, options, callback) {
  ...
  log('debug collection.collectionName', collection.collectionName)
  ...
  if (query && 'insert' !== method) {
    collection.find(query, options, function(err, found) {
      if (err) return callback(err)
      if (found.length > 1) {
        return callback(proxErr.badRequest('Safe requests can only affect one record at a time.'))
      }
  ...
  function executeMongoCommand(err, doc) {
    log('debug2 collection.collectionName', collection.collectionName)
    if (err) return callback(err)
  ...
  function handleUpdates(err, count) {
    ...
    if (err) return callback(err)
    // log('pt:', Object.keys(Collection.prototype))
    // log('find:', Collection.prototype.find)
    return Collection.prototype.find.call(collection, query, callback)
  }
}
...
function prepareUpdate(doc, previous) {
  ...
  delete doc._id
  doc = {$set: doc}
  log('debug prepare update doc:', doc)
  return doc
}
