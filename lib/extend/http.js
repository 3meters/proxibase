...
express.response.error = function(err, info) {
  ...
  if (typeof(err) === 'number' && errCodeMap[err]) {
    err = new HttpErr(errCodeMap[err])
  }
  ...
  // Give up and let V8 Error discern what the caller meant
  ...
  // err.stack exists but is not ennumerable
