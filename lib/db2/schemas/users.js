...
var log = util.log
var users = {} // TODO: bootstrapping problem
var assert = require('assert')
var crypto = require('crypto')
var mongodb = require('mongodb')
var users = Object.create(mongodb.schema)
...
function clearValidationDateOnEmailChange(doc, previous, options, next) {
  if (doc.email && previous && doc.email !== previous.email) {
    doc.validationDate = null
    doc.validationNotifiyDate = null
    users.reqValidate(doc, options, next)
  }
  next()
}
...
// Verify Password
function verifyPassword(doc, options, passPlain) {
  if (passPlain) {
...
users.methods.reqValidate = function(doc, options, callback) {
...
users.methods.reqResetPassword = function(doc, options, callback) {
...
users.methods.resetPassword = function(doc, options, callback) {
  // TODO: implement
...
users.methods.changePassword = function(doc, options, callback) {

  assert(options.oldPass && options.newPass, 'Invalid call to changePassword')

  // Admins can change anyone's password to anything
  if (options.user.role !== 'admin') {

    // Users can only change their own password
    if (options.user._id !== doc._id) {
      return next(proxErr.badAuth())
    }

    // If they know their old password
    if (!verifyPassword(options.oldPass)) {
      return next(proxErr.badAuthCred())
    }

    // And they make a strong one
    if (!ensurePasswordStrength(options.newPass, doc.name)) {
      return next(proxErr.badPassword())
    }
  }
