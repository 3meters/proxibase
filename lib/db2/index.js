...
exports.init = function(config, callback) {
  ...
  function loadSchemas() {
    ...
    var baseSchema = require(path.join(schemaDir, '_base')).getBaseSchema()
    ...
    fs.readdirSync(schemaDir).forEach(function(fileName) {
      if (path.extname(fileName) === '.js') {
        ...
        if (module.getSchema) {
          ...
          var schema = createMergedSchema(baseSchema, module.getSchema())
  ...
  /*
   * Merge one schema onto another leaving the originals unmodifed.
   * For ids and fields add overwrites origin.  For indexes and
   * validators, add appends to origin's
   */
  function createMergedSchema(origin, add) {
    // log('debug mongoskin.schema 1', mongoskin.schema)
    // var schema = Object.create(mongoskin.schema)
    var schema = util.clone(mongoskin.schema)
    // log('schema prototype:', Object.getPrototypeOf(schema))
    // log('mongoskin.schema prototype:', Object.getPrototypeOf(mongoskin.schema))
    // if(Object.getPrototypeOf(schema) === mongoskin.schema) log('yes')
    // else log('no')
    schema.id = add.id || origin.id || 0
    // log('debug schema.id', schema.id)
    // log('debug monogskin.schema 2', mongoskin.schema)
    util.extend(schema.fields, origin.fields)
    // log('debug mongoskin.schema 3', mongoskin.schema)
    util.extend(schema.fields, add.fields)
    schema.indexes = schema.indexes.concat(origin.indexes)
    schema.indexes = schema.indexes.concat(add.indexes)
    util.extend(schema.validators, origin.validators)
    for (var method in add.validators) {
      schema.validators[method] = schema.validators[method].concat(add.validators[method])
    }
    // prepend the all validators to each of the method validators
    schema.validators.insert = schema.validators.all.concat(schema.validators.insert)
    schema.validators.update = schema.validators.all.concat(schema.validators.update)
    schema.validators.remove = schema.validators.all.concat(schema.validators.remove)
    delete schema.validators.all
    // log('debug mongoskin.schema end ', mongoskin.schema)
    log('debug schema ', schema)
    return schema
  }
