...
exports.init = function(config, callback) {
  ...
  function loadSchemas() {
    ...
    fs.readdirSync(schemaDir).forEach(function(fileName) {
      if (path.extname(fileName) === '.js') {
        ...
        if (module.getSchema) {
          ...
          connection.bind(schemaName, methods) // from mongoskin
          var schema = createMergedSchema(baseSchema, module.getSchema())
          schema.name = schemaName // from the file name
          schemas.push(schema)
        }
      }
    })
    schemas.sort(function(a, b) { return a.id - b.id })  // sort map by id
    schemas.forEach(function(schema) {
      connection.schemas[schema.name] = schema
    })
    log('schemas', connection.schemas)
    process.exit(1)
    ...
    log('Loaded schemas')
    if (config && config.log > 1) logRefs()
    logRefs()  // temp
    ensureIndexes()
  }
   ...
   * For ids, fields, and indexes, add overwrites origin.  For
   * validators, add appends its validator routines to origin's
   */
  function createMergedSchema(origin, add) {
    ...
    util.extend(schema.indexes, origin.indexes)
    util.extend(schema.indexes, add.indexes)
