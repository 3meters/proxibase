...
exports.init = function(config, callback) {
  ...
  /*
   * Merge one schema onto another.  Not a simple property merge
   * because the validators are arrays of functions that need to be
   * concatenated, not overwritten
   */
  function extendSchema(origin, add) {
    var addVals = add.validators
    delete add.validators
    utils.extend(origin, add)
    var validators = {
      all: [],
      insert: [],
      update: [],
    ...
    util.extend(validators, origin.validators)
    for (var method in validators) {
      if (addVals[method]) {
        validators[method] = validators[method].concat(addVals[method])
      }
    }
    origin.validators = validators
    return origin
  }
