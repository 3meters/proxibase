...
exports.init = function(config, callback) {
    ...
    fs.readdirSync(schemaDir).forEach(function(fileName) {
      if (path.extname(fileName) === '.js') {
        ...
        if (module.getSchema) {
          ...
          Object.defineProperty(schema, 'name', {value: schemaName}) // non-enumerable
          schemas.push(schema)
        }
      }
    })
  ...
  function createMergedSchema(origin, add) {
    ...
    if (origin.indexes) schema.indexes = schema.indexes.concat(origin.indexes)
    if (add.indexes) schema.indexes = schema.indexes.concat(add.indexes)
