...
function startAppServer() {
  ...
  // In production the keys are shared, and must be signed for: host, www.host, and api.host
 ...
 // this little hack lets nodemon be my forever and restart the server
process.on('uncaughtException', function(err) {
  console.error(err);
  console.trace();
  if (serverStarted && config.env === 'production') {
    fs.writeFileSync("./appCrashed.js", "// App crashed " + Date() + "\n");
  }
  process.exit(1);
});
