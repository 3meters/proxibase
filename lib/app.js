...
function validateVersion(req, res, next) {
  ...
  var serviceVersionString = util.config.service.version
  var clientVersionString = semver.valid(req.query.version)
  if (!clientVersionString) return res.error(proxErr.badValue(req.query.version))
  var sver = parseVersionString(serviceVersionString)
  var cver = parseVersionString(clientVersionString)
  log('sver', sver)
  log('cver', cver)
  next()

  function parseVersionString(vs) {
    var elem = vs.split('.')
    return {
      major: elem[0],
      minor: elem[1],
