...
function main(req, res) {
  ...
  // If called with no search parameters default to users and patches
  if (!options.places && !options.users && !options.patches) {
    options.users = true
...
function run(options, cb) {
  ...
  function findProviderPlaces() {
    ...
    // Google docs say location should be location=lat,lng but it does not work
    if (options.location) {
      query.location = options.location
      query.radius = options.radius || 1000
    }
