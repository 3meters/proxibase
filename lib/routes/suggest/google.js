...
function get(options, cb) {
  ...
  var search = options.search || {
    path: 'autocomplete/json' || options.path,
    query: {
      input: options.input,  // for place search
  ...
  util.callService.google(search, function(err, res, body) {
    ...
    function finish(err) {
      // Sometimes google gets confused between its summary and detail results,
      // and two separate summary places will return the same google detail place.
      // This next code uniqifies the results based on Google's unique id.
