...
function run(options, cb) {
  ...
  if (options.users) {
    if (options.aircandiFts) findLocal.push(findStoredUsersFts)
    if (options.aircandiRe) findLocal.push(findStoredUsersRe)
  }

  if (options.places) {
    if (options.aircandiFts) findLocal.push(findStoredPlacesFts)
    if (options.aircandiRe) findLocal.push(findStoredPlacesRe)
  }
  ...
  function findStoredPlacesRe() {
    findStoredEntityRe('places', cb)
  }

  function findStoredUsersRe() {
  ...
  function findProviderPlaces() {
    ...
    // Google
      ...
      ents.forEach(function(ent) {
        if ('place' === ent.schema && place.provider && place.provider.google) {
