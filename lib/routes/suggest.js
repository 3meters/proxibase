 ...
 *   patch provider.  Currently only Google and Foursquare are supported.
 */
...
function run(options, cb) {
  ...
  function findEntitiesFts(clName, cb) {
    ...
    // if ((clName === 'patches') && nearby) cmd.filter = nearby
  ...
  function flagWatchedEnts() {
    ...
    if (!options._user) return findProviderPatches()
  ...
  // Find patches from external providers
  function findProviderPatches() {

    if (!(options.patches && options.provider)) return assembleEnts()
    ...
    if (options.provider === 'foursquare' && options.input.length >= 3
      ...
      ents.forEach(function(ent) {
        if ('patch' === ent.schema && ent.provider && ent.provider.foursquare) {
    ...
    // Google
      ...
      ents.forEach(function(ent) {
        if ('patch' === ent.schema && ent.provider && ent.provider.google) {
      ...
      // See https://developers.google.com/patches/documentation/autocomplete#location_biasing
      if (options.location) {
        query.location =  options.location.lat + ',' + options.location.lng
        query.radius = options.radius || 100000
      }

      // Returns a maximum of 5 patch suggestions
      options.search = {
  ...
  // Process the results
  function processProviderResults(err, providerPatches) {
    if (err) return cb(err)

    providerPatches.forEach(function(patch) {
      var provider = patch.provider
