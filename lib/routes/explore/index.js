...
function showQueryForm(req, res, next) {
  ...
  var qlen = Object.keys(req.query).length
  if (qlen) {
    req.viewData.displayPath = req.query.path
    delete req.query.path
    if (qlen > 1) {
      req.viewData.displayQuery = util.inspect(req.query)
      req.viewData.displayJSON = JSON.stringify(req.query)
    }
  }
...
function runQuery(req, res, next) {
  var cmd, url, path, clean, queryString, body = req.body
