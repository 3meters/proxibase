...
function showQueryForm(req, res, next) {
  res.render('explore', req.expData)
}
...
function runQuery(req, res, next) {
  ...
  debug('req.body', body)
  var clean = true
  ;['sort', 'query', 'links'].forEach(function(okey) {
    if (!clean) return
    if (!body[okey]) return
    try { body[okey] = JSON.parse(body[okey]) }
    catch(err) { body[okey] = err; clean = false }
  })
  if (!clean) return res.send(400, body)
  body.count = tipe.truthy(body.count)
  var svcReq = {
    method: 'post',
    path: '/find/' + body.collection
  }
  if (body._id) svcReq.path += '/' + body._id
  delete body.collection
  delete body._id
  svcReq.body = body
  if (req.session && req.session.user) {
    svcReq.body.user = req.session.user._id
    svcReq.body.session = req.session.user.session
  }
  sreq(svcReq, function(err, sres, body) {
    if (err) return next(err)
    res.send(body)
