...
function showNewAccountForm(req, res) {
...
function createAccount(req, res, next) {
  ...
  util.request
    ...
    .end(function(err, sres, body) {
      ...
      res.redirect(req.apiVersionPrefix + '/explore')
    })
}
...
function signin(req, res, next) {
  ...
  .end(function(err, sres, body) {
    ...
    req.session.user = body.user
    req.session.user.session = body.session.key
    res.redirect(req.apiVersionPrefix + '/explore?' + qs.stringify({
      user: body.user._id,
...
function buildPath(req) {
  ...
  var body = req.body || {}
  var options = _.extend(body, req.query)
  var path = req.apiVersionPrefix + '/find'
  var cl = options.collection || req.params.collection
