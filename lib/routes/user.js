...
function invite(req, res) {
  ...
  var invitePath = path.join(statics.assetsDir, 'html/invite.html')
  fs.readFile(invitePath, {encoding: 'utf8'}, function(err, rawMsg) {

    if (err) return res.error(err)

    async.each(emails, whitelistUser, sendInviteEmails)
    ...
    function whitelistUser(email, cb) {
      ...
      db.documents.safeInsert(doc, req.dbOps, cb)
    }
...
function changePassword(req, res, cb) {
  ...
  users.changePassword({
    ...
    db.sessions.safeRemove({
      _id: req.session._id
    }, {user: req.user, asAdmin: true}, function(err) {
      if (err) return cb(err)
    ...
    db.installs.findOne({_id: genInstallId}, function(err, install) {
      ...
      db.sessions.safeRemove({
        ...
        users.reqResetPassword(user, function(err, savedUser) {
          if (err) return res.error(err)
          auth.upsertSession(savedUser, installId, 30*60*1000, function(err, session) {  // 30 minutes
            if (err) return res.error(err)
...
function resetPassword(req, res, cb) {
  ...
  function(err, foundUser) {
    ...
    db.users.resetPassword(req.user, req.body.password, function(err, savedUser) {
      if (err) return cb(err)
      auth.upsertSession(savedUser, req.body.installId, null, function(err, session) {
        if (err) return res.error(err)
        res.send({user: savedUser, session: session})
      })  // Default timeout
    })
