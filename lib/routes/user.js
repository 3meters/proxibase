...
create = function(req, res, cb) {
  ...
  wasUserInvited(body.data.email, function(err, invited) {
    if (err) return cb(err)
    if (invited) body.data.validationDate = util.now()  // Benefit of the doubt
...
// the test suite.
function wasUserInvited(email, cb) {
...
changePassword = function(req, res, cb) {
  ...
  users.changePassword({
    ...
    db.sessions.safeRemove({
      _id: req.session._id
    }, {user: req.user}, function(err, count) {
      if (err) return cb(err)
...
reqResetPassword = function(req, res) {
  ...
  users.findOne({
    ...
    var installId = db.installs.genId({installationId: installationId})
    debug('installId', installId)
    db.installs.findOne({_id: installId}, function(err, install) {
      if (err) return res.error(err)
      ...
      db.sessions.safeRemove({
        _owner: user._id,
        _install: installId,
      }, {user: util.adminUser}, function(err, count) {
        if (err) return res.error(err, count)
        users.reqResetPassword(user, function(err, savedUser) {
          if (err) return res.error(err)
          auth.upsertSession(savedUser, installationId, 30*60*1000, function(err, session) {  // 30 minutes
            if (err) return res.error(err)
...
resetPassword = function(req, res, cb) {
  ...
  function(err, foundUser) {
    ...
    db.users.resetPassword(req.user, req.body.password, function(err, savedUser) {
      if (err) return cb(err)
      auth.upsertSession(savedUser, req.body.installationId, null, function(err, session) {
        if (err) return res.error(err)
        res.send({user: savedUser, session: session})
      })  // Default timeout
    })
