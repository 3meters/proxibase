...
reqResetPassword = function(req, res) {
  ...
  users.findOne({
    ...
    db.installs.findOne({
      ...
      db.sessions.remove({  // bypassing safeRemove for convienence
        _owner: user._id
      }, function(err) {
        if (err) return res.error(err)
        var resetUserRole = {
        ...
        db.users.safeUpdate(resetUserRole, dbOps, function(err, savedUser) {
          if (err) return res.error(err)
          auth.upsertSession(req, res, user, 30*60*1000)  // 30 minutes
        })
      })
    })
  })
...
resetPassword = function(req, res, cb) {
  ...
  /*
  * look up user.  if role !== 'reset' return badauth
  * call user.reset password.  might need to validate 
  * password separately
  * if passes, set user role to 'user'
  * If successful either delete and recreate new session
  * or update the session expiration date to the normal
  * return success
  */
