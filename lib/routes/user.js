...
var _getNotifications = require('./do/getNotifications').main   // not run
...
function create(req, res, cb) {
  ...
  users.safeInsert(userDoc, options, function(err, savedUser) {
    ...
    function autowatch(entityId, nextEntity) {
      ...
      db.links.safeInsert(watchLink, {user: savedUser}, nextEntity)
    }
...
// Translate the rest-formated params to do-formatted params
function getNotifications(req, res) {

  if (!req.user) return res.error(perr.badAuth())

  req.body.entityId = req.user._id
  req.body.cursor = {
    sort: req.query.sort,
...
function deleteUser(req, res) {
  ...
  db.users.safeRemove({_id: req.params.id}, req.dbOps, function(err, userMeta) {
    ...
    async.eachSeries(['patches', 'messages'], deleteOwned, finish)

    function deleteOwned(clName, nextCl) {

      var findOwnedOps = _.assign(req.dbOps, {fields: {_id: 1}})

      db[clName].safeFind({_owner: _id}, findOwnedOps, function(err, owned) {
        ...
        function deleteOwnedDoc(docId, nextDoc) {
          db[clName].safeRemove({_id: docId}, req.dbOps, function(err, meta) {
            if (err) return finish(err)
            if (meta && meta.count) {
