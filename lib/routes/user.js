...
function service(req, res, next) {
  ...
  if (!methods[method]) return res.error(new HttpErr(httpErr.notFound))
...
create = function(req, res, next) {
  ...
  if (!(req.body && req.body.data &&
    ...
    return res.error(new HttpErr(httpErr.missingParam,
        ['data.email', 'data.password', 'secret']))
  }

  if (util.config.service.newAccountSecret !== req.body.secret) {
    return res.error(new HttpErr(httpErr.notHuman))
  }
  ...
  newUser.save(function (err, savedUser) {
    ...
    if (!savedUser) return res.error(new HttpErr(httpErr.serverError))
...
changePassword = function(req, res, next) {
  ...
  users.findOne({_id: user._id }, function(err, foundUser) {
    ...
    if (!foundUser) return res.error(new HttpErr(httpErr.notFound))
...
reqResetPassword = function(req, res, next) {
  ...
  users.findOne({email: req.query.email}, function(err, user) {
    ...
    if (!user) return res.error(new HttpErr(httpErr.notFound))
...
resetPassword = function(req, res, next) {
  return res.error(new HttpErr(httpErr.serverErrorNYI))
}
...
reqValidate = function(req, res, next) {
  ...
  if (!(req.user && req.user.role && req.user.role === 'admin')) {
    return res.error(new HttpErr(httpErr.badAuth))
  }
...
validate = function(req, res, next) {
  ...
  users.findOne({_id: req.query.user}, function(err, user) {
    ...
    if (users.hashValidationKey(user._id, user.email) !== req.query.key) {
      return res.error(new HttpErr(httpErr.badValue))
    }
