...
function get(req, res) {
  if (tipe.truthy(req.query.refresh)) {
    state.refresh('clientVersion', function(err) {
      if (err) return res.error(err)
      res.send({data: util.config.clientVersion})
    })
  }
  else res.send({data: util.config.clientVersion})
}
...
function post(req, res) {
  ...
  if (err) return res.error(err)
  state.set('clientVersion', req.body.data, function(err) {
    if (err) return res.error(err)
    res.send({data: util.config.clientVersion})
  })
}
