...
function post(req, res) {
  ...
  if (!(req.body && req.body.data)) return finish(proxErr.missingParam('body.data'))
  ...
  docs.safeUpsert({_id: staticVersion._id, data: req.body.data}, finish)
...
var read = exports.read = function(cb) {
  docs.findOne({ _id: staticVersion._id }, function(err, doc) {
    ...
    if (doc) {
      ...
      util.config.clientVersion = version
    }
    else {
      util.config.clientVersion = parse(staticVersion.data.version)
    }
    cb(null, doc)
  })
}
...
exports.validate = function(req, res, next) {
  var sver = util.config.clientVersion
