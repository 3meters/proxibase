...
function get(options, cb) {
  ...
  // focus yelp on very close places, meaning they are useless in
  ...
  util.callService.yelp(search, function(err, res, body) {
    ...
    venues.forEach(function(venue) {
      ...
      // Filter out places we don't want or can't dedupe
      if (!venue.phone) {
      ...
      if (venue.is_closed) {
        skipped.push({place: venue, reason: 'closed'})
        return
      }
      ...
      var cat = null
      if (venue.categories && venue.categories.length > 0) cat = venue.categories[0]
      place.category = {
        id: cat[1],
        name: cat[0],
      }
