...
var get = function(req, res) {
  ...
  //
  // TODO:  BUG: This function needs to be moved into the place trigger code
  ...
  function merge(places, cb) {
    ...
    util.db.places
      ...
      .toArray(function(err, entIds) {
        ...
        getEntities(req, {entityIds: entIds}, function(err, splaces) { // service places
          if (err) return cb(err)
          ...
          for (var s = splaces.length; s--;) {
            ...
            for (var p = places.length; p--;) {
              ...
              try {
                // TODO: test
                if (splace.provider[req.body.provider].id ===
                    place.provider[req.body.provider].id) {
                  providerMatch = true
                }
              } catch (e) {}
              if (providerMatch || splace.phone === place.phone) {
          ...
          for (var s = splaces.length; s--;) {
            if (mergeMap[s] >= 0) {
              ...
              _.extend(splaces[s].provider, places[mergeMap[s]].provider)

              // add the place's applinks to the splaces's applinks array
              splaces[s].applinks = splaces[s].applinks || []
              splaces[s].applinks = splaces[s].applinks.concat(places[mergeMap[s]].applinks)

              // overwrite the place with our splace
              places[mergeMap[s]] = splaces[s]
            }
