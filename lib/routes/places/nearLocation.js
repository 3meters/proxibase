...
var get = function(req, res) {
  ...
  function mergePlaceEntities(places, cb) {
    ...
    var query = {
      ...
      type: _place.type,  // com.aircandi.entity.place
    }
        ...
        getEntities(req, {entityIds: entIds}, function(err, entities) {
          ...
          for (var e = entities.length; e--;) {
            ...
            for (var p = places.length; p--;) {
              ...
              if ((JSON.stringify(ent.place.provider) === JSON.stringify(place.place.provider))
                  || (typeof ent.place.phone != 'undefined' && typeof place.place.phone != 'undefined' && ent.place.phone === place.place.phone)) {
  ...
  function decorate(places) {
    places.forEach(function(place) {
      ...
      if (place.place && place.place.category) {
        place.place.category.photo = { sourceName: 'assets.categories', prefix:'/img/categories/' + req.body.provider + '/' + place.place.category.id + '_88.png' }
      }
