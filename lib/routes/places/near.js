...
function get(req, res) {
  ...
  aircandi.get(req, function(err, places) {
    ...
    function merge(extPlaces) {
      var x, p, mergeMap = {}, logme
      // Mark duplicates
      for (p = places.length; p--;) {
        for (x = extPlaces.length; x--;) {
          if (extPlaces[x].name.match(/^Fremont Co/) && places[p].name.match(/^Fremont Co/)) logme = true
          else logme = false
          if (logme) {
            debug('place', places[p])
            debug('extPlace', extPlaces[x])
          }
          if (db.places.isDupe(places[p], extPlaces[x])) {
            if (logme) debug('flagged as dupe')
            mergeMap[x] = p
          }
        }
      }
