...
function get(req, res) {
  ...
  function run() {
    ...
    // TODO: test
    if (req.tries > 15) {
      return fail(perr.serverError('Recursive near search detected for request ' + req.tag, options))
    }
    ...
    getPlaces(false, function(err) {
      ...
      db.near.safeFind(nearQry, dbOps, function(err, nearDocs) {
        ...
        if (nearDocs && nearDocs.length && !options.refresh) {
          ...
          if (unfinished.length) { debug(unfinished); return recurse() }
          debug('Skipping external near query for req ' + req.tag, loc)
          return finish()
        }

        // set lock: mark the external call as started
          ...
          function readLock() {
            ...
            db.near.safeFind(nearQry, dbOps, function(err, docs) {
              ...
              if (docs && docs.length === 1) {
                ...
                // Log and remove the unused lock. This itself is vulnerable to a race.
  ...
  function recurse(interval) {
    ...
    req.tries++
    setTimeout(function() {
      debug('req ' + req.tag + ' recursing tries: ' + req.tries)
      run()
    }, interval)
  }
    ...
    db.places.safeFind(placesQuery, placesDbOps, function(err, places) {
      ...
      if (finished || ((places.length >= target) && !options.refresh)) {
        ...
        getEntities(req, entOps, function(err, places, more) {
          ...
          debug('sending response for ', req.tag)
          res.send({
            data: places,
