 ...
 *
 * This baby is complex.  It's goal is to return patches from our db, or if necessary, to query our
 * patch providers.  However, if several clients ask for patches in a new-to-us location at nearly the
 ...
 *
 *    cd prox/test
 *    node test -m <multiple clinets> -i <interval between starting clients> -t basic/49_patches_concurrent.js
 *
...
function get(req, res) {
  ...
  options.excludeCount = (options.excludePatchIds)
    ? options.excludePatchIds.length
    : 0

  options.target = options.limit + options.excludeCount

  // We need to track whether radius was passed or is the default.
  // This was the least invasive way to favor radius over limit.
  var radiusProvided = true
  if (!options.radius) {
    radiusProvided = false
    options.radius = 250
  }

  // Need to prevent auto patch generation because old client versions
  // don't use 'none' and the client update force has a bug.
  options.provider = 'none'

  if (options.provider === 'none')
    run(options, { finished: true, radius: options.radius })
  else
    run(options)
  ...
  function run(options, nearDoc) {
    ...
    function checkNearDoc() {
      if (nearDoc.finished) return getPatches(options, nearDoc)
      else return recurse(1000, options, nearDoc)
    }

    // Near is a collection containing a record of our queries to our
    // external patch providers.  Unless the refresh flag is set, we
    ...
    function getNearDoc(options, cb) {
      ...
      var scope = !radiusProvided ? {cReturned: {$gte: options.target}} : {radius: options.radius}
      ...
      db.near.safeFind(nearQry, nearOps, function(err, nearDocs) {
        ...
        if (nearDocs && nearDocs.length) {
          ...
          if (unfinished.length) return cb(null, unfinished[0])    // pick one to wait for
            ...
            // We have enough patches in the db to satisfy the request
  ...
  // Get patches from our database.  We may have used a bigger radius
  // to fullfill the limit than originally asked for.
  function getPatches(options, nearDoc) {

    var patchesQuery = makeNearQuery(options.location, nearDoc.radius)

    var patchesOps = util.clone(dbOps)
    patchesOps.fields = {_id: 1, location: 1}
    // patchesOps.refs = '_id,name,schema'

    db.patches.safeFind(patchesQuery, patchesOps, function(err, patches) {
      if (err) return fail(err)

      if (options.log) log('patches query radius ' + nearDoc.radius, {
        query: patchesQuery,
        options: patchesOps,
      ...
      if (options.sort === 'distance') {
        patches = sortPatches(patches, patches)
      }
      ...
      for (var i = 0, len = patches.length; i < len; i++) {
        if (!exclude(patches[i])) {
          myPatches.push(patches[i])
          patchIds.push(patches[i]._id)
        }
      }

      if (patchIds.length > options.limit) patchIds = patchIds.slice(0, options.limit)
      ...
      getEntities(req, entOps, function(err, entPatches, more) {
        ...
        // On by default. To turn off set sort to any other value.  Patches with a
        ...
        if (options.sort === 'distance') {
          ...
          sortedPatches.forEach(function(patch) {
            if (patch.location && (patch.location.accuracy < 100)) entPatches.push(patch)
            else fuzzyPatches.push(patch)
          })
          fuzzyPatches.forEach(function(patch) {
            entPatches.push(patch)
          })
        }

        if (options.log) logPatches(entPatches)
      ...
      // Move patches with fuzzy accuracy (aka yelp) to the end of the line,
      // otherwise preserving the order returned by the mongo near query
      function sortPatches(patches) {
        var sorted = []
        var fuzzy = []
        patches.forEach(function(patch) {
          if (patch.location && (patch.location.accuracy < 100)) sorted.push(patch)
          else fuzzy.push(patch)
        })
        fuzzy.forEach(function(patch) {
          sorted.push(patch)
        })
        return sorted
      }
  ...
  // Call each patch provider in series to prevent cross-provider races
  function callExternalProviders(options, nearDoc, cb) {
    ...
    function callProvider(key, nextProvider) {
      ...
      providers[key].get(options, function(err, externalPatches, extSkipped) {

        if (err) { logErr(err); return nextProvider() }

        if (options.log) {
          log('\n' + key + ' results: ', externalPatches.length)
          externalPatches.forEach(function(patch) {
            var msg = patch.name
            if (patch.phone) msg += ' ' + patch.phone
            msg += ' photo: '
            msg += patch.photo ? '1' : '0'
            log(msg)
          })
          log('\n' + key + ' skipped: ', extSkipped.length)
          extSkipped.forEach(function(elm) {
            log(elm.patch.name + ': ' + elm.reason)
          })
        }

        async.eachSeries(externalPatches, saveExternalPatch, nextProvider)

        function saveExternalPatch(patch, nextPatch) {
          db.patches.safeUpsert(patch, dbOps, function(err) {
            if (err) return fail(err)
            nextPatch()
          })
        }
    ...
    function finishCallAll(err) {
      ...
      var patchesQuery = makeNearQuery(options.location, options.radius)
      var countOps = util.clone(dbOps)
      countOps.count = true

      db.patches.safeFind(patchesQuery, countOps, function(err, count) {
        if (err) return fail(err)
        ...
        if (!radiusProvided && count < options.target) {
  ...
  function logPatches(patches) {
    patches.forEach(function(patch) {
      var google = patch.provider.google || ''
      var yelp = patch.provider.yelp || ''
      var foursquare = patch.provider.foursquare || ''
      log(patch.name + ' ' + patch.phone + ' yelp:' + yelp +
  ...
  function exclude(patch) {
    if (!options.excludePatchIds) return false
    if (options.excludePatchIds.indexOf(patch._id) >= 0) return true
    return false
  }
