...
var get = function(req, res) {
    ...
    util.db.places
      ...
      .toArray(function(err, entIds) {
        ...
        getEntities(req, { entityIds: entIds, links: req.body.links }, function(err, splaces) { // service places
          ...
          // log('debug places near splaces', splaces)
          ...
          for (var s = splaces.length; s--;) {
            ...
            for (var p = places.length; p--;) {
              ...
              if (!Object.keys(splace.provider).some(function(provName) {
                    return (provName === req.body.provider)
                  })) {
  ...
  function decorate(places) {
    places.forEach(function(place) {
      ...
      // TODO: This code should go away when we map all category
      // IDs to candiCateogry Ids
      if (place.category) {
