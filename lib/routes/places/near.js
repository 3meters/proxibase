...
function get(req, res) {
  ...
  if (req.tries > 10) {
    err = perr.serverError('Recursive near search detected for request ' + req.tag, options)
    logErr(err)
  ...
  getPlaces(false, function(err) {
    ...
    db.near.safeFind(nearQry, dbOps, function(err, nearDocs) {
      ...
      if (nearDocs && nearDocs.length && !options.refresh) {
        ...
        debug('Skipping nearby query for location req.tag ' + req.tag, loc)
        return finish()
      }
  ...
  // Wait an interval and then call again
