...
function get(req, res) {
  ...
  aircandi.get(req, function(err, places) {
    ...
    debug('places retrived from aircandi: ' + places.length)
    ...
    function getExternal(provider, nextProvider) {
      provider(req, function(err, externalPlaces, extRawData) {
        ...
        if (req.body.includeRaw) raw.push(extRawData)  // for tests
    ...
    function finish(err) {
      ...
      function savePlace(place, next) {
        if (!place.save) return next(null, place)
        delete place.save
        db.places.safeUpsert(place, {asAdmin: true}, next)
      }
    }
