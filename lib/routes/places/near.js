...
function get(req, res) {
  ...
  var placesQuery = {
    'location.geometry': {
      ...
      // $maxDistance: options.radius / 111120,  // Commenting out as trial fix for issue 218
    }
  }
  ...
  function getPlaces(finished, cb) {
    ...
    db.places.safeFind(placesQuery, placesDbOps, function(err, places) {
      ...
      if (finished || (places.length >= target && !options.waitForContent)) {
        ...
        getEntities(req, req.body, function(err, unsortedPlaces, more) {
          ...
          unsortedPlaces.forEach(function(place) {
            places[placeIds.indexOf(place._id)] = place
          })
          res.send({
            data: places,
