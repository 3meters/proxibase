...
function get(req, res) {
  ...
  // Need to prevent auto place generation because old client versions
  ...
  function callExternalProviders(options, nearDoc, cb) {
    ...
    function callProvider(key, nextProvider) {
      ...
      providers[key].get(options, function(err, externalPlaces, extSkipped) {

        if (err) { logErr(err); return nextProvider() }

        if (options.log) {
          log('\n' + key + ' results: ', externalPlaces.length)
          externalPlaces.forEach(function(place) {
            var msg = place.name
            if (place.phone) msg += ' ' + place.phone
            msg += ' photo: '
            msg += place.photo ? '1' : '0'
            log(msg)
          })
