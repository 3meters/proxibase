...
function get(req, cb) {
  ...
  var qLimit = Math.min(200, Math.floor(req.body.limit * 1.3))

  var search = {
    path: 'radarsearch/json',
    query: {
  ...
  debug('google near search', search)
  ...
  util.callService.google(search, function(err, res) {
    ...
    var results = []
    var all = res.body.results
    for (var i = 0, c = 0; i < all.length && c < req.body.limit; i++) {
      if (req.exclude(all[i].id)) continue
      results.push(all[i])
      c++
    }
    ...
    async.eachLimit(results, 10, makePlace, finish)
