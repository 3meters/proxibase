  ...
  util.callService.google(search, function(err, res) {
    ...
    function makePlace(result, cb) {
      ...
      util.callService.google({
        ...
        if (venue.vicinity) {
          entity.place.location.address = venue.vicinity.replace(/,[^,]+$/, '')
        }
        ...
        if (venue.address_components) {
          venue.address_components.forEach(function(elm) {
            ...
            switch (elm.types[0]) {
              case 'locality':
                entity.place.location.city = elm.short_name
                break
              case 'administrative_area_level_1':
                entity.place.location.state = elm.short_name
                break
              case 'country':
                entity.place.location.cc = elm.short_name
                break
              case 'postal_code':
                entity.place.location.postalCode = elm.short_name
                break
            }
          })
        }
