...
var optionsSpec = {
  type: 'object', value: {
    rest:           {type: 'boolean', default: true},  // use the rest apis rather than getEntities to return results
                                        // if set use the rest linked param, rather than the getEntities
                                        // links param
    getEntities:    {type: 'boolean'},  // Inverse of the above.  If true, getEntities, if false, use rest
...
function near(req, res) {
  ...
  if (!options.getEntities) {
    patchOps = _.extend(patchOps, _.pick(options, ['limit', 'fields', 'refs', 'linked', 'links']))
  }
  else {
  ...
  db.patches.safeFind(query, patchOps, function(err, patches) {
    ...
    // Android client should set options.getEntities to true
    // Once all android clients in the wild are sending getEntities = true
    // We will remove the isDefined check below, and the default for the client
    // with no flags set will be rest, not getEntities
    if (options.rest || (tipe.isDefined(options.getEntities && !options.getEntities))) {
      return res.send({data: patches})
    }
