...
function near(req, res) {
  ...
  var patchOps = util.clone(req.dbOps)
...
function interesting(req, res) {
  ...
  stats.get(statOps, function(err, stats, meta) {
    if (err) return fail(err)

    // debug('interesting statOps', statOps)
    // debug('interesting stats', stats)
    // debug('interesting meta', meta)

    var patchIds = stats.map(function(stat) { return stat._id })

    db.patches.safeFind({_id: {$in: patchIds}}, options, function(err, patches) {
      if (err) return fail(err)
