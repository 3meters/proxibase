...
function near(req, res) {
  ...
  if (options.getEntities) patchOps.fields = { _id: 1, location: 1 }
    ...
    patchOps = _.extend(patchOps, _.pick(options, ['limit', 'fields', 'refs', 'linked', 'links', 'linkCount']))
  }
  ...
  db.patches.safeFind(query, patchOps, function(err, patches) {
    ...
    // If not options.getEntities we are done
  ...
  stats.get(statOps, function(err, stats) {
    ...
    db.patches.safeFind({_id: {$in: patchIds}}, options, function(err, patches, meta) {
      ...
      patches.forEach(function(patch) {
        patchMap[patch._id] = patch  // map patches by _id
      })

      var results = []
      stats.forEach(function(stat) {
        if (patchMap[stat._id]) results.push(patchMap[stat._id])
      })
