...
function getCrosswalk(applink, scope, cb) {
  ...
  util.callService.factual(search, function(err, res, body) {
    ...
    if (scope.raw) {
      scope.raw.factual = scope.raw.factual || {}
      scope.raw.factual[applink.appId] = results
    }

    // Detect spurious Factual patch merge by the presense of two or
    // more foursquare Ids that foursquare doesn't resolve the same patch
    var foursquareIds = []
    var validatedFoursquareIdMap = {}
    ...
    else async.each(foursquareIds, callFoursquare, finish)
    ...
    function finish(err) {
      ...
      if (validatedFoursquareIds.length === 1) {
        // Foursquare agrees this patch exists and only has one non-screwed up
        // entry in factual's database.  Set the factual provider id in the patch,
        // saving one lookup next time the patch is refreshed
        if (scope.patch && scope.patch.provider && !scope.patch.provider.factual) {
          scope.patch.provider.factual = applink.appId
      ...
      if (validatedFoursquareIds.length <= 1) return addCrosswalkLinks()
        ...
        cb(perr.partnerError('Factual possible mis-merge', corpse))
      }
    }
