  ...
  util.callService.factual(search, function(err, res, body) {
    ...
    function finish(err) {
      ...
      if (validatedFoursquareIds.length === 1) {
        // Foursquare agrees this place exists and only has one non-screwed up
        // entry in factual's database.  Set the factual provider id in the place,
        // saving one lookup next time the place is refreshed
        if (scope.place && scope.place.provider && !scope.place.provider.factual) {
          scope.place.provider.factual = applink.appId
