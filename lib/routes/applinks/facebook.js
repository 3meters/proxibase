...
function normalize(applink) {
  ...
  if (!applink) return
  if (applink.id) return // done
  if (!type.isString(applink.url)) return applink = null  // nothing to work with
  ...
  if (id) applink.id = id
    ...
    logErr('Could not find id in facebook url: ' + applink.url)
    applink = null
  }
}
...
function get(applink, scope, cb) {
  ...
  util.callService.facebook(query, function(err, res) {
    ...
    if (!body.id) {
      ...
      return cb(perr.badApplink('Invalid facebook Id ' + applink.id, body))
    }
    applink.id = body.id
    applink.name = body.name
    applink.photo = {
      prefix: 'https://graph.facebook.com/' + applink.id +
        '/picture?type=large',
      source: 'facebook'
    }
