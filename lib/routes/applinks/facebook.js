...
function get(applink, scope, cb) {
  ...
  var query = {
    path: '/' + applink.appId,
    query: {fields: 'name,likes,website'},
    log: scope.log,
  ...
  util.callService.facebook(query, function(err, res) {
    ...
    if (!place.id) {
      /*
       * Turning this feature off -- too many bogus fb ids were slipping through
      if (place.error && place.error.code === 100) {
      ...
      */
      ...
      else return cb(perr.badApplink('Invalid facebook Id ' + applink.appId, place))
    }
...
function find(applink, scope, cb) {
  ...
  search2.query.q = searchName.split(' ')[0]
  // async.each([search, search2], callFacebook, cb)
  async.each([search], callFacebook, cb)


  function callFacebook(search, next) {
    debug('calling fb with q: ', search)

    util.callService.facebook(search, function(err, res, body) {
      debug('fb returned body', body)
      ...
      debug('facebook places found with q ' + search.query.q, count)
      next()
    })
  }
}
