...
function normalize(applink) {
  ...
  if (applink.appId) return  // done
  if (!type.isString(applink.appUrl)) return  // nothing to work with

  var u = url.parse(applink.appUrl)
  if (!u.pathname) return perr.badApplink('Could not parse facebook url')
  ...
  else logErr('Could not find id in facebook url: ' + applink.appUrl)
}
...
function get(applink, scope, cb) {
  if (applink.query) {
    return getFacebookPlaces(applink, scope, cb)
  }
...
function getFacebookPlaces(applink, scope, cb) {
    ...
    places.forEach(function(place) {
      ...
      if (places.length === 1 || place.likes > minLikes) {
        scope.applinkQ.push({
          ...
          appId: place.id,
          name: place.name,
          photo: {prefix: 'https://graph.facebook.com/' + place.id + '/picture?type=large', applinkName: 'facebook'},
