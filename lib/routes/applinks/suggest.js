...
function main(req, res) {
  ...
  var err = chek(req.body, _body)
  if (err) return res.error(err)
  req.body.tag = req.tag
  req.body.user = req.user || util.adminUser

  // unpack applinks from the place object
  req.body.applinks = req.body.place.applinks
  delete req.body.place.applinks
    ...
    if (!sent) {
      ...
      for (var type in applinkMap) {
        for (var id in applinkMap[type]) {
          applinks.push(applinkMap[type][id])
        }
      }
      ...
      done(null, {place: place, raw: scope.raw})
    }
  }
}
