...
var _body = {
  entity: {type: 'object', default: {}, value: {
    name: {type: 'string'},
  ...
  applinkEnts: {type: 'array', default: [], value: {
    type: 'object', value: {
      type: { type: 'string', default: statics.typeApplink, value: statics.typeApplink },
      applink: { type: 'object', default: {}, value: {
        type: { type: 'string' },
        id: { type: 'string' },
        url: { type: 'string' },
        data: { type: 'object' },
...
function run(ops, done) {
  ...
  var applinkEnts = scope.applinkEnts
  ...
  applinkEnts.forEach(function(ent) {
    // TODO:  the applink check is necessary because check is not setting the default array
    if (ent.applink && ent.applink.type) startingApplinkMap[ent.applink.type] = true
  })
