...
var _body = {
  ...
  applinks: {type: 'array', value: {
    type: 'object', value: {
      type: { type: 'string', value: statics.typeApplink },
    },
  }},
  user: {type: 'object'},
...
function run(ops, done) {
  ...
  applinks.forEach(function(applink) {
    // Trim the type prefix com.aircandi.applink from the type
    applink.type = aplink.type.slice(statics.typeApplink.length)
  ...
  function decorate(applinks) {
    applinks.forEach(function(applink) {
      var _applink = _applinks[applink.type]  // bug won't work
  ...
  function finish(err) {
    ...
    if (!sent) {
      ...
      decorate(applinks)
      sent = true
      done(null, {entity: ent, applinks: applinks, raw: scope.raw})
    }
  }
}
