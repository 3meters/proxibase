...
function get(applink, scope, cb) {
  ...
  // TEMP for diagnosing 164 in producion
  scope.log = true
  ...
  if (!tipe.isFunction(cb)) {
    cb = function(err, results) {
      if (scope.log) log('Thumbnail returned without callback for ' + applink.appId, (results) ? results : '')
  ...
  function callThumbnailService(applink, ops, cb) {
    ...
    function finish() {
      ...
      fs.stat(filePath, function(err, stats) {
        ...
        if (!stats.size) return cb(perr.partnerError('Thumbnail is an empty file', corpse))
