...
var AWS = require('aws-sdk')  // currently not used, but we should shift
                              // to it over time as it matures
...
// has been created.
function getFileName(ent) {
  var urlObj = nodeUrl.parse(ent.applink.id)
...
// Main worker
function get(ent, user, res, cb) {

  // Currently fire and forget from the caller
  cb = cb || function(err, ent) {
    if (err) return logErr(err)
    if (ent) log('Thumbnail completed for applink: ', ent)
  }

  if (ent.applink.data.skipThumbnail) return cb()

  if (util.config.service.mode === 'test'
      && !util.config.testThumbnails)  return cb()

  var fileName = getFileName(ent)
  ...
  if (useU2p) {
    ...
    var u2pURL = url2png.buildURL(ent.applink.id, {
      protocol: 'http',
      viewport: '1280x1280',
  ...
  function uploadToS3(filePath, FileName) {
    ...
    s3.putFile(filePath, fileName, headers, function(err, s3Res) {
      ...
      if (200 === s3Res.statusCode) return cb(null, ent)
      else {
