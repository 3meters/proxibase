...
function get(applink, scope, cb) {
  ...
  if (!tipe.isFunction(cb)) {
    cb = function(err, results) {
      ...
      else log('Thumbnail generation finished for applink ' + applink.appId, results)
    }
  }

  if (util.config.service.mode === 'test'
      && !scope.testThumbnails) {
    return cb(null, 'Test mode: skipped thumbnail generation')
  }
  ...
  var u2pURL = url2png.buildURL((applink.appUrl ? applink.appUrl : applink.appId), {
    protocol: 'http',
    viewport: '1280x1280',
    thumbnail_max_width: 200,
  })
  var ops = {uri: u2pURL, log: true, provider: 'u2p'}
  ...
  function callThumbnailService(ops) {
    ...
    req.end(function(err, res) {
      ...
      ws.on('close', function() { uploadToS3(filePath, fileName, ops) })
    })
  }

  function uploadToS3(filePath, fileName, ops) {
    ...
    s3.putFile(filePath, fileName, headers, function(err, s3Res) {
      ...
      if (200 === s3Res.statusCode) return cb(null, s3Res)
      else {
