...
exports.addRoutes = function (app) {
  ...
  app.get('/applinks/place/:placeId', callGet)
  app.post('/applinks/place/:placeId', callGet)
}
...
function callGet(req, res) {
  ...
  if (req.params.placeId) req.body.placeId = req.params.placeId
  get(req.body, function(err, results) {
    if (err) return res.error(err)
    var body = {data: results.applinks}
    if (results.place) body.place = results.place
...
function get(ops, cb) {
  ...
  // Setting save and waitForContent to true for place lookup
  if (ops.placeId) {
...
function savePlace(ops, cb) {
  ...
  var dbOps = util.clone(ops.dbOps)
  dbOps.asAdmin = true
  ops.place.applinkDate = util.now()
