    ...
    for (var type in ops.applinkMap) {
      ...
      for (var appId in ops.applinkMap[type]) {
        ...
        links.sort(function(a, b) {
          var aValid = a.validationDate || 0
          var bValid = b.validationDate || 0
          return aValid - bValid
        })
      }

      var haveValidatedApplink = false
      links.forEach(function(link) {
        if (link.validatedDate) {
          haveValidatedApplink = true
          ops.applinks.push(link)
        }
        else {
          // We only add non-validated links if we don't 
          // have any validated ones
          if (!haveValidatedApplink) ops.applinks.push(link)
        }
      })
    }
