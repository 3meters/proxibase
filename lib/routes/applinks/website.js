...
function normalize(applink) {
  ...
  id = url.format(urlObj)
  id = id.replace(/\/$/, '')   // trim trailing slash
  if (!/^http/.test(id)) id = 'http://' + id
  applink.appId = id
  delete applink.appUrl
  return applink
}
...
function get(applink, scope, cb) {
  ...
  function processWebPage(err, res, body) {
    ...
    if (_.isEmpty(body)) {
      ...
      else return cb(perr.badApplink(applink.appId, res.headers), null)
    }
