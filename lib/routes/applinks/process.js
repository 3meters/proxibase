...
module.exports = function(applink, scope, cb) {
  ...
  var originalApplink = _.cloneDeep(applink)
  ...
  function addApplink() {
    ...
    if (driver && driver.dedupe) {
      driver.dedupe(applink, applinkMap)
    }

    // dedupe by appId
    if (applinkMap[applink.type][applink.appId]) return  // dupe by id

    // dedupe by appUrl
    var dupeUrl = false
    for (var appId in applinkMap[applink.type]) {
      if (applink.appUrl
          && (applink.appUrl === applinkMap[applink.type][appId].appUrl)) {
        dupeUrl = true
        break
      }
    }
    ...
    // We have a winner
    if (!dupeUrl) applinkMap[applink.type][applink.appId] = applink
  }
