...
// data router
exports.service = function(scope) {
  app = scope
  app.get('/data', welcome)
  app.all('*', parse) // lops /data/collectionName from the url
  app.get('*', find)
  app.post('/', insert)
  app.post('/*', update)
  app.delete('/*', remove)
...
function welcome(req, res) {
  res.send({data: app.info.data})
}


// Basic scrub and parse for all data requests
function parse(req, res, next) {

  // Shift off the leading /data
  req.paths.shift()

  // If the next path element is not a mongo collection name return not found
  if (!util.db.cNames[req.paths[0]]) {
    return res.error(httpErr.notFound)
  }

  // Set the request collection and model
  req.cName = req.paths[0]
  req.model = gdb.models[req.cName]

  // Shift off the collection name and recreate the url
  req.paths.shift()

  // For backward compat
  if (req.paths.length && req.paths[0].indexOf('ids:') === 0) {
    req.paths[0] = req.paths[0].slice(4)
  }

  // TODO: replace this with a better idea, possibly /data/collection/:fieldName/value
  if (req.paths.length && req.paths[0].indexOf('names:') === 0) {
    req.query.names = req.paths[0].slice(6).split(',')
    req.paths.shift()
  }

  // Next path is either an id or an array of ids
  if (req.paths.length) req.query.ids = req.paths[0].split(',')

  // Reset the url to the unparsed parts
  req.url = '/' + req.paths.join('/')

  // Make sure user is logged in for writes
  switch (req.method) {
    case 'get':
      delete req.body
      return next()
...
var find = exports.find = function (req, res) {
  ...
  // Minimal group with only one agregate, count
  if (req.query.countBy) {
    if (!req.model.schema.paths[req.query.countBy]) {
      return res.error(new HttpErr(httpErr.badParam, ['countBy', 'must be a key in the collection']))
    }
    var reduceFn = 'function (doc, prev) { prev.count++ }'
    return db.collection(req.cName)
      .group([req.query.countBy], selector, {'count': 0}, reduceFn, true, sendResults)
  }

  // Execute regular find
  return db.collection(req.cName)
    .find(selector, options)
    .toArray(sendResults)
...
var find2 = function(req, res)  {
  ...
  if (req.query.names) {
    ...
    for (var i = req.query.names.length; i--;) {
      req.query.names[i] = req.query.names[i].toLowerCase()
    }
    query.where('namelc').in(req.query.names)
  }
  if (req.query.fields) {
...
// delete /data/collection/ids:id1,id2,
