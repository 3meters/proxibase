...
function signin(req, res, next) {
  ...
  users.authByPassword(credentials, function(err, user) {
    ...
    if (req.body.newUser) {
      _.extend(user, req.body.newUser) // for testing
    }
    ...
    users.update(
      ...
      function(err) {
        if (err) return next(err)
        upsertSession(user, req.body.installationId, null, function(err, session) {
          if (err) return next(err)
          return res.send({user: user, session: session})
        })
      }
    )
  })
