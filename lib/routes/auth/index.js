...
function addRoutes(app) {
  ...
  app.get('/auth/:method/:service', callOauth)
...
function welcome(req, res) {
  res.send({
    info: {
      ...
      comment:  'All params required. InstallId is included in client local storage.',
    }
  })
}


// Patch the current version back on the url so that authom redirects will work
function callOauth(req, res) {
  req.url = util.statics.currentApiVersionPrefix + req.url
  oauth(req, res)
}
