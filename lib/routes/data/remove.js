...
module.exports = function remove(req, res) {
  ...
  function removeDoc(id, next) {
    req.collection.safeRemove({_id: id}, {user: req.user}, function(err, meta) {
      debug('routes/data/removeDoc err', err)
      debug('routes/data/removeDoc meta', meta)
  ...
  function finish(err, meta) {
    debug('routes/data/remove.finish meta2', meta)
    if (err) return res.error(err)
    if (meta) rCount += meta.count // called as admin with multi-delete
