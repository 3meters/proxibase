...
module.exports = function(req, arg1) {  // arg1 can be cb or res
  ...
  err = util.check(req.query, _query, {strict: true, allowEmpty: true})
  if (err) return cb(err)

  selector = req.query.find || {}
  if (req.query.ids) {
    // Caller asked to pregenerate an id
    if (req.query.ids[0] === 'genId') {
      return sendResults(null, {_id: util.genId(req.collection.schema.id)})
    }
    selector._id = {$in: req.query.ids}
  }
  ...
  function sendResults(err, docs) {
    ...
    if (util.truthy(req.query.datesToUTC)) {
      docs.forEach(function(doc) {
