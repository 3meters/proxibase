...
function send(err, docs, meta, res) {
  ...
  if (tipe.isObject(docs) && meta.promote) {
    if (meta.promote === 'links') {
      if (docs.moreLinks) {
        meta.more = docs.moreLinks
        meta.moreLinksQueries = docs.moreLinksQueries
      }
    }
    if (meta.promote === 'linked') {
      if (docs.moreLinked) {
        meta.more = docs.moreLinked
        meta.moreLinkedQueries = docs.moreLinkedQueries
      }
    }
    docs = docs[meta.promote]
    if (tipe.isArray(docs)) meta.count = docs.length
  }
