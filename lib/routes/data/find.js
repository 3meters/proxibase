...
function find(req, res) {
  ...
  if (req.query.id) {
    req.collection.safeFindOne(req.query, {user: req.user}, function(err, result) {
      res.send(err, {data: result, count: (result) ? 1 : 0})
    })
  }
  else {
    req.query.sort = req.query.sort || [{modifiedDate: -1}]
    req.collection.safeFind(req.query, {user: req.user}, function(err, result) {
      res.send(err, result)
    })
  }
}
...
find.countBy = function(req, res) {
  var err = data.scrubReq(req)
  if (err) return res.error(err)
  req.query.countBy = req.params.fieldNames
  req.collection.safeFind(req.query, function(err, result) {
    res.send(err, result)
  })
}
