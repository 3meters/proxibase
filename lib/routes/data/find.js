...
function find(req, res) {
  ...
  if (req.findOne) {
    req.collection.safeFindOne(req.selector, req.dbOps, function(err, result) {
      res.send(err, {data: result, count: (result) ? 1 : 0})
    })
  }
  else {
    req.dbOps.sort = req.dbOps.sort || [{modifiedDate: -1}]
    req.collection.safeFind(req.selector, req.dbOps, function(err, result) {
      res.send(err, result)
    })
  }
}
...
find.genId = function(req, res) {
  ...
  req.collection.safeFind(req.query, req.dbOps, function(err, result) {
    res.send(err, result)
  })
}
...
find.count = function(req, res) {
  ...
  debug('selector', req.selector)
  debug('dbOps', req.dbOps)
  req.collection.safeFind(req.selector, req.dbOps, function(err, result) {
    res.send(err, result)
  })
}
