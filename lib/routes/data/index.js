...
exports.addRoutes = function (app) {
  ...
  app.all('/:cmd(data|find)/:collection*', checkSystem)
  app.all('/:cmd(data|find)/:collection*', checkOwnerAccess)
  app.get('/data/:collection/next', find.next)
  app.get('/data/:collection/count', find.count)
  app.get('/data/:collection/count/:fieldNames', find.countBy)
...
// For ownerAccess schemas restrict all queries to documents owned by the user
function checkOwnerAccess(req, res, next) {
  if (req.schema.ownerAccess) {
    if (!(req.user && req.user.role)) return res.error(perr.badAuth())
    if ('admin' !== req.user.role) {
      req.query.query = req.query.query || {}
      req.query.query._owner = req.user._id
    }
  }
  next()
}
