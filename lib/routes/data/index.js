...
exports.addRoutes = function(app) {
  ...
  app.delete('/data/:collection/:id', checkRemove)
  app.all('/data|find)/:collection/:id?*', scrubReq)
  app.all('/(data|find)/:collection/:id?*', scrubReq)
  app.get('/(data|find)/:collection/genId', find.genId)
  app.get('/(data|find)/:collection/next', find.next)
  app.get('/(data|find)/:collection/count', find.count)
  app.get('/(data|find)/:collection/count/:countBy', find.countBy)
...
function welcome(req, res) {
  ...
  for (var clName in statics.collections) {
    greeting.data[clName] = config.service.url + '/v1/data/' + clName
  }
...
function findSpec(req, res) {
  res.send({findSpec: db.safeFindSpec()})
}
      ...
      switch (tipe(req.query.fields)) {
        ...
        case 'object':
          for (var field in req.query.fields) {
            req.dbOps.fields[field] = tipe.truthy(req.query.fields[field]) // strings to boolean
          }
          break
      }
    }
