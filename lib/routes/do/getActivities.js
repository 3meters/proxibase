...
var run = exports.run = function(req, options, cb) {
  ...
  /* Use logged in user if entityId is not passed */
  if (!tipe.isDefined(options.entityId)) {
  ...
  if (!options.cursor.linkTypes && !options.cursor.schemas) direct = true

  if (!direct)
    findActivityLinks()
  else
    getActivityActionsDirect()
  ...
  function getActivityActionsDirect() {
    log('getActivityActionsDirect')
    var query = { _user: options.entityId, event: { $in: options.events }}
  ...
  function addActionEntities() {
    ...
    function processCollection(collectionName, next) {
      ...
      var query = { _id:{ $in: collectionIds }}
