...
var run = exports.run = function(req, options, cb) {
  ...
  function getActivityActions() {
    ...
    var query = {
      $or: [
        { _target: { $in: activityIds }},
        { _toEntity: { $in: activityIds }},
      ]
    }
    if (options.cursor.where) {
      query = { $and: [query, { _user: { $ne: options.entityId }}, options.cursor.where] }
    }
    ...
    var pipe = [
      ...
      { $group: {
        ...
        actions: { $push: { type: "$type", _user: "$_user", _toEntity: "$_toEntity", _fromEntity: "$_fromEntity", modifiedDate: "$modifiedDate" }},
        count: { $sum: 1 }}},
  ...
  function gatherEntityIds() {
    ...
    actions.forEach(function(action) {
      if (action._target) {
        entityMap[action._target] = action._target
      }
      if (action._toEntity) {
  ...
  function assembleActivities() {
    ...
    actions.forEach(function(action) {
      ...
      if (action._target) {
        var actionEntity = actionEntities[action._target]
      ...
      if (action._toEntity) {
        var actionEntity = actionEntities[action._toEntity]
      ...
      if (action.grouped) {
        ...
        else if (activity.action.event.indexOf('expand') !== -1) {
          activity.grouped = true
          activity.summary = 'More places...'
        }
      }
      /*
       * Filter out broken activities just in case. Usual cause is that the action
       * wasn't cleaned up when an associated entity was deleted.
       */
      if ((activity.action.entity && activity.action.toEntity)
        || (activity.action.event.indexOf('place') !== -1 && activity.action.entity)) {
  ...
  function activityTrigger(action) {
    ...
    activityLinks.forEach(function(link) {
      ...
      if (action._target === link._to && link.type === statics.typeWatch) watch = 'watch'
      if (action._toEntity === link._to && link.type === statics.typeCreate) create = 'own_to'
      if (action._target === link._to && link.type === statics.typeCreate) create = 'own'
    })
