...
<<<<<<< HEAD
          ...
          debug('starDate now biatch', startDate)
          var randomDate = random(startDate, endDate)
          var span = endDate - startDate
          debug('dateSpan', span)
          var rdf = (randomDate - startDate) / span
          debug('rd factor of dateSpan', rdf)
          var query = {
            type: 'proximity',
          ...
          debug('move cg query', query)
          db.links.find(query, {
            _from: 1,
          }).limit(20).sort({
            modifiedDate: 1
          }).toArray(function(err, docs) {
            debug('found docs cb args', arguments)
            if (err) return next(err)
            if (docs && docs.length > 0) {
...
<<<<<<< HEAD
          ...
          */
          ...
          var ops = {
            fields: {_id: 1},
            limit: statics.limitMax
          }
          db.links.safeFind(qry, ops, function (err, links) {
            if (err) return next(err)
            async.eachSeries(links.data, inactivateLink, finishInactivateLinks)

            function inactivateLink(link, nextLink) {
              db.links.safeUpdate({_id: link._id, inactive: true}, dbuser, nextLink)
            }

            function finishInactivateLinks(err) {
              if (err) return next(err)
              /*
               * Check to see if we've been to this place before and there
               * is an inactive link we should re-activate.
