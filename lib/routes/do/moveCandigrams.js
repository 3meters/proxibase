...
var run = exports.run =  function(params, cb) {
  ...
  var startDate
  var endDate
  var user = (params.user ? params.user : util.adminUser)

  log('moveCandigrams executing: ' + util.nowFormatted())
  ...
  function process() {
    ...
    function candigram(entityId, next) {
      var options = { placeAttempts: 1, stop: false }
      ...
      db.candigrams.findOne({ _id: entityId }, function(err, doc) {
        ...
        if (options.candigram.hopsMax != -1 && options.candigram.hopCount >= options.candigram.hopsMax) {
          ...
          db.links.findOne(query, function(err, doc) {
            if (err) return next(err)
            if (!doc) {
              util.logErr('error: expected active link but none found for: ' + options.candigram._id)
              return next()
            }
        ...
        { safe: true, multi: true }, function(err) {
          ...
          if (options.stop) {
            ...
            db.links.findOne({ _to: options.nextPlaceId, _from: options.candigram._id, type: util.statics.typeCandigram }, function(err, doc) {
              ...
              if (doc) {
                doc.inactive = false
                db.links.safeUpdate(doc, { asAdmin: true, user: user }, function(err, updatedLink) {
                  if (err) return next(err)
                  log('link reactivated successfully from: ' + options.candigram._id + ' to: ' + options.nextPlaceId)
                ...
                db.links.safeInsert(link, { asAdmin: true, user: user }, function(err, savedDoc) {
                  if (err) return next(err)
                  log('link created successfully from: ' + options.candigram._id + ' to: ' + options.nextPlaceId)
    ...
    function updateHops(options, next) {
       ...
       */
      db.candigrams.safeUpdate(options.candigram, { asAdmin: true, user: user }, function(err, updatedDoc) {
        if (err) return next(err)
        if (options.stop) {
          next()
        }
