...
var run = exports.run =  function(req, body, cb) {
  ...
  function process() {
              ...
              if (doc) {
                ...
                db.links.safeInsert(link, { user: req.user }, function(err, savedDoc) {
                  ...
                  if (!body.skipActivityDate) {
                    methods.propogateActivityDate(options.candigram._id, activityDate) // Fire and forget
                  }
                  if (req.body.skipNotifications) return next()
                  updateHop(options, next)
                })
    ...
    function updateHop(options, next) {
      /*
       * Housekeeping on the candigram
       */
      log('updating hop properties for: ' + options.candigram._id)
      if (options.stop) {
