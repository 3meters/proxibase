  ...
  function process() {
    ...
    function candigram(entityId, next) {
      ...
      db.candigrams.findOne({ _id: entityId }, function(err, doc) {
        ...
        if (!params.expand && options.candigram.hopsMax != -1 && options.candigram.hopCount >= options.candigram.hopsMax) {
          /*
    ...
    function manageLinks(options, next) {
      ...
      var event = params.expand ? 'expand_candigram' : 'move_candigram'
      ...
      if (!params.expand) action._fromEntity = options.currentPlaceId
      ...
      function inactivateLinks(options, next) {
        ...
        function invalidateLink(link, nextLink) {
          ...
          db.links.safeUpdate(inactiveLink, dbuser, function(err, savedLink) {
    ...
    function sendMessage(options, next) {
      ...
      var type = params.expand ? 'expand' : 'move'
      var message = {
        type: type,
      ...
      if (!params.expand) message.fromId = options.currentPlaceId
      /*
