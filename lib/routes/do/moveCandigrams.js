  ...
  setup()
    ...
    function setup() {
      ...
      async.parallel([
        ...
        if (err) return done(err)
        if (verbose) log('modified date earliest: ' + startDate)
        if (verbose) log('modified date latest: ' + endDate)
        process()
      })
    }
    ...
    function process() {
      ...
      function nextPlaceRange(options, next) {
         ...
         */
        var randomDate = random(startDate, endDate)
        if (verbose) log('modified date random: ' + randomDate)
         ...
         * we need the sort to get ordering on modifiedDate to handle $gte: randomDate.
         */
