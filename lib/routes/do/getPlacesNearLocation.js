    ...
    util.callService.factual(path, function(err, sres) {
      ...
      sres.body.data.forEach(function(place) {
        ...
        var entity = {
          ...
          name: place.name,
          sources: [{source: 'factual', id: place.factual_id, name:'factual', icon:'resource:source_generic', iconInverse:'resource:source_generic'}]
        }
        util.extend(entity, _entity)
        if (place.website) {
          entity.sources.push({source: 'website', id: place.website, name:'website', icon:'resource:source_website', iconInverse:'resource:source_website'})
        }
        entity.place = {
          source: 'factual',
          sourceId: entity._id,
    ...
    util.callService.foursquare(path, function(err, sres) {
      ...
      sres.body.response.venues.forEach(function(venue) {
        ...
        var entity = {
          _id: venue.id,
          name: venue.name,
          sources: [{source:'foursquare', id:venue.id, name:'foursquare', icon:'resource:source_foursquare', iconInverse:'resource:source_foursquare'}],
        }
        util.extend(entity, _entity)
        if (venue.url) {
          entity.sources.push({source:'website', id:venue.url, name:'website', icon:'resource:source_website', iconInverse:'resource:source_website'})
        }
        entity.place = {
          source:'foursquare',
          sourceId:venue.id,
        ...
        if ('contact' in venue) {
          ...
          if (entity.place.contact.twitter) {
            entity.sources.push({source:'twitter', id:venue.contact.twitter, name:'twitter', icon:'resource:source_twitter', iconInverse:'resource:source_twitter'})
