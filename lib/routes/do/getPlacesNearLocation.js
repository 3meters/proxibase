...
var _body = {
  source: {type: 'string', required: true,
    comment: 'Source must be factual, foursqure, or google',
    value: function(v) {
  ...
  limit: {type: 'number', default: 20,
    comment: 'Max limit is 100',
    commentFactual: 'Factual max limit is 50',
    value: function(v, body) {
      if (v > 50 && body.source === 'factual') return this.commentFactual
      if (v > 100) return this.comment
      return null
    },
  },
}
...
exports.main = function(req, res) {
    ...
    util.callService.factual(path, function(err, sres) {
      ...
      sres.body.data.forEach(function(place) {
        ...
        var entity = {
          ...
          name: place.name,
          sources: [{source: 'factual', id: place.factual_id, name:'factual',
            icon:'source_generic.png', iconInverse:'source_generic.png', origin:'factual'}]
        }
        util.extend(entity, _entity)
        if (place.website) {
          entity.sources.push({source: 'website', id: place.website, name:'website',
            icon:'source_website.png', iconInverse:'source_website.png', origin:'factual'})
        }
        entity.place = {
          source: 'factual',
          sourceId: entity._id,
  ...
  if (req.body.source === 'google') {
    ...
    // google returns 20 results at a time with a next page model. We accept
    // max results of 100.  cReqs is how many google requests we plan to make
    var cReqs = Math.min(5, Math.ceil(req.body.limit / 20))

    util.callService.google({logReq: true, path: path}, function(err, sres) {
      if (err) return finish(err)
      raw = sres.body.results
      log('body', sres.body)
      cReqs--
      if (cReqs && false) {
        path = pathBase + ''
    ...
    util.callService.foursquare(path, function(err, sres) {
      ...
      sres.body.response.venues.forEach(function(venue) {
        ...
        var entity = {
          _id: venue.id,
          name: venue.name,
          sources: [{source:'foursquare', id:venue.id, name:'foursquare',
            icon:'source_foursquare.png', iconInverse:'source_foursquare.png', origin:'foursquare'}],
        }
        util.extend(entity, _entity)
        if (venue.url) {
          entity.sources.push({source:'website', id:venue.url, name:'website',
            icon:'source_website.png', iconInverse:'source_website.png', origin:'foursquare'})
        }
        entity.place = {
          source:'foursquare',
          sourceId:venue.id,
        ...
        if ('contact' in venue) {
          ...
          if (entity.place.contact.twitter) {
            entity.sources.push({source:'twitter', id:venue.contact.twitter,
              name:'twitter', icon:'source_twitter.png', iconInverse:'source_twitter.png',
              origin:'foursquare'})
