      ...
      sres.body.response.venues.forEach(function(venue) {
        ...
        if ('contact' in venue) {
          ...
          if (entity.place.contact.twitter) {
            // TODO: discus with Jay, is this useful?
            entity.sources.push({
              source:'twitter',
    ...
    entities.forEach(function(entity) {
      ...
      if (entity.sources && entity.sources.length) {
        entity.sources.forEach(function(source) {
          util.extend(source, _sources[source.source])
          // TODO: discuss with Jay.
          // This removes properties prefixed with _ such as _sortorder.
          // Is this necessary?  If so should it be built into utils.extend?
          for (key in source) {
            if (key.indexOf('_') === 0) delete source[key]
          }
        })
      }
    })
