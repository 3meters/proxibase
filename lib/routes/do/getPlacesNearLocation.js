...
var main = function(req, res) {
  ...
  function getFactual() {
    ...
    callService.factual(search, function(err, sres, body) {
      ...
      body.data.forEach(function(place) {
        ...
        if (!(place.category_ids && place.category_ids.length)) return
        var factPlace = {
          name: place.name,
          place: {
            provider: 'factual',
          ...
          sources: [{
            ...
            data: {origin: 'factual', originId: place.factual_id}
          }]
        }
        if (place.website) {
          var source = {
  ...
  function mergePlaceEntities(places, cb) {
    ...
    db.entities
      ...
      .limit(1000) // Need to use a higher limit for our own location place entities
      .toArray(function(err, entIds) {
        ...
        log('entity hits within distance: ' + entIds.length)
        getEntities(req, {entityIds: entIds}, function(err, entities) {
          ...
          for (var e = entities.length; e--;) {
            if (entities[e].place && entities[e].place.location) {
              var distance = haversine(
                req.body.latitude,
                req.body.longitude,
                entities[e].place.location.lat,
                entities[e].place.location.lng
              )
              entities[e].place.distance = distance
            }
            ...
            if (entities[e].place && entities[e].place.location &&
              ...
              if (!match) {
                // We still show aircandi places even if the provider isn't tracking them anymore
                places.unshift(entities[e])                
              }              
            }
          }

          // We add custom places last so there is no chance they will get overwritten because of dup phone number
          ...
          // If requested limit has been exceeded, sort by distance and trim
          if (places.length > req.body.limit) {
            places.sort(function(a, b) { 
              return ((a.place.distance < b.place.distance) ? -1 : 
                      (a.place.distance > b.place.distance) ? +1 : 0) })
            places = places.slice(0, req.body.limit)
          }

          // Delete temp distance property used for sorting
          for (var p = places.length; p--;) {
            delete places[p].place.distance
          }
          cb(null, places)
        })
      })
