    ...
    util.callService.factual(path, function(err, sres) {
      ...
      sres.body.data.forEach(function(place) {
        ...
        var entity = {
          ...
          name: place.name,
          sources: [{source: 'factual', id: place.factual_id, name:'factual', icon:'source_generic.png', iconInverse:'source_generic.png'}]
        }
        util.extend(entity, _entity)
        if (place.website) {
          entity.sources.push({source: 'website', id: place.website, name:'website', icon:'source_website.png', iconInverse:'source_website.png'})
        }
        entity.place = {
          source: 'factual',
          sourceId: entity._id,
    ...
    util.callService.foursquare(path, function(err, sres) {
      ...
      sres.body.response.venues.forEach(function(venue) {
        ...
        var entity = {
          _id: venue.id,
          name: venue.name,
          sources: [{source:'foursquare', id:venue.id, name:'foursquare', icon:'source_foursquare.png', iconInverse:'source_foursquare.png'}],
        }
        util.extend(entity, _entity)
        if (venue.url) {
          entity.sources.push({source:'website', id:venue.url, name:'website', icon:'source_website.png', iconInverse:'source_website.png'})
        }
        entity.place = {
          source:'foursquare',
          sourceId:venue.id,
        ...
        if ('contact' in venue) {
          ...
          if (entity.place.contact.twitter) {
            entity.sources.push({source:'twitter', id:venue.contact.twitter, name:'twitter', icon:'source_twitter.png', iconInverse:'source_twitter.png'})
