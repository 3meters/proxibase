...
var run = exports.run = function(options, cb) {
  ...
  function grovelWebPage(uri, next) {
    ...
    request({uri: uri, json: false}, function(err, sres, body) {
      ...
      $('a[href*="twitter.com"]').each(function(i, elem) {
        newSources.push({
          type: 'twitter',
          url: url.parse($(this).attr('href')).path,
          name: $(this).text(),
        })
      })
  ...
  function dedupe(sources, newSources) {
    return newSources // TODO: implement
    /*
     *  Below is a guess. Test later.
    var concatted = sources.concat(newSources)
    var concattedDeduped = _.uniq(concatted, false, function(elem, i) {
      return elem.type + elm.id
    })
    return concattedDedupued.slice(sources.length)
    */
  }
  ...
  function finish(err) {
    log('finished called.  err: ', err)
    log('sources:', sources)
    log('newSources 2:', newSources)
