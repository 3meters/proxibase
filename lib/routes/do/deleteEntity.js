...
var run = exports.run = function(req, options, cb) {
  ...
  debug('deleteEnt activityDate', activityDate)
  ...
  function doDeleteEntity() {
    ...
    db[entityIdParsed.collectionName].safeRemove({ _id: entityId }, userModify, function(err, meta) {
      ...
      if (!meta.count) return res.error(proxErr.notFound())
  ...
  function updateActivityDateSynch() {
    ...
    // Short curcuiting to test trigger
    return deleteLinksFrom()
  ...
  function deleteLinksFrom() {
    ...
    db.links.find({ _from:entityId }).toArray(function(err, links) {
      ...
      var n = util.now()
      debug('delete entites calling deleteLinksFrom now', n)
