...
exports.main = function(req, res) {
  ...
  function deleteStrongLinkedEntities() {
    ...
    var query = { fromCollectionId:'0004', _to:entityId, strong:true }
    db.links.find(query).toArray(function(err, links) {
      ...
      function deleteEntity(deleteId, next) {
        ...
        db.entities.safeRemove({ _id:deleteId }, options, function(err) {
    ...
    if (req.body.skipActivityDate) {
      ...
      db.links.find({ _from:entityId }).toArray(function(err, links) {
        ...
        if (links.length == 0) {
           ...
           */
          db.entities.update(
            { _id: { $in: activityEntityIds }}, 
            { $set: { activityDate: activityDate }}, 
            { safe: true, multi: true }, 
            function(err) {
              if (err) util.logErr('Update entities failed in updateActivityDateSynch')
              deleteLinksFrom()
            }
          )
        }
