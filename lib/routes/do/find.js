...
module.exports = function(req, res) {
  ...
  function setProp(key, val) {
    var props = {
      ids: function(val) {
        if (val instanceof Array) req.query.ids = val
        else return new HttpErr(httpErr.badType, 'ids: array')
      },
      names: function(val) {
        if (val instanceof Array) req.query.name = val
        else return new HttpErr(httpErr.badType, 'name: array')
      },
      find: function(val) {
        req.query.find = val
      },
      fields: function(val) {
        if (val instanceof Array) req.query.fields = val
      ...
      lookups: function(val) {
        if (typeof val === 'boolean') req.query.lookups = val
        else return new HttpErr(httpErr.badType, 'lookups: boolean')
      },
      limit: function(val) {
        if (val === parseInt(val) && val > 0) req.query.limit = val
        else return new HttpErr(httpErr.badValue, 'limit')
      },
      children: function(val) {
        if (val instanceof Array) req.query.children = val
