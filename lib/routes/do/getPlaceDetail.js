...
function doPlaceDetail(req, res) {
  ...
  if (req.body.source == 'foursquare') {
    var svc = util.statics.services.foursquare
    var serviceUri = svc.root + req.body.sourceId + '?' + svc.cred
    sreq({ uri: serviceUri }, function(err, sres, body) {
      ...
      if (req.entity) {
        /* 
         * Linked entity - this is used to extend so the link info isn't required.
         */
        entity = req.entity
        ...
        entity.name = venue.name
        entity._id = venue.id
        entity.links = [{_to:'0008.11:11:11:11:11:11', _from:entity._id, primary:true}]
