...
exports.sendNotification = function(options, cb) {
  ...
  function addNotificationEntities() {
    ...
    async.each(entityIds, getEntity, finish)

    function getEntity(entityId, next) {

      var ops = {dbOps: {user: {_id: options.blockedId, role: 'user'}}}
      getEntities(ops, {entityIds: [entityId]}, function(err, results) {
  ...
  function checkForProximityNearbyNotifications() {
    ...
    if (!options.beaconIds) return checkForLocationNearbyNotifications()
  ...
  function checkForLocationNearbyNotifications() {
    ...
    if (!location) return checkForEntityWatchNotifications()
  ...
  function checkForEntityWatchNotifications() {
    ...
    var next = checkForToEntityCreateNotifications
    ...
    db.links.find({
      ...
      var ids = []
      links.forEach(function(link) { ids.push(link._from) })
      findInstalls(ids, 'watch_to', next)
    })
  }
   ...
   */
  function checkForToEntityCreateNotifications(err) {
    if (err) return done(err)
    if (options.log) log('Checking for [to] entity create notifications')

    var next = checkForFromEntityCreateNotifications
    ...
    findInstalls(userIds, 'own_to', next)
  }
   ...
   * a new entity. Owning trumps watching.
   */
  function checkForFromEntityCreateNotifications(err) {
    if (err) return done(err)

    var next = finishNotification
    ...
    findInstalls(userIds, 'own_from', next)
  }

  /*
   * Assemble notification notifications.
   */
  function finishNotification(err) {
    if (err) return done(err)
  ...
  function findInstalls(userIds, trigger, cb) {
    var query = {
      parseInstallId: { $exists: true },
      _user: { $in: userIds },
    }
    ...
    db.installs.find(query, fields).toArray(function (err, installs) {
      ...
      installs.forEach(function(install) {
        if (install._user === options.blockedId) return
