...
var buildNotification = exports.buildNotification = function(options) {
  ...
  if (options.event === 'watch_entity_patch') {
    ...
    privacy = (options.to.visibility === 'public') ? '' : 'private '
    if (options.trigger === 'own_to') {
      notification.summary = util.format('<b>%s</b> added your %spatch <b>%s</b> as a favorite', options.from.name, privacy, options.to.name)
      notification.ticker = notification.summary
    }
    else if (options.trigger === 'own_both') {
      notification.summary = util.format('<b>%s</b> added a %spatch <b>%s</b> as a favorite', options.from.name, privacy, options.to.name)
      notification.ticker = notification.summary
    }
  }
  ...
  if (options.deviceTarget === 'ios' || options.deviceTarget === 'ios_7') {
    ...
    var maxSize = (options.deviceTarget === 'ios_7') ? (256 - wrapperSize) : (2048 - wrapperSize)
