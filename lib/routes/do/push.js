...
var buildNotification = exports.buildNotification = function(options) {
  ...
  if (options.event === 'watch_entity_place') {
    ...
    if (options.trigger === 'own_both') {
      notification.subtitle = 'Started watching as owner: ' + options.to.name
    }
    else if (options.trigger === 'own_to') {
      notification.subtitle = 'Started watching your patch: ' + options.to.name
      notification.ticker = options.from.name + ': Started watching a patch of yours'
    }
    else if (options.trigger === 'own_from') {
      notification.subtitle = 'Started watching: ' + options.to.name
    }
  }
    ...
    if (options.trigger === 'own_to') {
      notification.subtitle = 'Has pending request to join your patch: ' + options.to.name
      notification.ticker = options.from.name + ': Wants to join a patch of yours'
    }
    else if (options.trigger === 'own_from') {
      notification.subtitle = 'Sent request to join: ' + options.to.name
    }
  }
    ...
    if (options.trigger === 'own_both') {
      notification.subtitle = 'Joined as owner: ' + options.to.name
    }
    else if (options.trigger === 'own_to') {
      notification.subtitle = 'Has been approved to join your patch : ' + options.to.name
    }
    else if (options.trigger === 'own_from') {
      notification.subtitle = 'Your request to join has been approved: ' + options.to.name
      notification.ticker = options.to.creator.name + ': Approved your request to join: ' + options.to.name
    }
  }
    ...
    if (options.trigger === 'nearby') {
      notification.subtitle = 'Dropped a new patch nearby.'
      notification.ticker = options.to.creator.name + ': Dropped a new patch nearby.'
    ...
    if (options.event === 'insert_entity_message_share') {
      ...
      if (options.from.photo) {
        notification.subtitle = 'Shared a photo with you'
        notification.ticker = options.from.creator.name + ': shared a photo with you'
      }
      else {
        notification.subtitle = 'Shared with you'
        notification.ticker = options.from.creator.name + ': Shared with you'
      }
    }
    else {
      notification.userId = options.from.creator._id
      if (options.to.schema === statics.schemaMessage) {
        if (options.trigger === 'nearby') {
          notification.subtitle = 'Replied to a message at a patch nearby.'
          notification.priority = 1
        }
        else if (options.trigger === 'watch_to') {
          notification.subtitle = 'Replied to a message at a patch you are watching.'
        }
        else if (options.trigger === 'own_to') {
          notification.subtitle = 'Replied to a message at a patch of yours.'
        }
      }
      else {
        if (options.trigger === 'nearby') {
          notification.subtitle = 'Sent a message to a patch nearby.'
          notification.priority = 1
        }
        else if (options.trigger === 'watch_to') {
          notification.subtitle = 'Sent a message to a patch you are watching.'
        }
        else if (options.trigger === 'own_to') {
          notification.subtitle = 'Sent a message to a patch of yours.'
        }
      }
    }
  }
