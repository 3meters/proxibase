...
exports.sendNotification = function(options, cb) {
  ...
  function finishNotification() {
    ...
    options.triggers.forEach(function(trigger) {
      ...
      for (var deviceTarget in parseInstalls ) {
        if (parseInstalls[deviceTarget].length > 0) {
          var params = {
            event: options.event,
            to: options.to,
            from: options.from,
          ...
          if (test_push) {
            notification.parseInstallIds = parseInstalls[deviceTarget]
          }
          else {
            sendNotifications(notification, parseInstalls[deviceTarget], options)
          }
        }
      }
    })
...
var sendNotifications = exports.sendnotifications = function(notification, parseInstallIds, options) {
   ...
   */
  if (options.log) log('Sending ' + notification.trigger + ' notifications to ' + parseInstallIds.length + ' installs(s)')
  ...
  function finish(err) {
    if (err) {
      util.logErr('Error clearing parseInstallId', err)
      return
    }
  }
}
...
var buildNotification = exports.buildNotification = function(options) {
  ...
  if (options.deviceTarget === 'ios' || options.deviceTarget === 'ios_7') {
    ...
    var maxSize = (deviceTarget === 'ios_7') ? 256 : 2048
    var remaining = maxSize - notification.length
    notification.aps.alert = notification.summary.substring(0, remaining)
  }
