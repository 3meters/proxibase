...
var buildNotification = exports.buildNotification = function(options) {
  ...
  if (options.event === 'watch_entity_patch') {
    ...
    privacy = (options.to.visibility === 'public') ? 'public' : 'private'
    ...
    privacy = (options.to.visibility === 'public') ? 'public' : 'private'
    if (options.trigger === 'own_to') {
      notification.subtitle = util.format('<b>%s</b> wants to join your %s patch <b>%s</b>', options.from.name, privacy, options.to.name)
      notification.ticker = util.format('<b>%s</b> wants to join your %s patch <b>%s</b>', options.from.name, privacy, options.to.name)
    }
    else if (options.trigger === 'own_from') {
      notification.subtitle = util.format('You asked to join %s patch <b>%s</b>', privacy, options.to.name)
    }
  }
    ...
    privacy = (options.to.visibility === 'public') ? 'public' : 'private'
    if (options.trigger === 'own_to') {
      notification.subtitle = util.format('<b>%s</b> likes your %s patch <b>%s</b>', options.from.name, privacy, options.to.name)
      notification.ticker = notification.subtitle
    }
  }
    ...
    privacy = (options.to.visibility === 'public') ? 'public' : 'private'
    if (options.trigger === 'own_to') {
      notification.subtitle = util.format('<b>%s</b> likes your message', options.from.name, privacy, options.to.name)
      notification.ticker = notification.subtitle
    }
  }
    ...
    if (options.event === 'insert_entity_message_share') {
      ...
      if (options.to.schema === statics.schemaMessage) {
        if (options.to.patch) {
          if (options.trigger === 'nearby') {
            notification.subtitle = util.format('<b>%s</b> replied to a message at nearby patch <b>%s</b>: "%s"', options.from.creator.name, options.to.patch.name, options.from.description)
            notification.priority = 1
          }
          else if (options.trigger === 'watch_to') {
            notification.subtitle = util.format('<b>%s</b> replied to a message at patch <b>%s</b>: "%s"', options.from.creator.name, options.to.patch.name, options.from.description)
          }
          else if (options.trigger === 'own_to') {
            notification.subtitle = util.format('<b>%s</b> replied to your message at patch <b>%s</b>: "%s"', options.from.creator.name, options.to.patch.name, options.from.description)
          }
        }
        else {
          if (options.trigger === 'nearby') {
      ...
      /* Message to a patch */
        ...
        if (options.trigger === 'nearby') {
          subtitle = '<b>%s</b> sent a %s to a nearby patch <b>%s</b>'
          ...
          subtitle = '<b>%s</b> sent a %s to patch <b>%s</b>'
          if (options.from.description)
            notification.subtitle = util.format(subtitle += ': "%s"', options.from.creator.name, context, options.to.name, options.from.description)
          else
            notification.subtitle = util.format(subtitle, options.from.creator.name, context, options.to.name)
          ...
          subtitle = '<b>%s</b> sent a %s to your patch <b>%s</b>'
          if (options.from.description)
            notification.subtitle = util.format(subtitle += ': "%s"', options.from.creator.name, context, options.to.name, options.from.description)
          else
            notification.subtitle = util.format(subtitle, options.from.creator.name, context, options.to.name)
