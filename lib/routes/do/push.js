...
exports.sendNotification = function (options, cb) {
  ...
  function checkForProximityNearbyNotifications() {
    ...
    if (!options.beaconIds) {
      ...
      log('Checking for nearby notifications by proximity')
      ...
      util.db.installs.find(query, { registrationId: true, _user: true }).toArray(function (err, installs) {
        ...
        log('Nearby candidates by proximity mapped: ' + installs.length)
  ...
  function checkForLocationNearbyNotifications() {
    ...
    if (!location) {
      ...
      log('Checking for nearby notifications by location')
      /*
      ...
      util.db.installs.find(query, { registrationId: true, _user: true }).toArray(function (err, installs) {
        ...
        log('Nearby candidates by location mapped: ' + installs.length)
        for (var i = installs.length; i--;) {
          if (!options.blockedId || options.blockedId !== installs[i]._user) {
            registrationMap[installs[i].registrationId] = 'nearby'
          }
