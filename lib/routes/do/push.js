...
exports.sendNotification = function(options, cb) {
  ...
  function checkForLocationNearbyNotifications() {
    ...
    if (!location) {
       ...
       */
      var query = {
        registrationId:   { $exists: true },    // install must have this to get notifications
  ...
  function finishNotification() {
    ...
    if (options.log) log('Active registration map:', registrationMap)
    ...
    options.triggers.forEach(function(trigger) {
      ...
      if (registrationIds.length > 0) {
        ...
        if (test_registration) {
          ...
          sendNotifications(notification, registrationIds, options)
        }
      }
    })
  ...
  function findInstalls(userIds, trigger, callback) {
    ...
    db.installs.find(query, { registrationId: true, _user: true }).toArray(function (err, installs) {
      if (err) callback(err)
...
var sendNotifications = exports.sendnotifications = function(notification, registrationIds, options) {
  /*
   ...
   */
  if (options.log) log('Sending ' + notification.trigger + ' notifications to ' + registrationIds.length + ' installs(s)')
