...
module.exports.main = function(req, res) {
  ...
  var _body = {
    ...
    status:             {type: 'string', value: 'requested|approved|revoked' },
  ...
  var link = { _from:req.body.fromId, _to:req.body.toId, type:req.body.type, status:req.body.status }

  db.links.safeInsert(link, dbOps, function(err, savedDoc) {
    if (err) return res.error(err)
    ...
    db[toIdParsed.collectionName].update({ _id: req.body.toId }, { $set: { activityDate: activityDate }}, { safe: true, multi: false }, function(err) {
      ...
      if (req.body.actionEvent) {
        log('Logging ' + req.body.actionEvent + ' action for ' + req.body.toId)
        methods.logAction({
