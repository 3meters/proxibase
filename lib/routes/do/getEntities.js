...
var run = exports.run = function(req, options, cb) {
  ...
  function addEntityPatches() {
    ...
    var query = { _id: { $in: parts.patchIds }}
    var ops = util.clone(req.dbOps)
    ops.limit = joinLimit
    ...
    function getEntityPatch(patchId) {
      if (parts.patches) {
        ...
        if (patch) {
          ...
          if (!result.photo && patch.category && patch.category.photo) {
            result.photo = patch.category.photo
            result.photo.colorize = true
            result.photo.colorizeKey = patch.category.name
          }
          return result
        }
      }
