...
var run = exports.run = function(req, options, cb) {
  ...
  function addEntities() {
    ...
    function finish(err) {
      if (err) return done(err)
      log('entities found: ' + parts.entities.length)
  ...
  function addLinksInCounts() {
    ...
    db.links.safeFind(req.query, function(err, results) {
      ...
      log('linkInCounts: ', parts.linksInCounts)
      addLinksOut()
    })
  }
  ...
  function addLinkStats() {
    ...
    if (!parts.linkIds) {
      ...
      req.query = {
        countBy: ['_entity', 'event'],
        find: { _entity:{ $in:parts.linkIds }, event:{ $in:['link_proximity', 'link_proximity_minus'] }}
      }

      //req.query = {countBy: ['_entity'], find:{ _entity:{ $in:parts.linkIds } }}
  ...
  function buildPayload() {
    ...
    parts.entities.forEach(function(entity) {
      log('building payload', entity._id)
    ...
    function makeShortcut(entity) {
      ...
      if (entity.schema === util.statics.schemaMessage) {
    ...
    function getEntityUser(userId) {
      if (parts.users) {
        ...
        if (user != null) {
          result = { _id: user._id, photo: user.photo, area: user.area, name: user.name, schema: user.schema }
          return result
        }
      }
    }
