...
var _body = {
  ...
  actions:        { type: 'object', value: {
    sort:           { type: 'object', default: { activityDate: -1 }},
...
exports.main = function(req, res) {
  ...
  run(req, options, function(err, entities, extras) {
      if (err) return res.error(err)
      res.send({
        data: entities,
...
var run = exports.run = function(req, options, cb) {
  ...
  var parts = { entities:[], links:[], entityIds:[], userIds:[], placeIds:[], actions:[] }
  var activeLinkTypes = []
  var lookupTypes = []
  var inCountTypes = []
  var outCountTypes = []
  ...
  function addEntities() {
    ...
    function processCollection(collectionName, next) {
      ...
      /*
       * Actions shift the sorting and paging logic to here.
       */
      if (options.actions) {
        db[collectionName]
          .find(query)
          .sort(options.actions.sort)
          .skip(options.actions.skip)
          .limit(options.cursor.limit + (options.cursor.limit === 0 ? 0 : 1))
          .toArray(function(err, entities) {

          if (err) return next(err)

          if (entities.length > options.actions.limit) {
            entities.pop()
            extras.more = true
          }
  ...
  function addActions() {
    ...
    if (!options.actions) {
      ...
      async.forEach(options.entityIds, process, finish)
      ...
      function process(entityId, next) {
        ...
        if (options.actions.where) {
          query = { $and: [query, options.actions.where] }
        }

        log('actions query: ', query)
        db.actions
          .find(query)
          .sort({ modifiedDate: -1 })
          .limit(options.actions.limit)
          .toArray(function(err, actions) {

          if (err) return next(err)
          if (actions.length > 0) {
            parts.actions.push.apply(parts.actions, actions)
          }
          next()
        })
      }
  ...
  function addLinksInCounts() {
    ...
    req.query = {countBy: ['_to', 'type', 'fromSchema', 'inactive'], find: query}
  ...
  function addLinksOutCounts() {
    ...
    req.query = { countBy: ['_from', 'type', 'toSchema', 'inactive'], find: query}
  ...
  function buildPayload() {
    ...
    parts.entities.forEach(function(entity) {
      ...
      /* Actions */
      if (options.actions && parts.actions.length > 0) {
        entity.actions = getActions(entity._id)
      }
      ...
      if (parts.linksInCounts) {
        parts.linksInCounts.forEach(function(linkCount) {
          if (linkCount._to === entityId) {
            linkCounts.push({ type: linkCount.type, schema: linkCount.fromSchema, inactive: linkCount.inactive, count: linkCount.countBy })
          }
        })
      }
      return linkCounts
      ...
      if (parts.linksOutCounts) {
        parts.linksOutCounts.forEach(function(linkCount) {
          if (linkCount._from === entityId) {
            linkCounts.push({ type: linkCount.type, schema: linkCount.toSchema, inactive: linkCount.inactive, count: linkCount.countBy })
          }
        })
      }
      return linkCounts
    ...
    function getActions(entityId) {
      var actions = []
      if (parts.actions) {
        parts.actions.forEach(function(action) {
          if (action._target === entityId || action._toEntity === entityId) {
            actions.push(action)
          }
        })
      }
