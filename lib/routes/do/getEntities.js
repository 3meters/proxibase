...
var _body = {
  ...
  ignoreInactive: { type: 'boolean', default: false },
  ...
  function buildPayload() {
    ...
    parts.entities.forEach(function(entity) {
      ...
      if (entity.schema === util.statics.schemaApplink) {
        if (entity.type == 'twitter') {
          entity.photo = { 
            prefix: "/img/applinks/twitter.png", 
            source: "assets",
          }
        }
      }
    ...
    function getLinksIn(entityId, limitedLinks) {
      ...
      limitedLinks.forEach(function(link) {
        if (link._to === entityId && linkTypes.indexOf(link.type) > -1) {
          ...
          var decoratedLink = { _from: link._from, type: link.type, strong: link.strong, inactive: link.inactive, stats: link.stats, modifiedDate: link.modifiedDate }
    ...
    function getLinksOut(entityId, limitedLinks) {
      ...
      limitedLinks.forEach(function(link) {
        if (link._from === entityId && linkTypes.indexOf(link.type) > -1) {
          ...
          var decoratedLink = { _to: link._to, type: link.type, strong: link.strong, inactive: link.inactive, stats: link.stats, modifiedDate: link.modifiedDate }
          if (link.proximity) decoratedLink.proximity = link.proximity
    ...
    function getEntityPlace(placeId) {
      if (parts.places) {
        ...
        if (place != null) {
          result = { _id: place._id, photo: place.photo, name: place.name }
