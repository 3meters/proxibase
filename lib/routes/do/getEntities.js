  ...
  function addEntities() {
    ...
    function processCollection(collectionName, next) {
      ...
      db[collectionName].safeFind(query, req.dbOps, function(err, entities) {
        ...
        processEntities(entityIds, entities, next)
      })
    }

    function processEntities(entityIds, unsortedEntities, next) {
      var entities = []
      ...
      if (unsortedEntities.length > 0) {
        ...
        unsortedEntities.forEach(function(entity) {
  ...
  function addLinksIn() {
    ...
    function processEntity(entityId, next) {
      ...
      function processLinkType(activeLink, next) {
        ...
        ops.sort = {modifiedDate: -1}  // BUG, this should be default not override
        // BUG: missing ops.skip = activeLink.skip
  ...
  function addLinksOut() {
    ...
    function processEntity(entityId, next) {
      ...
      function processLinkType(activeLink, next) {
        ...
        var ops = util.clone(req.dbOps)
        ops.sort = {modifiedDate: -1}  // BUG: this should be default, not overrid
        // BUG  missing ops.skip = activeLink.skip
        ops.limit = activeLink.limit
