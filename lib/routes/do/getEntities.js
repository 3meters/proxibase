...
exports.main = function(req, res) {
  ...
  options.log = options.log || options.test

  run(options, req.dbOps, function(err, entities, more) {
    if (err) return res.error(err)
    res.send({
      data: entities,
 ...
 */
var run = exports.run = function(options, dbOps, cb) {
  ...
  function addEntities() {
    ...
    async.eachSeries(collectionNames, processCollection, finish)
    ...
    function processCollection(collectionName, next) {
      ...
      if (entityIds.length === 1) {
        ...
        db[collectionName].safeFindOne(query, dbOps, function(err, entity) {
      ...
      // SafeFind does not check acl-based perms
        ...
        var ops = _.cloneDeep(dbOps)
        ops.limit = joinLimit

        db[collectionName].safeFind(query, ops, function(err, entitiesUnsorted) {
          if (err) return next(err)
  ...
  function addLinksIn() {
    ...
    async.eachSeries(parts.entityIds, processEntity, finish)

    function processEntity(entityId, next) {

      async.eachSeries(inActiveLinks, processLinkType, finish)
  ...
  function addLinksInCounts() {
    ...
    findOps.countBy = ['_to', 'fromSchema', 'type', 'enabled']
  ...
  function addLinksOut() {
    ...
    function processEntity(entityId, next) {
      ...
      async.eachSeries(outActiveLinks, processLinkType, finish)
  ...
  function addLinksOutCounts() {
    ...
    var findOps = _.cloneDeep(dbOps)
    findOps.countBy = ['_from', 'toSchema', 'type', 'enabled']
  ...
  function addLinkedEntities() {
    ...
    function processCollection(collectionName, next) {
      ...
      var ops = _.cloneDeep(dbOps)
      async.eachSeries(entityIds, getLinkedEntity, next)

      function getLinkedEntity(id, nextLinkedEnt) {

        db[collectionName].safeFindOne({_id: id}, ops, function(err, linkedEntity) {

          if (err) return next(err)
          if (!linkedEntity) return nextLinkedEnt()
          /*
    ...
    parts.entities.forEach(function(entity) {
      ...
      if (options.links && options.links.active && options.links.active.length > 0) {
        ...
        if (parts.linksInCounts) {
          var linksInCounts = getlinksInCounts(entity._id) /* Link counts: likes, messages, watchers */
      ...
      if (parts.linksInCounts) {
        parts.linksInCounts.forEach(function(linkCount) {
          if (linkCount._to === entityId) {
            linkCounts.push({ type: linkCount.type, schema: linkCount.fromSchema, enabled: linkCount.enabled, count: linkCount.count })
          }
        })
      }
      return linkCounts
      ...
      if (parts.linksOutCounts) {
        parts.linksOutCounts.forEach(function(linkCount) {
          if (linkCount._from === entityId) {
            linkCounts.push({ type: linkCount.type, schema: linkCount.toSchema, enabled: linkCount.enabled, count: linkCount.count })
          }
        })
      }
      return linkCounts
