...
var _link = {
  fields: {
    ...
    schema:     { type: 'string', required: true,
      validate:  function(v) {
        if (!statics.schemas[v]) {
          return 'Unknown schema ' + v
        }
    }},
...
var run = exports.run = function(req, options, cb) {
  ...
  var inCountTypes = []
  var outCountTypes = []
  var inCountCollections = []
  var outCountCollections = []
  var linkTypes = []
  var inTypes = []
  var outTypes = []
  var inActiveLinks = []
      ...
      db[collectionName]
        ...
        if (entities.length > 0) {
          ...
          parts.entities.forEach(function(entity) {
            entity.schema = util.parseId(entity._id).schemaName
            log('debug entity.schema: ' + entity.schema)
  ...
  function prepLinks() {
    ...
    if (!options.links || !options.links.active || options.links.active.length == 0) {
      ...
      options.links.active.forEach(function(activeLink) {
        activeLink.collection = statics.schemas[activeLink.schema].collection
        ...
        if (activeLink.direction === 'both' || activeLink.direction === 'in') {
          ...
          if (activeLink.count) {
            ...
            inCountCollections.push(activeLink.collection)
          }
        }
        if (activeLink.direction === 'both' || activeLink.direction === 'out') {
          ...
          if (activeLink.count) {
            ...
            outCountCollections.push(activeLink.collection)
          }
        }
      })
      addLinksIn()
    ...
    function processEntity(entityId, next) {
      ...
      function processLinkType(activeLink, next) {
        var query = {
          ...
          fromCollection: activeLink.collection,
          inactive: false,
        }
  ...
  function addLinksInCounts() {
    ...
    var query = {
      ...
      fromCollection: { $in: inCountCollections },
      inactive: false,
    }

    req.query = {countBy: ['_to', 'type', 'fromCollection'], find: query}
    ...
    function processEntity(entityId, next) {
      ...
      function processLinkType(activeLink, next) {
        var query = {
          _from: entityId,
          type: activeLink.type,
          toCollection: activeLink.collection,
          inactive: false,
        }
  ...
  function addLinksOutCounts() {
    ...
    var query = {
      ...
      toCollection: { $in: outCountCollections },
      inactive: false,
    }

    req.query = { countBy: ['_from', 'type', 'toCollection'], find: query}
      ...
      allLinks.forEach(function(link) {
        if (link._to === entityId && linkTypes.indexOf(link.type) > -1) {
          ...
          var decoratedLink = {
            _from: link._from,
            type: link.type,
            schema: statics.schemaLink,
            fromSchema: statics.collections[link.fromCollection].schema,
            toSchema: statics.collections[link.toCollection].schema,
            targetSchema: statics.collections[link.fromCollection].schema,
      ...
      allLinks.forEach(function(link) {
        if (link._from === entityId && linkTypes.indexOf(link.type) > -1) {
          ...
          var decoratedLink = {
            ...
            schema: statics.schemaLink,
            toSchema: statics.collections[link.toCollection].schema,
            targetSchema: statics.collections[link.toCollection].schema,
            fromSchema: statics.collections[link.fromCollection].schema,
            inactive: link.inactive,
            stats: link.stats,
            sortDate: link.modifiedDate,
      ...
      if (parts.linksInCounts) {
        parts.linksInCounts.forEach(function(linkCount) {
          if (linkCount._to === entityId) {
            linkCounts.push({
              type: linkCount.type,
              schema: statics.collections[linkCount.fromCollection].schema,
              count: linkCount.countBy
            })
          }
        })
      }
      ...
      if (parts.linksOutCounts) {
        parts.linksOutCounts.forEach(function(linkCount) {
          if (linkCount._from === entityId) {
            linkCounts.push({
              type: linkCount.type,
              schema: statics.collections[linkCount.toCollection].schema,
              count: linkCount.countBy
            })
          }
        })
