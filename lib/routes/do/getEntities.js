...
var run = exports.run = function(req, options, cb) {
  ...
  function addEntities() {
    ...
    function processCollection(collectionName, next) {
      ...
      db[collectionName].safeFind(query, req.dbOps, function(err, entitiesUnsorted) {
        ...
        // log('entitiesUnsorted', entitiesUnsorted)
    ...
    function processEntities(entities, next) {
      ...
      checkReadPermissions(entities, next)
    }

    function checkReadPermissions(entities, next) {

      async.eachSeries(entities, userCanReadLinkedTos, next)

      // Build the asReader map for each entity
      function userCanReadLinkedTos(entity, nextEntity) {

        if (!entity.restricted) return set(true)  // public entity, ownerAccess?
        if (!(req.dbOps && req.dbOps.user)) return set(false)  // anonymous

        var linkQry = {
          _to: entity._id,
          _from: req.dbOps.user._id,
          type: 'watch',
        }
        db.links.safeFindOne(linkQry, req.dbOps, function(err, watchLink) {
          if (err) return nextEntity(err)

          if (watchLink) return set(true)

          // if entity has a _place field, check it for watch link
          if (!entity._place) return set(false)
          linkQry._to = entity._place
          db.links.safeFindOne(linkQry, req.dbOps, function(err, watchLink) {
            if (err) return nextEntity(err)
            if (watchLink) return set(true)
            set(false)
        ...
        function set(userIsReader) {
          asReader[entity._id] = userIsReader
          next()
        }
      }
    }
          ...
          if (links.length !== 0) {
            for (var i = 0; i < links.length; i++) {
              ...
              if (lookupTypes.indexOf(links[i].type) > -1) {
                ...
                // If the user has elevated permissions on the _to entity
                // elevate the read permissions on the _from entity
                if (asReader[entityId]) asReader[links[i]._from] = true
              }
            }
          }
          next()
  ...
  function addLinkedEntities() {
    ...
    function processCollection(collectionName, next) {
      ...
      var asReaderEntityIds = entityIds.map(function(id) {
        if (asReader[id]) return id
      })

      if (asReaderEntityIds.length !== entityIds.length) {
        return finish(perr.badValue('mixed security queries not supported, use getEntityForEntities'))
      }
      ...
      // Elevate the read permission
      if (asReaderEntityIds.length) ops.asReader = true
  ...
  function addEntityUsers() {
    ...
    var query = { _id: { $in: parts.userIds }}
    var ops = util.clone(req.dbOps)
    ops.limit = joinLimit
  ...
  function addEntityPlaces() {
    ...
    var query = { _id: { $in: parts.placeIds }}
    var ops = util.clone(req.dbOps)
    ops.limit = joinLimit
    db.places.safeFind(query, ops, function(err, places) {
      if (err) return done(err)
  ...
  function buildPayload() {
    ...
    parts.entities.forEach(function(entity) {
      ...
      if (options.links && options.links.active && options.links.active.length > 0) {
        debug('getEntity options', options)
        if (parts.links && !(options.links && options.links.statsOnly)) {
