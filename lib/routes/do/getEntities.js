  ...
  function prepLinks() {
    ...
    if (!options.links || !options.links.active || options.links.active.length == 0) {
      ...
      options.links.active.forEach(function(activeLink) {
        ...
        if (options.links.shortcuts || activeLink.load) {
  ...
  function addLinkStats() {
    ...
    if (!parts.linkIds) {
      ...
      req.query = {
        ...
        find: { _target:{ $in:parts.linkIds }, type:{ $in:['link_proximity_first', 'link_proximity', 'link_proximity_minus'] }}
      }
    ...
    function getLinksIn(entityId, limitedLinks) {
      ...
      limitedLinks.forEach(function(link) {
        if (link._to === entityId && linkTypes.indexOf(link.type) > -1) {
          ...
          var decoratedLink = { _from: link._from, type: link.type, stats: link.stats }
          ...
          if (options.links.shortcuts) {
            decoratedLink.shortcut = { 
              id: linkedEntity._id,
              name: linkedEntity.name, 
              photo: linkedEntity.photo,
              type: linkedEntity.schema,
              app: linkedEntity.schema,
            }
            if (linkedEntity.schema === util.statics.schemaApplink) {
              decoratedLink.shortcut.app = linkedEntity.type
              decoratedLink.shortcut.appId = linkedEntity.appId
              decoratedLink.shortcut.appUrl = linkedEntity.appUrl
              decoratedLink.shortcut.position = linkedEntity.position
              if (!decoratedLink.shortcut.name) {
                decoratedLink.shortcut.name = linkedEntity.type
              }
              if (!decoratedLink.shortcut.photo) {
                decoratedLink.shortcut.photo = { prefix: '/img/applinks/' + decoratedLink.shortcut.app + '.png', source: 'assets' };
              }
            }
          }
    ...
    function getLinksOut(entityId, limitedLinks) {
      ...
      limitedLinks.forEach(function(link) {
        if (link._from === entityId && linkTypes.indexOf(link.type) > -1) {
          ...
          var decoratedLink = { _to: link._to, type: link.type, stats: link.stats }
          if (link.proximity) decoratedLink.proximity = link.proximity
