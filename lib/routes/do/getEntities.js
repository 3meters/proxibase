...
var run = exports.run = function(req, options, cb) {
  ...
  var parts = { entities:[], links:[], entityIds:[], userIds:[], placeIds:[], more: false }
  var activeLinkTypes = []
  var lookupTypes = []
  var inCountTypes = []
  var outCountTypes = []
  ...
  function addEntities() {
    ...
    function processCollection(collectionName, next) {
      ...
      if (options.cursor && options.cursor.sort) req.dbOps.sort = options.cursor.sort
      if (cursor.limit) req.dbOps.limit = cursor.limit + 1
      if (cursor.skip) req.dbOps.skip = cursor.skip
      db[collectionName].safeFind(query, req.dbOps, function(err, entities) {
        if (err) return next(err)
        if (entities.length > cursor.limit) {
          entities.pop()
          parts.more = true
        }
  ...
  function addLinkedEntities() {
    ...
    function processCollection(collectionName, next) {
      ...
      // BUG: The following is a gaping security hole
      ops.asAdmin = true
  ...
  function addEntityUsers() {
    ...
    var query = { _id: { $in: parts.userIds }, enabled: true}
    var ops = util.clone(req.dbOps)

    // BUG: this is a security hole
    ops.asAdmin = true
  ...
  function addEntityPlaces() {
    ...
    var query = { _id: { $in: parts.placeIds }, enabled: true}
    var ops = util.clone(req.dbOps)
