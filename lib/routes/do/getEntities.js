...
var _link = {
  fields: {
    ...
    limit:      { type: 'number', default: statics.optionsLimitDefault,    // always the top n based on modifiedDate
      validate: function(v) {
...
var run = exports.run = function(req, options, cb) {
  ...
  debug('getEntities log: ', log)

  addEntities()

  function addEntities() {
    if (log) log('addEntities')
    ...
    function finish(err) {
      if (err) return done(err)
      if (log) log('entities found: ' + parts.entities.length)
  ...
  function addLinksInCounts() {
    ...
    var findOps = {countBy: ['_to', 'type', 'fromSchema', 'inactive']}
  ...
  function addLinksOutCounts() {
    ...
    var findOps = { countBy: ['_from', 'type', 'toSchema', 'inactive']}
  ...
  function buildPayload() {
    ...
    parts.entities.forEach(function(entity) {
      if (log) log('building payload', entity._id)
