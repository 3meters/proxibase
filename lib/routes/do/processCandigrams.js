    ...
    db.candigrams.find(query).toArray(function(err, docs) {
      ...
      function process(candigram, next) {
        if (candigram.createdDate + candigram.lifetime >= activityDate) {
          ...
          /* Retirement qualifies for activityDate update on candigram and all actively linked places */
          if (!params.skipActivityDate) {
            methods.propagateActivityDate(candigram._id, activityDate, true, false) // Fire and forget
          }
