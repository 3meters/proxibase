...
function addLocations(req, res) {
  if (req.state.observation) {
    addUsers(req, res);
  }
  else {
    query = {_beacon:{$in:req.parts.beaconIds}, _entity:{$in:req.parts.entityIds}}
    db.observations.find(query).toArray(function(err, observations) {
      if (err) return res.error(err)
      req.parts.observations = observations
      addUsers(req, res)
    })
  }
}
...
function buildPayload(req, res) {
  ...
  function getBeaconLinks(entityId, req) {
    ...
    req.parts.beaconLinks.forEach(function(beaconLink) {
      if (beaconLink._from === entityId) {
        beaconLinks.push({beaconId:beaconLink._to, linkId:beaconLink._id})
      }
    })
    return beaconLinks
  }
  ...
  function getLocation(entityId, beaconId, req) {
    var result = null
    if (req.state.observation) {
      result = {lat:req.state.observation.latitude, lng:req.state.observation.longitude}
      return result
    }
    else {
      req.parts.observations.forEach(function(observation) {
        if (observation._entity === entityId && observation._beacon === beaconId) {
          result = {lat:observation.latitude, lng:observation.longitude}
          return result
        }
      })
      return result
