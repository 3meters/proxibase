...
var propogateActivityDate = module.exports.propogateActivityDate = function (entityId, activityDate, updateSelf) {
  ...
  function updateSelf() {
    if (!updateSelf) {
      ...
      var entityIdParsed = util.parseId(entityId)
      util.db[entityIdParsed.collectionName].findOne({_id: entityId}, function(err, doc) {
        if (err || !doc) {
          util.logErr('Find failed in propogateActivityDate', err)
          return
        }
        doc.activityDate = activityDate
        util.db[entityIdParsed.collectionName].safeUpdate(doc, options, function(err, updatedDoc) {
          if (err || !updatedDoc) {
      ...
      for (var i = links.length; i--;) {
        ...
        util.db.collection(cName).findOne({ _id: links[i]._to }, function (err, doc) {
          ...
          if (doc) {
             ...
             * Currently something like five seconds.
             */
            if (!doc.activityDate 
