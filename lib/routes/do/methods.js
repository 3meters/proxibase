...
exports.notify = function (notification) {
  ...
  function checkForNearbyNotifications() {
    ...
    db.devices.find(query, { registrationId: true, _user: true }).toArray(function (err, devices) {
      ...
      checkForWatchNotifications()
    })
  }

  function checkForWatchNotifications() {
    log('Checking for watch notifications')

    /* Find the parent entity */
    var fields = { _id: true, name: true, _owner: true, type: true }
    var entityId = notification.toId ? notification.toId : notification.entity._id
    var entityIdParsed = util.parseId(entityId)
    var entityCollection = entityIdParsed.collectionName

    db[entityCollection].findOne({ _id: entityId }, fields, function (err, doc) {
      if (err || !doc) {
        logErr('Find failed in notify: ', err.stack || err)
        return
      }
      toEntity = doc
      db.links.find({ _to: entityId, fromCollectionId: '0001', type: 'watch' }).toArray(function (err, links) {
        if (err) {
          logErr('Find failed in notify: ', err.stack || err)
          return
        }
  ...
  function finishNotification() {
     ...
     * Title and message
     */
    if (notification.entity.schema === util.statics.schemaComment) {
      notification.title = notification.entity.name
      notification.message = notification.entity.description
    }
    else {
    ...
    /*
     * Include a compact version of the entity with the notification.
     */
    ...
    if (registrationIds.length > 0) {
      ...
      notification.subtitle = 'Added a ' +  notification.entity.schema
      if (toEntity && toEntity.name) {
        notification.subtitle += ' to "' + toEntity.name + '"'
      }
    ...
    if (registrationIds.length > 0) {
      ...
      notification.subtitle = 'Added a new ' + notification.entity.schema + ' near you'
      if (notification.entity.name) notification.subtitle += ' called "' + notification.entity.name + '"'
      sendNotifications(notification, registrationIds)
    }
  }
  ...
  function supportedNotification(notification) {
    ...
    if (notification.entity.schema === util.statics.schemaApplink && notification.action === 'insert') return true
...
var sendNotifications = exports.sendNotifications = function (notification, registrationIds) {
  ...
  log('Notification: ' + notification.subtitle)
  log(notification)
...
var propogateActivityDate = module.exports.propogateActivityDate = function (entityId, activityDate) {
   ...
   */
  db.links.find({ _from:entityId, strong:true }).toArray(function (err, links) {
    if (err) {
    ...
    for (var i = links.length; i--;) {
      var toIdParsed = util.parseId(links[i]._to)
      db.collection(toIdParsed.collectionName).findOne({ _id: links[i]._to }, function (err, doc) {
        if (err) {
          util.logErr('Find failed in propogateActivityDate', err)
          return
        }
