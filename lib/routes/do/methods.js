...
exports.notify = function (notification) {
  ...
  var _notification = {
    ...
    beaconIds:    { type: 'array' },                  // in: beacons near action user or destination if some
  ...
  function checkForEntityWatchNotifications() {
    ...
    if (!notification.toId) {
      ...
      watchEntity(notification.toId, function(err, entity) {
        if (err) return logErr('watchEntity failed in notify: ', err.stack || err)
        toEntity = entity
        if (notification.action != 'move') {
          ...
          watchEntity(notification.entity._id, function(err, entity) {
            if (err) return logErr('watchEntity failed in notify: ', err.stack || err)
            if (!notification.fromId) {            
              checkForUserWatchNotifications()
            }
            else {
              watchEntity(notification.fromId, function(err, entity) {
                if (err) return logErr('watchEntity failed in notify: ', err.stack || err)
                fromEntity = entity
                checkForUserWatchNotifications()
              })        
  ...
  function watchEntity(entityId, callback) {
    ...
    util.db[entityCollection].findOne({ _id: entityId }, fields, function (err, doc) {
      ...
      util.db.links.find({ _to: entityId, fromCollectionId: 'us', type: 'watch' }).toArray(function (err, links) {
        ...
        if (ids.length === 0) return callback(null, entity)
 ...
 */
var propogateActivityDate = module.exports.propogateActivityDate = function (entityId, activityDate, updateSelf) {
  ...
  if (!updateSelf) {
    ...
    util.db.collection(entityIdParsed.collectionName).findOne({ _id: entityId }, function(err, doc) {
      if (err || !doc) {
        util.logErr('propogateActivityDate: Find failed in updateSelf', err)
        return
      }
      doc.activityDate = activityDate
      util.db.collection(entityIdParsed.collectionName).update({ _id: doc._id }, doc, function(err) {
        if (err) {
          util.logErr('propogateActivityDate: Update failed in updateSelf', err)
          return
        }
        log('updated self activityDate for ' + entityIdParsed.collectionName + ': ' + doc._id)
        propogate()
      })
    })
  }
  ...
  function propogate() {
    ...
    var query = {
      _from: entityId, 
      type: { $nin: ['like', 'create', 'watch']},
      inactive: false,
    }
    util.db.links.find(query).toArray(function (err, links) {
      if (err) {
        util.logErr('propogateActivityDate: Find failed in propogate', err)
        return
      }
        ...
        util.db.collection(cName).findOne({ _id: links[i]._to }, function (err, doc) {
          ...
          if (doc) {
            ...
            util.db.collection(cName).update({_id: doc._id}, doc, function(err) {
              if (err) {
                util.logErr('propogateActivityDate: Update failed in propogate', err)
                return
              }
              log('updated activityDate for ' + cName + ': ' + doc._id)
            })
            propogateActivityDate(doc._id, activityDate, false) // recurse
          }
        })
      }
    })
