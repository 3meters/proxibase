...
exports.notify = function (notification) {
  ...
  var _notification = {
    ...
    typeTargetId: { type: 'string' },                 // out: id of the entity targeted by type (entity, toEntity, fromEntity)
  ...
  function checkForEntityCreateNotifications() {
    ...
    if (!notification.toId) {
      ...
      relateUsersToEntity(notification.toId, 'create', 'own_to', function(err, entity) {
        ...
        toEntity = entity
        if (notification.action === 'move' || notification.action === 'expand') {
  ...
  function finishNotification() {
    ...
    var types = ['nearby', 'own', 'own_to', 'watch', 'watch_to', 'watch_user']
    types.forEach(function(type) {
      var registrationIds = []
      for (var key in registrationMap) {
        if (registrationMap[key] === type) {
  ...
  function supportedNotification(notification) {
    ...
    if (notification.entity.schema === util.statics.schemaCandigram && notification.action === 'expand') return true
    return false
  }

  function relateUsersToEntity(entityId, relationship, type, callback) {
    ...
    util.db[entityCollection].findOne({ _id: entityId }, fields, function (err, doc) {
      ...
      util.db.links.find({ _to: entityId, fromCollectionId: 'us', type: relationship }).toArray(function (err, links) {
        ...
        util.db.installs.find({ _user: {$in: ids }}, { registrationId: true }).toArray(function (err, installs) {
           ...
           */
          for (var i = installs.length; i--;) {
            registrationMap[installs[i].registrationId] = type
          }
...
var sendNotifications = exports.sendNotifications = function (notification, registrationIds) {
  log('Sending ' + notification.type + ' notifications to ' + registrationIds.length + ' installs(s)')
...
var propagateActivityDate
  ...
  if (!updateSelf) {
    ...
    util.db.collection(entityIdParsed.collectionName).findOne({ _id: entityId }, function(err, doc) {
      ...
      util.db.collection(entityIdParsed.collectionName).safeUpdate({ _id: doc._id }, doc, function(err) {
        if (err) {
