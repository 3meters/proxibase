...
exports.sendMessage = function (message, cb) {
  ...
  var _message = {
    ...
    triggers:         { type: 'array', default: ['nearby','own_to','watch_to'] },
                                                          // in: triggers allowed
  ...
  if (err) {
    logErr('Invalid call to sendMessage: ', err)
    return done(err)
  }
  ...
  function checkForNearbyMessages() {
    ...
    if (message.triggers.indexOf('nearby') === -1 || !message.beaconIds) {
      ...
      util.db.installs.find(query, { registrationId: true, _user: true }).toArray(function (err, installs) {
        if (err) {
          logErr('Find failed in sendMessage: ', err)
          return done(err)
        }
  ...
  function checkForEntityWatchMessages() {
    ...
    if (message.triggers.indexOf('watch_to') === -1 || !message.toEntity) {
      ...
      relateUsersToEntity(message.toEntity._id, 'watch', 'watch_to', function(err) {
        if (err) {
          logErr('relateUsersToEntity failed in sendMessage: ', err)
          return done(err)
        }
  ...
  function checkForEntityCreateMessages() {
    ...
    if (message.triggers.indexOf('own_to') === -1 || !message.toEntity) {
      finishMessage()
    }
    else {
      ...
      findInstalls(userIds, 'own_to', function(err) {
        if (err) {
          logErr('findInstalls failed in sendMessage: ', err)
          return done(err)
        }
        finishMessage()
      })
  ...
  /*
   * Assemble notification messages.
   */
  function finishMessage() {
    ...
    var triggers = ['nearby', 'own_to', 'watch_to']
  ...
  function supportedMessage(message) {
     ...
     */
    if (message.entity.schema === statics.schemaPlace
      && message.event.indexOf('insert') === 0)
      return true
    if (message.entity.schema === statics.schemaMessage
      && message.event.indexOf('insert') === 0) {
        if (message.entity.type === statics.typeShare
            && message.toEntity
            && message.toEntity.schema === statics.schemaPlace) {
          return false
        }
        return true
    }
