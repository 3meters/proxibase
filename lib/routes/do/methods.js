...
exports.notify = function (notification) {
  ...
  function checkForEntityCreateNotifications() {
    ...
    if (!notification.toId) {
      ...
      relateUserToEntity(notification.toId, 'create', function(err, entity) {
        ...
        if (notification.action != 'move') {
          ...
          relateUserToEntity(notification.entity._id, 'create', function(err, entity) {
            if (err) return logErr('relateUserToEntity failed in notify: ', err.stack || err)
            if (!notification.fromId) {
              checkForEntityWatchNotifications()
            }
            else {
              relateUserToEntity(notification.fromId, 'create', function(err, entity) {
                if (err) return logErr('relateUserToEntity failed in notify: ', err.stack || err)
                fromEntity = entity
                checkForEntityWatchNotifications()
              })
  ...
  function checkForEntityWatchNotifications() {
    ...
    if (!notification.toId) {
      ...
      /* Handle watch for the parent entity */
      relateUserToEntity(notification.toId, 'watch', function(err, entity) {
        if (err) return logErr('relateUserToEntity failed in notify: ', err.stack || err)
        toEntity = entity
        if (notification.action != 'move') {
          checkForUserWatchNotifications()
          ...
          /* Handle watch for the moved entity */
          relateUserToEntity(notification.entity._id, 'watch', function(err, entity) {
            if (err) return logErr('relateUserToEntity failed in notify: ', err.stack || err)
            if (!notification.fromId) {
              checkForUserWatchNotifications()
            }
            else {
              relateUserToEntity(notification.fromId, 'watch', function(err, entity) {
                if (err) return logErr('relateUserToEntity failed in notify: ', err.stack || err)
                fromEntity = entity
                checkForUserWatchNotifications()
              })
  ...
  function relateUserToEntity(entityId, relationship, callback) {
    ...
    util.db[entityCollection].findOne({ _id: entityId }, fields, function (err, doc) {
      ...
      entity = doc
      util.db.links.find({ _to: entityId, fromSchema: 'users', type: relationship }).toArray(function (err, links) {
        if (err) callback(err, null)

        var ids = []
        for (var i = links.length; i--;) {
          if (!notification.user || links[i]._from !== notification.user._id) { // Skip so we don't self notify.
            ids.push(links[i]._from)
          }
        }
 ...
 */
var propagateActivityDate
    = module.exports.propagateActivityDate
    = function(entityId, activityDate, updateSelf, verbose) {
