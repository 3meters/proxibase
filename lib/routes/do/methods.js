...
exports.notify = function (notification, cb) {
  ...
  log('notification before processing', notification)
  ...
  function addNotificationEntities() {
    ...
    function process(entityId, next) {
      ...
      util.db[entityCollection].findOne(query, function(err, entity) {
        ...
        if (notification.toId === entity._id) notification.toEntity = entity
        if (notification.fromId === entity._id) notification.fromEntity = entity
        next()
      })
    }
    ...
    function finish(err) {
      ...
      log('notification after adding entities', notification)
  ...
  function finishNotification() {
    ...
    if (tipe.isObject(notification.entity)) {
     ...
     * the property names because this isn't coming from storage.
     */
    if (tipe.isObject(notification.user)) {
     ...
     */
    if (tipe.isObject(notification.toEntity)) {
      activity.action.toEntity = {
        id: notification.toEntity._id,
        name: notification.toEntity.name,
        photo: notification.toEntity.photo,
        category: notification.toEntity.category,
        schema: notification.toEntity.schema,
      }
    }

    if (tipe.isObject(notification.fromEntity)) {
      activity.action.fromEntity = {
        id: notification.fromEntity._id,
        name: notification.fromEntity.name,
        photo: notification.fromEntity.photo,
        category: notification.fromEntity.category,
        schema: notification.fromEntity.schema,
      }
    }
    /*
    ...
    if (!(activity.action
        ...
        && activity.action.user
        && activity.action.entity)) {
      logErr('Notification is incomplete, send canceled: ', activity)
      return done()
    }
    ...
    triggers.forEach(function(trigger) {
      ...
      if (registrationIds.length > 0) {
        ...
        notifications.push(activity)
        sendNotifications(activity, registrationIds)
      }
    })
