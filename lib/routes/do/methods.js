...
module.exports.getEntities = function(entityIds, eagerLoad, beaconIds, observation, fields, options, more, req, res) {
  req.state = {}
  req.parts = {}
  req.state.entityIds = entityIds
  req.state.beaconIds = beaconIds
  req.state.eagerLoad = eagerLoad
  req.state.observation = observation
...
function addChildren(req, res) {
   ...
   */
  var query = { toTableId:2, fromTableId:2, _to:{ $in:req.parts.entityIds } }
...
function addParents(req, res) {
   ...
   */
  var query = { toTableId:2, fromTableId:2, _from:{ $in:req.parts.entityIds } }
  db.links.find(query).toArray(function(err, links) {
    if (err) return res.error(err)
    if (links.length == 0) {
      addBeaconLinks(req, res)
    }
    else {
...
function addBeaconLinks(req, res) {
  var query = null
  if (req.state.beaconIds) {
    query = {toTableId:3, _from:{$in:req.parts.entityIds}, _to:{$in:req.state.beaconIds}}
  }
  else {
    query = {toTableId:3, _from:{$in:req.parts.entityIds}}
  }

  db.links.find(query).toArray(function(err, links) {
    if (err) return res.error(err)
    req.parts.beaconLinks = links
    req.parts.beaconIds = []
    req.parts.linkIds = []
    for (var i = links.length; i--;) {
...
function addLinkStats(req, res) {
  sreq({ 
    ...
    body: {collection:'actions', countBy:'_target', find:{ _target:{ $in:req.parts.linkIds }, type:{ $in:['tune', 'tune_synthetic_first', 'tune_custom_first'] }}
    }}, 
    function(err, sres, body) {
      if (err) return res.error(err)

      req.parts.stats = body.data;
      addUsers(req, res)  
  })
}
...
function buildPayload(req, res) {
  ...
  function getBeaconLinks(entityId, req) {
    var beaconLinks = []
    req.parts.beaconLinks.forEach(function(beaconLink) {
      if (beaconLink._from === entityId) {
        /* Get tuning count */
        beaconLink.tuneCount = 0
        /*  commented out temporarily
        for (var i = 0; i < req.parts.stats.length; i++) {
          if (req.parts.stats[i]._target == beaconLink._id) {
            beaconLink.tuneCount = req.parts.stats[i].countBy
            break;
          }
        }
        */
        beaconLinks.push({beaconId:beaconLink._to, linkId:beaconLink._id, primary:beaconLink.primary, tuneCount:beaconLink.tuneCount})
      }
    })

    for (var i = 0; i < beaconLinks.length; i++) {
      for (var j = 0; j < req.parts.beacons.length; j++) {
        if (req.parts.beacons[j]._id == beaconLinks[i].beaconId) {
          beaconLinks[i].latitude = req.parts.beacons[j].latitude
          beaconLinks[i].longitude = req.parts.beacons[j].longitude
        }
      }
    }
  ...
  req.parts.entities.forEach(function(entity) {
    ...
    if (_.isEmpty(req.state.fields.entities) || req.state.fields.entities['_beacon'] == true) {
      var beaconLinks = getBeaconLinks(entity._id, req)
      if (beaconLinks !== null) {
        entity.beaconLinks = beaconLinks
      }
    }
...
var logAction = module.exports.logAction = function logAction(target, targetSource, type, req) {
  /*
