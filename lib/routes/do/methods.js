...
exports.sendMessage = function (message, cb) {
  ...
  var err = util.check(message, _message)
  ...
  if (message.user && (message.user._id === util.adminUser || message.user._id === util.anonUser)) {
    delete message.user
  }
  ...
  function checkForEntityCreateMessages() {
    ...
    if (!message.toEntity) {
      ...
      relateUsersToEntity(message.toEntity._id, 'create', 'own_to', function(err) {
        ...
        if (message.type === 'move' || message.type === 'expand') {
  ...
  function checkForUserWatchMessages() {
    ...
    if (!message.user) {
      ...
      util.db.links.find({ _to: message.user._id, fromSchema: 'users', type: 'watch' }).toArray(function (err, links) {
        if (err) {
          logErr('Find failed in sendMessage: ', err.stack || err)
          return done(err)
        }
  ...
  function finishMessage() {
    ...
    if (!(message_out.action
        ...
        && message_out.action.user
        && message_out.action.entity)) {
      logErr('message is incomplete, send canceled: ', message_out)
      return done()
    }

    if (cb && Object.keys(registrationMap).length === 0) {
      log('message constructed but no registered install with valid triggers')
    ...
    triggers.forEach(function(trigger) {
      ...
      if (registrationIds.length > 0) {
        message_out.trigger = trigger
        messages.push(message_out)
        sendmessages(message_out, registrationIds)
      }
    })
  ...
  function supportedMessage(message) {
    if (message.entity.schema === util.statics.schemaPlace && message.type === 'insert') return true
    if (message.entity.schema === util.statics.schemaPost && message.type === 'insert') return true
    if (message.entity.schema === util.statics.schemaComment && message.type === 'insert') return true
    if (message.entity.schema === util.statics.schemaCandigram && message.type === 'insert') return true
    if (message.entity.schema === util.statics.schemaCandigram && message.type === 'move') return true
    if (message.entity.schema === util.statics.schemaCandigram && message.type === 'expand') return true
    return false
  }
...
var sendmessages = exports.sendmessages = function (message, registrationIds) {
  ...
  gcmMessage.addData('activity', JSON.stringify(message))
