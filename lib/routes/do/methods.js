...
exports.sendMessage = function (message, cb) {
  ...
  function addMessageEntities() {
    ...
    async.forEach(entityIds, processEntity, finish)
  ...
  function checkForEntityCreateMessages() {
    ...
    if (!message.toEntity) {
      ...
      relateUsersToEntity(message.toEntity._id, 'create', 'own_to', function(err) {
        ...
        /* Handle creator of the moved/forwarded entity */
        if (message.event.indexOf('move') === 0 || message.event.indexOf('forward') === 0) {
  ...
  function checkForEntityWatchMessages() {
    ...
    if (!message.toEntity) {
      ...
      relateUsersToEntity(message.toEntity._id, 'watch', 'watch_to', function(err) {
        ...
        /* Handle watch for the moved/forwarded entity */
        if (message.event.indexOf('move') === 0 || message.event.indexOf('forward') === 0) {
  ...
  function finishMessage() {
    ...
    if (tipe.isObject(message.fromEntity)) {
      message_out.action.fromEntity = {
        ...
        type: message.fromEntity.type,
        placeId: message.fromEntity._place,
      }
    }
    /*
  ...
  function supportedMessage(message) {
    ...
    if (message.entity.schema === statics.schemaMessage && message.event.indexOf('insert') === 0) return true
    if (message.entity.schema === statics.schemaCandigram && message.event.indexOf('insert') === 0) return true
    if (message.entity.schema === statics.schemaCandigram && message.event.indexOf('move') === 0) return true
    if (message.entity.schema === statics.schemaCandigram && message.event.indexOf('forward') === 0) return true
    return false
  }
...
var sendmessages = exports.sendmessages = function (message, registrationIds) {
  ...
  sender.send(gcmMessage, registrationIds, sendRetries, function (err, result) {
    ...
    if (result && result.failure > 0) {
      ...
      async.forEach(notRegisteredIds, processId, finish)

      processId = function(notRegisteredId, next) {
