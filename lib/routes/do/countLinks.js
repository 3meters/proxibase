...
exports.run = function run(req, cb) {
  ...
  // $group
  var group = {
    _id: (clName === 'tos') ? '$_to' : '$_from',
    count: {$sum: '$count'},
  }
  if ('tos' == clName) group.toSchema = {$first: '$toSchema'}
  else group.fromSchema = {$first: '$fromSchema'}
  ops.push({$group: group})

  // $sort, $limit, $skip
  var sort = req.body.sort || {count: -1}
  ops.push({$sort: sort})
  ops.push({$limit: req.body.limit})
  ops.push({$skip: req.body.skip})

  // map the id back
  var project = {count: '$count'}
  if ('tos' === clName) {
    project._to = '$_id'
