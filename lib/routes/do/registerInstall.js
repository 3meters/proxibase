...
module.exports.main = function(req, res) {
  ...
  if (req.body.register) {
    db.installs.findOne({ installationId: req.body.install.installationId }, function(err, doc) {
      if (err) return res.error(err)
      if (doc) {
        /* update */
        db.installs.safeUpdate(req.body.install, dbuser, function(err, updatedDoc) {
          if (err) return res.error(err)
        ...
        /* insert */
        db.installs.safeInsert(req.body.install, dbuser, function(err, insertedDoc) {
          if (err) return res.error(err)
