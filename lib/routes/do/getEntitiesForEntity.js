...
exports.main = function(req, res) {
  ...
  function doEntitiesForEntity() {
    ...
    db[clName].safeFindOne({_id: req.body.entityId}, dbOps, function(err, ent) {
      if (err) return res.error(err)
      if (!ent) return res.error(perr.notFound(req.body.entityId))
      ...
      if (clName === 'places' && ent.visibility === 'public') {
        dbOps.asReader = true
        return getChildren()
      }
      ...
      db[clName].userIsWatching(dbOps.user._id, ent, function(err, isWatching) {
        ...
        if (isWatching) {
          ...
          if (clName !== 'places') getChildren()
          // The entity is a place, the place is private, the user is logged in, she does not
          // own the place, and is not watching it.  In this corner case skip getting
          // the children to fix (or hack) https://github.com/3meters/proxibase/issues/274:
          // Don't display messages she created for a place when she formerly was a member.
          else {
