...
exports.main = function(req, res) {
  ...
  function doEntitiesForEntity() {
    ...
    db[clName].safeFindOne(query, dbOps, function(err, ent, newOps) {
      if (err) return res.error(err)
      if (!ent) return finish()
      entity = ent

      // If the user has read permissions for the _acl of the entity we are done
      if (newOps._acl) dbOps._acl = newOps._acl
  ...
  function doGetEntities(more) {
    ...
    getEntities(options, dbOps, function(err, entities) {
      if (err) return res.error(err)
