...
exports.main = function(req, res) {
  ...
  function doEntitiesForEntity() {
    ...
    if (!entId.collectionName) return res.error(perr.badValue(req.body.entityId))

    if ('places' !== entId.collectionName) return getChildren()

    db.places.safeFindOne({_id: req.body.entityId}, req.dbOps, function(err, place) {
      if (err) return res.error(err)
      if (!place) return res.error(perr.notFound(req.body.entityId))

      req.place = place

      // The asReader safeFind option tells schemas/_base#ownerAccess to allow reads
      // of ownerAccess collections.  It can be set if the parent place is public, (aka
      // not restricted), or if the user has an enabled watch link to the place.
      if (place.restricted) {

        if (!req.dbOps.user) return finish()  // no results
        ...
        db.links.safeFindOne(linkQry, req.dbOps, function(err, watchLink) {
          ...
          if (watchLink) req.dbOps.asReader = true
          getChildren()
        })
      }
      else {
