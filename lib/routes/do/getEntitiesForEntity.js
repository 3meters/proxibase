...
exports.main = function(req, res) {
  ...
  entityIdParsed.collectionName === 'users' ? doEntitiesForUser() : doEntitiesForEntity()

  function doEntitiesForUser() {
    log('doEntitiesForUser')

    var collectionNames = []
    req.body.entityTypes.forEach(function(entityType) {
      collectionNames.push(entityType + 's')
    })
  ...
  function doEntitiesForEntity() {
    ...
    var query = { _to: req.body.entityId, type: { $in: req.body.entityTypes }}
    if (req.body.cursor.where) {
      query = { $and: [query, req.body.cursor.where] }
    }
    db.links
  ...
  function decorate(entities, collectionName) {
    entities.forEach(function(entity) {
      if (entity.type) entity.subtype = entity.type
      entity.type = util.statics.collectionTypeMap[collectionName]
    })
  }
}
