...
exports.main = function(req, res) {
  ...
  function doEntitiesForEntity() {
    ...
    if ('places' !== entId.collectionName) return getChildren(dbOps)
    ...
    db.places.safeFindOne({_id: req.body.entityId}, req.dbOps, function(err, place) {
      ...
      if ('public' === place.visibility) dbOps.asAdmin = true  // the magic key

      getChildren(dbOps)
    })
  }

  function getChildren(dbOps) {
    ...
    dbOps.limit = req.body.cursor.limit
    dbOps.sort = req.body.cursor.sort
    dbOps.skip = req.body.cursor.skip
    db.links.safeFind(query, dbOps, function(err, links, meta) {
      ...
      doGetEntities(dbOps, more)
    })
  }

  function doGetEntities(dbOps, more) {
    ...
    if (entityIds.length === 0) {
      ...
      options = util.extend(options, dbOps)
