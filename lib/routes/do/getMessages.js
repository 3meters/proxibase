...
var run = exports.run = function(req, options, cb) {
  ...
  function findLinks() {
    ...
    if (_.contains(req.body.cursor.linkTypes, statics.typeShare)) {
      query = {
        $or: [
          { _from: options.entityId,
            toSchema: { $in: req.body.cursor.schemas },
          ...
          { _to: options.entityId,
            fromSchema: statics.schemas.message.name,
            type: statics.typeShare,},]
      }
    }

    db.links.find(query, { type: true, _to: true, _from: true }).toArray(function(err, links) {
      if (err) return done(err)

      links.forEach(function(link) {
        if (link._to == options.entityId) {
          linkToMap[link._from] = link._from
        }
        else {
      ...
      log('Total seed entities found using links: ' + activityIds.length)
      getActivityActions()
    })
  }
  ...
  function getActivityActions() {
    ...
    query = { $and: [query, { event: { $in: options.events }}, { _user: { $ne: options.entityId }}] }
    ...
    var pipe = [
      ...
      { $group: {
        ...
        actions: { $push: { event: "$event", _user: "$_user", _toEntity: "$_toEntity", modifiedDate: "$modifiedDate" }},
        count: { $sum: 1 }}},
