  ...
  function doGetEntities() {
    ...
    if (entityIds.length === 0) {
      ...
      getEntities(req, options, function(err, entities) {
        if (err) return done(err)
        entities.forEach(transformEntity)
        var cUsers = 0; var cMessages = 0
        function transformEntity(ent) {
          if (ent.schema === 'message' && !cMessages) {
            cMessages++
            debug('getMessage message', ent)
          }
          if (ent.schema === 'user' && !cUsers) {
            cUsers++
            debug('getMessage user', ent)
          }
        }
