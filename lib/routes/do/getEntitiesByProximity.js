...
exports.main = function(req, res) {
   ...
   * If an installationId is passed, we assume the entity ids are for beacon entities.
   */
  function updateInstall() {
    if (!req.body.installationId) {
      ...
      db.installs.findOne({ installationId: req.body.installationId }, function(err, doc) {
        if (err) return res.error(err)
        /*
         * An unregistered/invalid installationId isn't great but shouldn't prevent
  ...
  function doGetEntities() {
    ...
    if (entityIds.length == 0) {
      ...
      getEntities(req, options, function(err, entities, extras) {
          if (err) return res.error(err)
          res.send({
            data: entities,
            date: util.getTimeUTC(),
