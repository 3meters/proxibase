...
exports.main = function(req, res) {
  ...
  var _body = {
    ...
    cursor:         { type: 'object', value: cursor.fields, default: { sort: { modifiedDate: -1 }, skip: 0, limit: statics.optionsLimitDefault }},
  ...
  function updateInstall() {
    if (!req.body.installId) {
      ...
      db.installs.findOne({ installId: req.body.installId }, function(err, doc) {
        ...
        // Check valid install / user combination
        if (doc.users.indexOf(req.user._id) < 0) return doEntitiesByProximity()
