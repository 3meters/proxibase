...
exports.main = function(request, response) {
  ...
  if (req.body.beaconLevels && req.body.beaconIdsNew) {
    if (req.body.beaconLevels.length != req.body.beaconIdsNew.length) {
      return res.sendErr(new Error("The size of beaconLevels[] and beaconIdsNew[] must match"))
    }
  }
...
function getEntitiesTracked() {
  ...
  var query = { _id:{ $in:entityIdsTracked}, enabled:true, $or:[{visibility:'public'}, {visibility:'private', _creator:req.user._id }]}
  if (req.body.filter) {
    query = { _id:{ $in:entityIdsTracked}, enabled:true, $or:[{visibility:'public'}, {visibility:'private', _creator:req.user._id }], namelc: { $regex: '^' + req.body.filter, $options: 'i'}}
  }
