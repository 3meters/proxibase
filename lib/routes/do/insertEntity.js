...
module.exports.main = function(req, res) {
  ...
  function doInsertEntity(err) {
    ...
    db[collectionName].safeInsert(req.body.entity, req.dbOps, function(err, savedDoc) {
      ...
      /* If there are no links we still want an action created */
      if (!req.body.links) {
        var action = {
        ...
        action.event = eventKeyBuilder(req.savedEntity)
        actions.push(action)
      }
  ...
  function insertLinks(err) {
    ...
    function insert(link, next) {
      ...
      db.links.safeInsert(link, req.dbOps, function(err) {
        ...
        if (req.savedEntity._place) {
          action._place = req.savedEntity._place  // used to find actions associated with a place but not linked to it (i.e. message to message)
        }
        action.event = eventKeyBuilder(req.savedEntity, link)
        actions.push(action)
  ...
  function sendMessage() {
    ...
    if (!req.body.links) {
      ...
      var message = {
        event: actions[0].event, // If no links then first item is the one
    ...
    function processLink(link, next) {
      var message = {
        event: eventKeyBuilder(req.savedEntity, link),
        entity: req.savedEntity,
        toId: link._to,
        user: req.user,
      }
  ...
  function eventKeyBuilder(entity, link) {

    var eventKey = 'insert_entity' + '_' + entity.schema

    /* Fill in the connection if one */
    if (link) {
      var toId = util.parseId(link._to)
      eventKey += '_to_' + toId.schemaName
    }

    /* System is default owner for place entities except custom ones */
    if (entity.schema === statics.schemaPlace) {
      if (db.places.isCustom(entity)) {
        eventKey += '_custom'
      }
      else {
        eventKey += '_linked'
