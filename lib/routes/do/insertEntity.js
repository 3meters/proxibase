...
module.exports.main = function(req, res) {
  ...
  var link = {
    fields: {
      ...
      type:               { type: 'string', },  // george: I don't think link type is required anymore
      strong:             { type: 'boolean' },
  ...
  function doInsertEntity() {
    ...
    var collectionName = req.body.entity.schema + 's'  // TODO: bogus, look up from reverse map
    if (!db[collectionName]) {
      return res.error(perr.badValue('Unknown entity schema: ', req.body.entity.schema))
    }

    /* System is default owner for place entities except custom ones */
    var options = {user: req.user}
    if (entity.schema === util.statics.typePlace) {
      if (req.user && entity.provider && entity.provider.user === req.user._id) {
        actionType += '_custom'
      }
      else {
        actionType += '_linked'
  ...
  /*
   * Optionally augment the applinks array before saving the entity
   * req.body params:
   *   @suggestApplinks: boolean, default false
  ...
  function insertApplinks() {
    var entity = req.insertedEntity
    if (!(req.body.suggestApplinks && util.statics.typePlace === entity.schema)) {
    ...
    suggest(options, function(err, applinks, raw) {
      ...
      function addApplink(applink, next) {
        db.applink.safeInsert(applink, req.dbOps, function(err, savedApplink) {
          if (err) return next(err)
          var link = {
            fromId: entity._id,
            toId: savedApplink._id,
          }
      ...
      function finish(err) {
        if (err) logErr('Error saving applinks for req ' + req.tag +
  ...
  function getEntity() {
    ...
    var options = { entityIds: [req.insertedEntity._id] }
  ...
  function notify() {
    var notification = {
      ...
      subjectType: req.body.entity.schema,
      action: 'insert',
      user: req.user,
    }
