...
function sendNotifications(req, res) {
  if (skipNotifications) {
    log('** skipping sendMessage code')
    done(req, res)
  }
  else {
    log('** running sendMessage code')
    ...
    if (!req.body.parentId){
      ...
      db.entities.findOne({ _id:req.body.parentId }, { _id: true, name: true, _owner: true }, function(err, doc) {
        ...
        /* Skip message if parent owner is same as request user */
        if (doc._owner !== req.user._id) {
          ...
          var notification = {
            ...
            title: req.user.name,
            subtitle: subtitle,
            entity: { _id: req.insertedEntity._id, name: req.insertedEntity.name, _owner: req.insertedEntity._owner },
            user: req.user
          }
          ...
          db.devices.find({ _user: doc._owner }, { registrationId: true }).toArray(function(err, devices) {
            ...
            log('Sending message to device(s) of owner: ' + doc._owner)
            var registrationIds = []
