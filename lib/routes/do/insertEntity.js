...
function doInsertEntity(req, res) {
  ...
  if (doc.type === util.statics.typeContent) actionType += '_content'
  if (doc.type === util.statics.typeComment) actionType += '_comment'
  if (doc.type === util.statics.typeSource) actionType += '_source'

  /* System is default owner for place entities except custom ones */
  var options = {user: req.user}
  if (doc.place) {
    if (doc.place.provider && doc.place.provider.user === req.user._id) {
      actionType += '_custom'
    }
    else {
      actionType += '_linked'
