...
module.exports.main = function(req, res) {
  ...
  function getEntity() {
    ...
    getEntities(req, options, function(err, entities) {
      ...
      beaconsForNotification()
    })
  }

  function beaconsForNotification() {
    log('beaconsForNotification')
  ...
  function sendNotification() {
    ...
    if (!req.body.links) {
       ...
       * viable trigger == nearby. Don't process notification if
       * no beacons available. OK to use nearby even if the patch
       * is not public.
       */
      var options = {
        event:      'insert_entity' + '_' + req.savedEntity.schema,
    ...
    function processLink(link, next) {
      ...
      var toId = util.parseId(link._to)
      if (toId.schemaName === statics.schemaPatch) {
        options.beaconIds = beaconIds  // For just inserted patch or patch we are linking to
      }
