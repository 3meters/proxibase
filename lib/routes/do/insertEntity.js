...
module.exports.main = function(req, res) {
  ...
  function doInsertEntity() {
    ...
    if (util.statics.schemaPlace === entity.schema
        && (entity.phone || entity.provider)) {
      var searchFields = []
      if (entity.phone) {
        searchFields.push({phone: entity.phone})
      }
      for (var p in entity.provider) {
        switch (p) {
          case 'factual':
            searchFields.push({'provider.factual': entity.provider[p]})
            break
          case 'foursquare':
            searchFields.push({'provider.foursquare': entity.provider[p]})
            break
          case 'google':
            searchFields.push({'provider.google': entity.provider[p]})
            break
        }
      }
      if (!searchFields.length) {
        return db.places.safeInsert(entity, options, finishDoInsert)
      }
      db.places.findOne({$or: searchFields}, function(err, foundPlace) {
        log('debug duplicate place entity:', foundPlace)
        if (err) return res.error(err)
        if (!foundPlace) {
          return db.places.safeInsert(entity, options, finishDoInsert)
        }
        else {
          // Update the the existing place with a bare minimum of fields
          // from the duplicate place the user tried to insert.  To
          // update other fields on the place, use update.
          foundPlace.phone = foundPlace.phone || entity.phone || undefined
          foundPlace.provider = foundPlace.provider || {}
          _.extend(foundPlace.provider, entity.provider)
          foundPlace._modifier = req.user._id || undefined
          delete foundPlace.modifiedDate
          return db.places.safeUpdate(foundPlace, {user: util.adminUser}, finishDoInsert)
        }
      })
    }
    else {
      return db[collectionName].safeInsert(entity, options, finishDoInsert)
    }

    function finishDoInsert(err, savedDoc) {
      if (err) {
  ...
  function updateActivityDate() {
    if (!req.body.skipActivityDate) {
      /* Fire and forget */
      methods.propogateActivityDate(req.savedEntity._id, activityDate, false)
    }
    getEntity()
  }
