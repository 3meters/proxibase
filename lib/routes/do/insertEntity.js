...
var notifyUser = require('../user/notify')
...
module.exports.main = function(req, res) {
    ...
    var options = {
      ...
      links: {
        ...
        active: [{
          ...
          schema: statics.schemaMessage,
          links: true,
          direction: 'in'
        }]
      }
    }
    getEntities(options, req.dbOps, function(err, entities) {
      if (err) return res.error(err)
  ...
  // This only works for an android client creating a patch with an array of
  // wifi beacons.  All other notifications are handled by a trigger on links.
  function sendNotification() {
    if (req.body.log) {
      log('sendNotification')
      log('locations', locations)
      log('beaconIds', beaconIds)
    }
    /*
    ...
    if (!req.body.test) {
      ...
      notifyUser(options, function(err, _notifications) {
