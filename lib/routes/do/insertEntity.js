...
module.exports.main = function(req, res) {
  ...
  function insertApplinks() {
    ...
    suggest(options, function(err, suggestPlace, raw) {
      if (err) logErr(err)
      log('debug suggestPlace', suggestPlace)
      ...
      async.each(suggestPlace.applinks, addApplink, finish)

      function addApplink(applink, next) {
        log('debug applink', applink)
        db.applinks.safeInsert(applink, req.dbOps, function(err, savedApplink) {
          ...
          log('debug saved Applink', savedApplink)
          var link = {
            _from: entity._id,
            _to: savedApplink._id,
            type: util.statics.schemaApplink, // TODO:  this is make up for a problem in get entties.  
            strong: true // delete applink if place is deleted
          }
          db.links.safeInsert(link, req.dbOps, function(err, savedLink) {
            if (err) return next(err)
            else log('debug saved link between place and applink', savedLink)
            next()
          })
        })
      }
      ...
      function finish(err) {
        ...
        delete suggestPlace.applinks
        if (_.isEqual(entity, suggestPlace)) {
          return insertBeacons()
        }
        else {
          log('Insert Applinks modified place. Old:', entity)
          log('New:', suggestPlace)
          db.places.safeUpdate(entity, suggestPlace, req.dbOps, function(err, updatedPlace) {
            if (err) return cb(err)
            req.savedEntity = updatedPlace
            insertBeacons()
          })
        }
      }
  ...
  function getEntity() {
    ...
    var options = { entityIds: [req.savedEntity._id] }
  ...
  function notify() {
    var notification = { // george:  Jay please check changes.
      subject: 'entity',
      action: 'insert',
