      ...
      suggest(options, function(err, applinks, raw) {
        ...
        function addApplink(applink, next) {
          ...
          db.applinks.safeUpsert(applink, req.dbOps, function(err, savedApplink) {
            ...
            log('saved applink', savedApplink)
            if (applink._id && (applink._id === savedApplink._id)) {  // BUG
              log('not creating links')
              return next() // update, links have already been created
            }
            else {
              // insert, create links between place and applinks
              log('creating link between place and applink')
              log('place', entity)
              log('applink', savedApplink)
              var link = {
              ...
              db.links.safeInsert(link, req.dbOps, function(err, savedLink) {
                ...
                log('debug link between place and applink', savedLink)
                next()
              })
            }
          })
