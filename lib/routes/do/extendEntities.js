...
function doExtendEntities(req, res) {
  ...
  function processBeacon(beacon, next) {
    ...
    function processEntity(entityId, next) {
      db.collection('links').findOne({ _to:beacon._id, _from:entityId }, function(err, doc) {
        if (err) return next(err)
        if (doc) return next()

        var link = {_from:entityId, _to:beacon._id, primary:false, signal:beacon.level}
        if (req.body.primaryBeaconId && req.body.primaryBeaconId == beacon._id) {
          link.primary = true
        }
        var doc = new gdb.models.links(link)
        doc.__user = req.user
        doc.save(function (err, insertLink) {
          if (err) return next(err)
          next()
        })    
      })
