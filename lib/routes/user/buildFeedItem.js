...
module.exports = function(options) {
  ...
  if (options.provider === 'one_signal') {
      ...
      if (options.deviceTarget === 'ios') {
        one_signal_notification.ios_sound = 'chirp.caf'
        one_signal_notification.content_available = true
        var content = notification.summary.replace(new RegExp('<b>|</b>', 'g'), '\"')  // Strips bolding markup because ios doesn't support it
        one_signal_notification.contents = { 'en': content }
        one_signal_notification.ios_badgeType = 'Increase'
        one_signal_notification.ios_badgeCount = 1

        // Mute priority 2 and higher notifications
        if (options.priority > 1) {
          delete one_signal_notification.contents // Allowed because we set content_available
          delete one_signal_notification.ios_sound
        }
      }
      else {
        one_signal_notification.android_visibility = 0
        one_signal_notification.android_group = notification._parent
      }
