...
var spec = {
  ...
  sort:     {type: 'object', value: {modifiedDate: -1}},
...
function run(ops, dbOps, cb) {
  ...
  function getDocs() {
    ...
    var findDocsOps = _.assign(_.cloneDeep(dbOps), {
      asAdmin: true,
      refs: '_id,name,photo,schema',
    })

    db.safeFindByIds(entityIds, findDocsOps, function(err, items) {
      if (err) return fail(err)
      entities = items
      build()
    })
  ...
  function build() {
    ...
    for (var key in feedItemMap) {
      feedItems.push(feedItemMap[key])
    }
