...
function signinAk(req, res, next) {
  ...
  util.callService.accountKit(ops, function(err, akres, akbody) {
    ...
    ops = {
      path: '/me',
      query: {access_token: akbody.access_token},
      log: req.body.log,
    }
    ...
    util.callService.accountKit(ops, function(err, akres, akuser) {
      ...
      err = util.scrub(akuser, {
        id:    {type: 'string', required: true},
        phone: {type: 'object', value: function(v) {return v.number}},
        email: {type: 'object', value: function(v) {return v.address}},
      })
      if (err) return next(perr.badAuth(err))
      ...
      // Testing the email lookup is impossible given current Facebook Account Kit
      // APIs, because their is no way from the server to simulate the clients
      // Initiation of the auth flow, and to put in test email address.
      var userQry = {akid: akid}
      if (akuser.email) userQry = {$or: {akid: akid, email: akuser.email}}

      db.users.safeFind(userQry, {asAdmin: true}, function(err, users) {
        if (err) return next(err)
        if (users && users.length) {
          var user = _.assign(users[0], _.pick(akuser, ['id', 'phone', 'email']))
