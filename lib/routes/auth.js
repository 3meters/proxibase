...
function signinAk(req, res, next) {
  ...
  util.callService.accountKit(ops, function(err, akres, akuser) {
    ...
    // that we trust
    debug('accountKit user query', userQry)
    db.users.safeFind(userQry, {asAdmin: true}, function(err, users) {
      debug('accountKit user query results', users)
      if (err) return next(err)
      ...
      if (users && users.length) {
        ...
        debug('accountKit updating existing user', user)
      ...
      debug('accountKit inserting new provisional user', provUser)
      db.users.safeInsert(provUser, {asAdmin: true}, function(err, savedUser) {
        debug('accountKit saved provisional user', savedUser)
        if (err) return next(err)
