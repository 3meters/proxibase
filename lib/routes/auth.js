...
function signin(req, res, next) {
  ...
  users.authByPassword(signinCred, function(err, foundUser) {
      ...
      modifiedDate: foundUser.modifiedDate,  // preserve old value
    }, {user: foundUser, asAdmin: true, tag: req.dbOps.tag}, function(err, savedUser) {
        if (err) return next(err)
        ...
        upsertSession(req, savedUser, function(err, session) {
          ...
          if (req.body.getEntities) {
            var geReq = {dbOps: {user: _.clone(savedUser), tag: req.dbOps.tag}}
            var geOps = {
...
function signout(req, res, next) {
  ...
  function deleteSession() {
    sessions.findOne({key: req.query.session}, function(err, session) {
      if (err) return next(err)
      sessions.safeRemove({_id: session._id}, {user: req.user, asAdmin: true, tag: req.dbOps.tag}, function(err) {
        if (err) return next(err)
        delete req.user
        return res.redirect('/')
      })
...
function upsertSession(req, user, cb) {
  ...
  sessions.safeFindOne({
  ...
  }, {asAdmin: true, tag: req.dbOps.tag}, function(err, session) {
    ...
    if (!session) {
      ...
      sessions.safeInsert(session, {user: user, asAdmin: true, tag: req.dbOps.tag}, cb)
    }
    else {
      session = {
        _id: session._id,
      ...
      sessions.safeUpdate(session, {user: user, asAdmin: true, tag: req.dbOps.tag}, cb)
    }
  })
}
  ...
  users.findOne({ _id: req.query.user }, function(err, user) {
    ...
    sessions.findOne({_owner: user._id, key: req.query.session}, function(err, session) {
      ...
      if (now > (session.modifiedDate + statics.session.refreshAfter)) {
        ...
        sessions.safeUpdate(session, {user: user, asAdmin: true, tag: req.dbOps.tag}, function(err) { // fire and forget
