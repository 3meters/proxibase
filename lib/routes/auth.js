      ...
      modifiedDate: foundUser.modifiedDate,  // preserve old value
        ...
        upsertSession(req, savedUser, function(err, session) {
          ...
          if (req.body.getEntities) {
            var geReq = {dbOps: {user: _.clone(savedUser)}}
            var geOps = {
...
function validateSession(req, res, next) {
  ...
  users.findOne({ _id: req.query.user }, function(err, user) {
    ...
    sessions.findOne({_owner: user._id, key: req.query.session}, function(err, session) {
      ...
      if (!req.query.install) return finish()

      // If install is provided validate it against the installs colletion for this user
      // If validation succeeds add installId to the requestObject, otherwise ignore it.
      // TODO: consider failing validation if user credentials validate but installId does not.
      db.installs.safeFindOne({_id: 'in.' + req.query.install}, {asAdmin: true}, function(err, install) {
        if (err) return next(err)
        if (!install) return finish()
        if (!install.users.some(function(_user) { return (_user === user._id) })) {
          return finish()
        }
        // TODO: update beacons, location?
        finish(null, install)
      })



      function finish(err, install) {
        if (err) return next(err)
        // Success
        req.user = user
        ...
        // req.session = session
        if (install) req.installId = install.installId
        return next()
      }
    })
  })
