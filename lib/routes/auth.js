    ...
    users.safeUpdate({
      ...
      modifiedDate: foundUser.modifiedDate,  // preserve old value
        ...
        upsertSession(req, savedUser, function(err, session) {
          ...
          updateInstall(savedUser, req.body.install, function(err) {
            if (err) return next(err)

            // Android legacy call
            if (req.body.getEntities) {
              var geReq = {dbOps: {user: _.cloneDeep(savedUser), tag: req.tag}}
              var geOps = {
...
function updateInstall(user, installId, cb) {
  ...
  db.installs.safeFindOne({_id: _install}, ops, function(err, install) {
    ...
    install.modifiedDate = util.now()
    db.users.safeUpdate(install, ops, cb)
  })
}
...
function signout(req, res, next) {
  ...
  function clearInstall() {
    var installOps = {asAdmin: true, tag: req.tag}
    db.installs.safeFindOne({_user: req.user._id}, installOps, function(err, install) {
      if (err) return next(err)
      if (!install) return finish()
      install._user = null
      install.modifiedDate = util.now()
      db.installs.safeUpdate(install, installOps, finish)
    })
  }
