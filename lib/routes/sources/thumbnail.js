...
// var url2png = require('url2png')('P5180054734736', 'SB0757D35C5907') // promo code
  ...
  if (useBlu) {
    ...
    ops = {
      ...
      query: {
        ...
        user: thumb.userid,
        size: 'medium2',
        output_type: 'png',   // doesn't work, getting jpegs
        cache: 1,
        hash: hash,
      },
      log: true,
  ...
  function callThumbnailService(ops) {
    ...
    req.end(function(err, res) {
        ...
        if (!(res.headers['content-type'].indexOf('image') >= 0) &&
          ...
          return cb(perr.partnerError('Thumbnail service ' +
              ops.provider + ' failed to return an image.', res.text))
        }

        var filePath = path.join(thumbDir, fileName + '.png')
