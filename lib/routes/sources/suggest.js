...
function run(ops, done) {
  ...
  if (!(ops.sources && ops.sources.length)
      && ops.place && ops.place.provider) {

    for (var providerKey in ops.place.provider) {
      if (_sources[providerKey]) {
        ops.sources.push({
          type:  providerKey,
          id: ops.place.provider[providerKey],
        })
      }
    }
  }
  ...
  if (ops.place.name
      && ops.place.location.lat
      && ops.place.location.lng) {
