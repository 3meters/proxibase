/*
 * routes/info.js
 *
 *   Human readable JSON describing various aspects of the service
 */

var util = require('util')
  , log = util.log
  , db = util.db
  , gdb = util.gdb
  , config = util.config
  , info = {}


exports.addRoutes = function(app) {
  init(app)
  app.get('/', function(req, res) {res.send(info.site)} )
  app.get('/client', getClient)
  app.get('/schema', function(req, res) { res.send({schemas: info.schema}) })
  app.get('/schema/:collection', getSchema)
  app.get('/langs', function(req, res) { res.send({langs: util.statics.langs}) })
  app.get('/errors', function(req, res) { res.send({errors: httpErr}) })
}


function init(app) {
  var url = config.service.url
  info = {
    site: {
      name: config.service.name,
      version: config.service.version,
      clients: url + '/client',
      schemas: url + '/schema',
      data: url + '/data',
      stats: url + '/stats',
      admin: url + '/admin',
      methods: url + '/do',
      languages: url + '/langs',
      errors: url + '/errors',
      docs: 'https://github.com/3meters/proxibase#readme'
    },
    data: {},
    schema: {}
  }

  Object.keys(db.cNames).forEach(function(name) {
    info.data[name] = url + '/data/' + name
    info.schema[name] = url + '/schema/' + name
  })
  app.info = info
}


// Experimental API to provide data-driven client upgrade trigger
function getClient(req, res, next) {
  util.db.documents.findOne({name:'client'}, function(err, doc) {
    if (err) return res.error(err)
    if (doc) res.send(doc.data)
    else res.send({
      notes: 'returns mongodb.documents[\'client\'].data if exists, else this sample:',
      android: {
        required: '3.0.2',
        stable: '3.1.3',
        head: '3.2.2'
      }
    })
  })
}

// Individual schema pages
function getSchema(req, res) {
  var collection = req.params.collection
  if (!gdb.models[collection]) return res.error(httpErr.notFound)
  res.send({schema: gdb.schemaDocs[collection]})
}
