...
exports.init = function(app) {
  ...
  greeting = {
    ...
    params: 'refresh=true to regenerate stat (requires admin)',
    stats: {}
  }
  for (stat in stats) {
...
var getStat = exports.getStat = function(req, arg) { // arg can be cb or response object

  var cb = (tipe.isFunction(arg)) ? arg : util.send(arg)
  ...
  if (req.refresh) return req.stat.refresh(req, cb)
...
function genLinksFromUsers(req, cb) {
  ...
  var map = function() {
    emit({
      user: this._from,
      collection: this.toCollectionId,
      type: this.type
    }, 1)
  }
  ...
  var options = {
    query: {fromCollectionId: 'us'},
    out: {inline: 1}
  }

  db.links.mapReduce(map, reduce, options, finish)
  ...
  function finish(err, results) {
    ...
    results.forEach(function(raw) {
      ...
      docs.push({
        _id: String(rank),
        _user: raw._id.user,
        collection: util.statics.collectionIdMap[raw._id.collection],
      ...
      db.collection(temp).drop(function(err) {
        ...
        db.createCollection(temp, function(err) {
          ...
          db.collection(temp).insert(docs, {safe: true}, function(err, savedDocs) {
            ...
            db.dropCollection(schema.name, function(err) {
              if (err) ; // continue, may not exist
              db.collection(temp).rename(schema.name, function(err) {
                if (err) return cb(err)
                // now set refresh to false and call getStat again, returning the refreshed collection
                req.refresh = false
                return getStat(req, cb)
              })
            })
          })
        })
