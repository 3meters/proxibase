...
var getStat = exports.getStat = function(req, res, next) {
  if (!req.scrubbed) {
    var err = scrub(req)
    if (err) return res.error(err)
  }
  if (req.refresh) return req.stat.generate(req, res, getStat)
...
function scrub(req) {
  ...
  if (!req.stat) return new HttpErr(httpErr.notFound)
  ...
  if (req.refresh) {
    ...
    if (!req.asAdmin) return new HttpErr(httpErr.badAuth)
