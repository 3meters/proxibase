 ...
 *
 *    while logged in as a admin will invoke the generation function before retrieving the collection
 *
 */
...
var util = require('util')
  ...
  , refresh = false
  , stats = {
      // statUrlPath: statGenFunction
      usersByEntity: genUsersByEntityCount
    }
...
function welcome(req, res) {
  res.send({
    info1: 'GET /stats/<statName> returns a statistic collection ' +
           'using the same parameters as get /data/collection.',
    info2: 'Set ?refresh=true to regenerate the collection. ' +
...
function scrub(req, res, next) {
  if (!stats[req.params.stat]) return res.error(httpErr.notFound)
  req.cName = 'stats_' + req.params.stat
...
function getStat(req, res, next) {
  if (refresh) {
    // gen new statistics collection
    return stats[req.params.stat](req, res, next)
  }
  data.parse(req, res, function(){
 ...
 */
function genUsersByEntityCount(req, res, next) {
