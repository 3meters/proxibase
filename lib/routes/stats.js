...
function run(options, cb) {
  ...
  // $match
  // pretend the underlying collection is flattend like the output, instead
  // of stored in mongodb reduced format.
  var filters = []

  // _id.toSchema and _id.fromSchema
  if (clTo) filters.push({'_id.toSchema': clTo.schema.name})
  if (clFrom) filters.push({'_id.fromSchema': clFrom.schema.name})
  ...
  // _catetory and query.  query is just passed through unmodified.
  // It is useful if the caller understands the underlying schema and
  // wants to query it directly without all this sugar
  ;['_category', 'query'].forEach(function(filter) {
  ...
  options.collection.aggregate(aggOps, function(err, aggResults) {
    ...
    options.collection.decorate(aggResults, decorateOps, function(err, raggedDocs) {
      ...
      // flatten the stored reduced-format collection into a conventional array
      var rank = 1
