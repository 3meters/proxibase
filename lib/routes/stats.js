...
function setFrom(req, res, next) {
  req.statClName = 'froms'
  next()
}


function refresh(req, res) {
  db[req.statClName].refresh(req.dbOps, function(err, results) {
...
function rebuild(req, res) {
  db[req.statClName].rebuild(req.dbOps, function(err, results) {
    if (err) return res.error(err)
    res.send(results)
  })
}
...
function getRest(req, res) {
  ...
  options.statClName = req.statClName
...
function get(options, cb) {
  ...
  var aggOps = []
  var statClName = options.statClName
  ...
  ;['type', 'day'].forEach(function(filter) {
    var idFilter = {}
    if (options[filter]) {
      idFilter['_id.' + filter] = options[filter]
      filters.push(idFilter)
    }
    else if (options.query && options.query[filter]) {
      idFilter['_id.' + filter] = options.query[filter]
      filters.push(idFilter)
      delete options.query[filter]
    }
  })
  ...
  // $group
  var group = {
    _id: (statClName === 'tos') ? '$_id._to' : '$_id._from',
    value: {$sum: '$value'},
  }
  if ('tos' === statClName) group.toSchema = {$first: '$_id.toSchema'}
  ...
  if (logQuery) log(statClName + ' aggregate options:', aggOps)


  // do it
  db[statClName].aggregate(aggOps, function(err, aggResults) {
    if (err) return cb(err)

    // get referenced data from base documents
    var decorateOps = {
      asAdmin: true,
      refs: 'name,photo,category,location,visibility',
      utc: options.utc,
    }

    // decorate is defined in mongosafe/read
    db[statClName].decorate(aggResults, decorateOps, function(err, raggedDocs) {
      if (err) return cb(err)
      ...
      if (logQuery) {
        meta.query = {}
        meta.query[statClName + '.aggregate'] = aggOps
      }
