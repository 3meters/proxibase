 ...
 */
module.exports.sendErr = function(err, statusCode) {
 ...
 */
module.exports.findConfig = function(configFile) {
  ...
  try {
    ...
    try {
      config = require(__dirname + '/../conf/' + configFile)
    }
    catch(e) {
      log('Fatal: could not load config file ' + configFile)
 ...
 */
module.exports.getUrl = function(config) {
  return config.protocol + '://api.' + config.host + ':' + config.port
}

/*
 * Return the elapsed time between the input time and now expressed in seconds
 *  start is a Date object
 */
module.exports.getElapsedTime = function(start) {
