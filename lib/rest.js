...
exports.get = function(req, res) {
  ...
  var query = model.find().limit(defaultLimit);
  ...
  var setQueryOptions = {
    ...
    __find: function(criteria) {
      if (typeof criteria === "string") {
        try {
          ...
          return res.send({ error: "Could not parse __find criteria as JSON" }, 400);
        }
      }
      query.find(criteria);
    },
    ...
    __lookups: function(s) {
      if (truthy(s)) {
        if (req.query.__fields) return res.send({ error: "Cannot set both __lookups and __fields" }, 400);
    ...
    __limit: function(s) {
      query.limit(Math.min(defaultLimit, parseInt(s)));
    }
  }

  // set the mongoose query options based on the req.query map
  for (var key in req.query) {
    if (!setQueryOptions[key]) return res.send({ error: "Unrecognized query parameter " + key }, 400);
    setQueryOptions[key](req.query[key]);
  }
