...
exports.get = function(req, res, cb) {
  ...
  stream.on('close', function() {
    ...
    return cb({ data: docs, count: docs.length, moreRecords: moreRecords });
  });
...
exports.update = function(req, res, cb) {
  ...
  if (req.body.data._id && req.body.data._id !== req.qry.ids[0])
    return cb({ Error: "Cannot change the value of _id" }, 400);

  var query = req.model.findOne({ _id: req.qry.ids[0] }, ['_id'], function (err, doc) {
    if (err) res.send({ Error: err.stack }, 500);
    if (!doc) res.send(notFound, 404);
    for (prop in req.body.data) {
      doc[prop] = req.body.data[prop];
    };
    doc.save(function(err, count) {
      return res.send({ info: "updated " + req.modelName, count: count });
    });
  });
  /*
  var doc = (req.body.data);
  delete doc._id;  // ignore any _id value in the body, use the one from the url

  var query = req.model.update(doc);
  query.where('_id').in(req.qry.ids);
  ...
  */
  ...
  /*
  req.model.update({ _id: req.qry.ids[0] }, doc, function(err, count) {
