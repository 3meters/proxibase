  ...
  db.collection(req.modelName)
    ...
    .toArray(function process(err, docs) {
      if (err) {
        ...
        return res.sendErr(err.stack || err.message || err)
      }
...
exports.get2= function(req, res, cb) {
  ...
  stream.on('close', function() {
    ...
    return cb({ data: docs, count: docs.length, more: moreRecords, time: req.timer.read() })
  })
...
exports.create = function(req, res, cb) {
  ...
  doc.save(function (err, savedDoc) {
    ...
    if (!savedDoc._id) {
      ...
      util.logErr('Server error: ' +  err.message)
      util.logErr('Document:', doc)
