...
exports.update = function(req, res, cb) {
  ...
  if (req.qry.ids.length > 1)
    return sendErr(res, new Error('Updating multiple documents per request is not supported'))
  var docId = req.qry.ids[0]
  var newDoc = req.body.data
  if (newDoc._id && newDoc._id !== docId)
    return sendErr(res, new Error('Cannot change the value of _id'))

  var query = req.model.findOne({ _id: docId }, function (err, doc) {
    if (err) return sendErr(res, err)
    if (!doc) sendErr(res, 404)
    for (prop in newDoc) {
    ...
    doc.save(function(err, updatedDoc) {
      if (err) return sendErr(res, err)
      if (!updatedDoc) {
        var err = new Error('Update failed for unknown reason for doc ' + docId)
        log('Error ' + err.message)
        return sendErr(res, err, 500)
      }
