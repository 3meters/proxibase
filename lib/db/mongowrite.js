...
function extendMongodb() {
  ...
  function(doc, options) {
    return util.chk(doc, this.schema.fields, options)
  }
...
function safeExec(collection, method, query, doc, options, cb) {
  ...
  function callValidator(validator, next) {
    // TODO: what if validator never calls its callback?  timeout?  
  ...
  function handleInserts(err, docs) {
    if (err) return cb(err)
    if (!validators.afterInsert) return cb(null, docs[0])
  ...
  function handleUpdates(err, count) {
    ...
    collection.findOne(query, function(err, savedDoc) {
      if (err) return cb(err)
      if (!validators.afterUpdate) return cb(null, savedDoc)
      async.forEachSeries(validators.afterUpdate, callValidator, function(err) {
        if (err) util.logErr('Error in afterUpdate validator:', err.stack||err)  // logged, not returned
        cb(null, savedDoc)
      })
    })
  }
...
function prepareInsert(doc, schema) {
  ...
  var err = util.chk(doc, schema.fields, {strict: true})
  if (err) return err
  else return doc
}
...
function prepareUpdate(doc, previous, schema) {
  ...
  var err = util.chk(doc, schema.fields, options)
  if (err) return err
