  ...
  function executeMongoCommand(err) {
    ...
    switch(method) {
      ...
      case 'update':
        ...
        if (!doc) return cb(new Error('Update failed'), previous)
        var err = util.chk(doc.$set, collection.schema.fields, options)
        if (err) return cb(err)
        else return collection.update(query, doc, options, handleUpdates)
        break
