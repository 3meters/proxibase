...
function ensureAuthSource(doc, previous, options, next) {
  ...
  if (previous) {
    if (doc.authSource && doc.authSource !== previous.authSource) {
      return next(proxErr.mustChangeViaApi('authSource: NYI'))
    }
  }
  else {
...
function authByPassword(credentials, callback) {
  ...
  this.findOne({email: credentials.email}, function(err, user) {
    ...
    if (!user) return callback(proxErr.badAuthCred())
    if (!verifyPassword(user.password, credentials.password)) return callback(proxErr.badAuthCred())
...
function reqValidate(user, options, callback) {

  var self = this
  var link = util.config.service.url + '/user/validate?user=' + user._id +
      '&key=' + hashValidationKey(user._id, user.email)
  ...
  util.sendMail({
    ...
    log('User email validation notification sent to ' + user.email)
    // Fire-and-forget update bypassing revalidation
    var doc = {
      _id: user._id,
      validationNotifyDate: util.getTime()
    }
    self.safeUpdate(doc, options, callback)
  })
}
...
function setValidationDate(user, options, callback) {
  this.update(
    {_id: user._id},
    {validationDate: util.getTime()},
    options,
    callback
  )
}
