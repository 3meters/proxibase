...
// Verify Password
function verifyPassword(UserId, plainPass) {
  this.findOne({_id: UserId}, function(err, user) {
    if (plainPass) return (user.password === hashPassword(plainPass))
    else return !user.password  // possible for legacy users
  })
}
...
function changePassword(doc, options, callback) {

  assert(doc.oldPassword && doc.newPassword, 'Invalid call to changePassword')
  ...
  if (options.user.role !== 'admin') {
    ...
    if (!verifyPassword(doc.oldPassword)) {
