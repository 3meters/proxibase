...
Users.methods.reqValidate = function(callback) {
  var self = this
  var link = util.config.service.url + '/user/validate?email=' + encodeURIComponent(self.email) + 
      '&key=' + hashValidationKey(self.email)
  var text = 'Thank you for singing up for AirCandi.  To complete your registration ' + 
      'please click the link below or copy-paste it into your browser.  \n\n' + link + 
      '\n\nThanks!\n\n--The AirCandi Team'
  util.sendMail({
    // to: self.name + ' <' + self.email + '>',
    to: self.name + ' <georgesn@gmail.com>',
...
function hashValidationKey(email) {
  var hashData = [email, util.serverSecret, 'uK1R4']
