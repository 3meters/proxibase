...
Users.methods.sendValidationMail = function() {
  this.data.emailValidationKey = hashEmailValidationKey(this._id)
  util.sendMail({
    to: '<' + this.name + '> ' + this.email,
    subject: 'Complete your AirCandi Registration',
    body: 'Click this link to complete your AirCanid registration: \n\n' + 
        util.config.service.url
    }, function(err, res) {
      if (err) util.logErr('Server Error.  Validated user email notification failed', err) 
      log ('User email validation notification sent to ' + this.email)
    })
}


// Validate Email
Users.methods.validateEmail = function(key) {
  return (key === this.data.emailValidationKey)
}
...
function hashEmailValidationKey(id) {
  return crypto.createHmac('sha1', 
      [id, util.getTimeUTC(), util.statics.serverSecret]).digest('hex')
}
