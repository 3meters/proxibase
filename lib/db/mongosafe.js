...
;(function() {
  ...
  Collection.prototype.safeUpsert = function(doc, options, callback) {
    checkParams(arguments, true)
    this.findOne({_id: doc._id}, function(err, doc) {
      if (err) return callback(err)
      if (doc) this.safeUpdate(doc, options, callback)
      else this.safeInsert(doc, options, callback)
    })
  }
...
function checkParams(args, idRequired) {
  ...
  assert(!args[1].multi, 'multi option')
