...
function checkArgs(args, idRequired) {
  try {
    assert(type.isObject(args[0]),
        'Invalid param document')
    assert(type.isObject(args[1]),
        'Invalid param options')
    assert(!args[1].upsert,
        'upsert option not supported, use safeUpsert')
    assert(!args[1].multi,
        'multi option not supported')
    assert(type.isFunction(args[2])
        || type.isUndefined(args[2]),
        'Invalid param cb')
    args[2] = args[2] || console.log
    if (idRequired) {
      assert(type.isString(args[0]._id),
          'Missing or invalid document._id')
    }
  }
  catch (e) { return e }
  return null
