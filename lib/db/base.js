...
var Schema = exports.Schema = function(schemaId) {
  ...
  // Turn off Mongoose V3 document versioning
  // See http://aaronheckmann.posterous.com/mongoose-v3-part-1-versioning
  this.set('versionKey', false)
  this.set('safe', true)
  this.set('strict', false)     // try setting to 'throw'
  this.set('autoIndex', false)  // we will ensureIndexes synchronously on startup


  // Must be logged in to save
  this.pre('save', function(next) {
    if (!this.__user) return next(proxErr.badAuth())
    next()
  })
...
//   Not used due to lack of large numbers in Javascript
