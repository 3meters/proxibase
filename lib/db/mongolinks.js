...
var linksQuerySchema = {
  type: 'array', value: {
    type: 'object', value: {
      to:           {type: 'object', strict: false},  // collection or schema document links to
      from:         {type: 'object', strict: false},  // collection or schema with links to document
      ...
      sort:         {type: 'array', value: {type: 'object'}, default: [{_id: 1}], strict: false},
      limit:        {type: 'number'},
      skip:         {type: 'number', default: 0},
      count:        {type: 'boolean', validate: function(v) {return 'Error: Count is NYI'}},
    },
    strict: true,
    validate: function(linkq, options) {
      ...
      var db = options.db
      // Ensure that each key in linkq.to is a known collection or schema
      for (var cl in linkq.to) {
        if (!linkq.to[cl]) delete linkq.to[cl]
        else {
          if (db.collection(cl)) {}  // known collection, we are done
          else {
            if (db.schema(cl)) {
              // cl is a known schema, replace it with its collection name
              linkq.to[db.schema(cl).collection] = 1
              delete linkq.to[cl]
            }
      ...
      for (var cl in linkq.from) {
        if (!linkq.from[cl]) delete linkq.from[cl]
        else {
          if (db.collection(cl)) {}   // known collection, we are done
          else {
            if (db.schema(cl)) {
              linkq.from[db.schema(cl).collection] = 1
    ...
    function getLinkq(linkq, nextLinkq) {
      ...
      function processTos() {
        ...
        function getLinkedTos(cl, nextCl) {
          ...
          var schema = db.schemas[cl].schema
          var selector = _.extend(linkq.linkFilter, {_from: doc._id, toSchema: schema})
      ...
      function getLinkedDocs(cl, ids, cb) {
        ...
        var selector = _.extend(linkq.filter, {_id: {$in: ids}})
