...
exports.parseId = function(idStr) {
  ...
  if (idStr.indexOf(sep) < 0) {
    ...
    if (!validTableId(id.tableId)) return new HttpErr(httpErr.badSchemaId, idStr)
    if (parts.length === 5) {
...
// Handle database errors, mainly pretty up duplicate key violations
exports.handleDbErr = function(err, res) {
  assert(err instanceof Error && res) 
