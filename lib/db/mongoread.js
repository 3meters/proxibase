...
function getQuerySchema() {
  return {
    ...
    links:      {type: 'array', value: {
      type: 'object',
      value: {
        to:           {type: 'string'},  // collection with links from
        from:         {type: 'string'},  // collection with links to
...
function maxLimit(v) {
  if (v > statics.limitMax) {
    return fail('Max limit is ' + statics.limitMax, v)
  }
}

function checkLinkQuery(v) {
  if (v.to && v.from) {
    return fail('Link queries cannot specify both to and from', v)
  }
  if (v.to) {
    v.collection = v.to
    v.direction = 'to'
  ...
  if (!db.schemas[v.collection]) {
    return fail('Invalid link query: invalid collection', v)
  }
}

function fail(msg, obj) {
  return 'Invalid find query: ' + msg + (obj && '\n' + util.inspect(obj))
}
...
function extendMongodb() {
  ...
  Collection.prototype.safeFind = function() { // query, options, cb
    ...
    var err = util.chk.call(this, args.query, getQuerySchema())
  ...
  function getLinkedDocuments(err, docs) {
    ...
    function getLinks(doc, nextDoc) {
      ...
      function getIds(linkq, nextLinkq) {
        ...
        doc[linkq.as] = [] // We return an empty array if no links are found

        var selector = {}
        if ('to' === linkq.direction) {
          selector = {
            _from: doc._id,
            toCollection: linkq.collection,
          }
        }
        else {
          selector = {
            _to: doc._id,
            fromCollection: linkq.collection,
          }
        }
