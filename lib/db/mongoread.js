...
function safeFind(collection, query, cb) {
  ...
  // Regular find
  return collection.find(selector, options).toArray(getLinked)
   ...
   */
  function getLinked(err, docs) {
    if (err) return cb(err)
    var toIds = {}, fromIds = {}

    if (!tipe.isObject(query.to)) return getLinkedFrom()

    async.forEach(docs, getLinkedTo, getLinkedFrom)

    function getLinkedTo(doc, nextTo) {

      log('query.to', query.to)

      async.forEach(Object.keys(query.to), getToIds, getTos)

      function getToIds(to, next) {
        if (!db.collection(to)) return next(perr.badValue(to))
        var selector = {
          _from: doc._id,
          toCollectionId: util.statics.collectionIds[to],
        }
        db.links.find(selector, {limit: query.to[to].limit || options.limit})
          .toArray(function(err, links) {
          if (err) return next(err)
          if (links && links.length) toIds[to] = links
      ...
      function getTos(err) {
        ...
        async.forEach(Object.keys(toIds), getToDocs, finish)

        function getToDocs(clName, next) {
          var links = toIds[clName]
          db[clName].find({_id: {$in: links}}, function(err, docs) {
            if (err) return next(err)
            doc.to = doc.to || {}
            doc.to[clName] = docs
            next()
    ...
    function getLinkedFrom(err) {
      if (err) return cb(err)
      if (!tipe.isObject(query.from)) return validate(null, docs)
      validate(null, docs)
    }
  }
