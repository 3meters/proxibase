...
function getQuerySchema() {
  return {
    ...
    links:      {type: 'array', value: {
      type: 'object',
      value: {
        to:           {type: 'string'},  // schema with links from
        from:         {type: 'string'},  // schema with links to
...
function checkLinkQuery(qry, options) {
  if (qry.to && qry.from) {
    return 'find link query cannot specify both to and from'
  }
  log('options', options)
  qry.collection = options.collection
  if (qry.to) {
    qry.schema = qry.to
    qry.direction = 'to'
  }
...
function extendMongodb() {
  ...
  Collection.prototype.safeFind = function() { // query, options, cb
    ...
    var err = util.chk(args.query, getQuerySchema(), {collection: this.schema.collection})
    ...
    function getLinks(doc, nextDoc) {
      ...
      function getIds(linkq, nextLinkq) {
        ...
        if ('to' === linkq.direction) {
          selector = {
            _from: doc._id,
            toSchema: linkq.schema,
          }
        }
        else {
          selector = {
            _to: doc._id,
            fromSchema: linkq.schema,
          }
        }
