...
function getQuerySchema() {
  return {
    ...
    skip:       {type: 'number'},
    limit:      {type: 'number', default: statics.limitDefault,
      value: function(v) { return (v <= statics.limitMax) }},
...
function checkArgs(query, idRequired) {
  var err = util.check(query, getQuerySchema())
  if (err) return err
  return null
}
    ...
    function getLinks(doc, nextDoc) {
      ...
      query.links.forEach(function(linkq, index) {
        if (!(linkq.to || linkq.from)) {
          return perr.missingParam('links[' + index + '] missing required to or from')
        }
        if (linkq.to) {
          if (!db.collection(linkq.to)) {
            return perr.badValue(linkq.to)
          }
          linkq.collection = linkq.to
          linkq.direction = 'to'
        }
        else {
          if (!db.collection(linkq.from)) {
            return perr.badValue(linkq.from)
          }
