...
function extendMongodb() {
  ...
  function(query, cb) {
    ...
    log('debug safeFind query', query)
    var err = util.chk(query, getQuerySchema())
    log('debug safeFind query after chk', query)
...
function safeFind(collection, query, cb) {
  ...
  log('debug query.limit ' + query.limit)
    ...
    function getLinks(doc, nextDoc) {
      ...
      function getIds(linkq, nextLinkq) {
        ...
        var options = {
          ...
          limit: Math.min(statics.limitMax, linkq.limit || outerQueryLimit),
          sort: ['_id', 'desc'],
        }
        ...
        db.links.find(selector, options).toArray(function(err, links) {
          ...
          log('debug find links selector', selector)
          log('debug find links options', options)
