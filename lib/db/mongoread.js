...
function safeFind(collection, query, cb) {
  ...
  // Regular find
  return collection.find(selector, options).toArray(validate)
  ...
  function validate(err, docs) {
    ...
    // TODO: implement
    return processResults(err, docs)
    ...
    async.series(validators.read, validateDocs, processResults)

    function validateDocs(validator) {
      async.forEachSeries(docs, validateDoc(doc, validator))
    }

    function validateDoc(doc, validator) {
