...
function safeFind(collection, query, options, cb) {
  ...
  /*
   * getLookups: populate refs (aka foreign keys) with data from the
  ...
  function getRefs(docs, cb) {
    ...
    // On^2 but in memory and limited by the outer query
    docs.forEach(function(doc) {
    ...
    function finish(err) {
      ...
      docs.forEach(function(doc) {
        for (var field in refs) {
          ...
          if (!clName) continue
          // if _owner create a field named owner with the name of the owner
          // How to specifiy more fields than that?
          // How to sort and filter based on values in the looked-up collection?
          var refFieldName = (field === '_id')
            ? 'name'            // self join
            : field.slice(1)    // e.g. _owner => owner
          var refDoc = refMap[clName][doc[field]]
          // depending on value of refs option
          // truthy: the entire ref doc is nested
          // just one field, put that value at top level, e.g. refs=name
          // more than one field, just those fields will be included in a nested doc
          if (!refDoc) continue
