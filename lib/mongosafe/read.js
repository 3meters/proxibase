...
function safeFind(collection, query, options, cb) {
  ...
  var selector = _.extend({}, query.find, query.filter, query.query)  // last listed wins
  ...
  if (_.isEmpty(selector) && !_.isEmpty(query)) selector = query
  ...
  // Regular find
  debug('safeFind selector', selector)
  debug('safeFind options', options)
