...
var findSpec = {
  type: 'object', value: {
    ...
    utc:            {type: 'boolean'},
    linkCount:      {type: 'object|array', deprecated: true},
...
// Options can include a refrence to the collection object with is not useful to dump
/*
...
function runSafeFind(collection, query, ops, cb) {
  ...
  // debug('safeFind selector for ' + clName, selector)
  ...
  function runQuery(err, collection, query, ops) {
    ...
    function processResults(err, results) {
      ...
      if (results && (ops.links || ops.linked || ops.linkCounts || ops.linkCount)) {
        var linkOps = _.pick(_.cloneDeep(ops),
            ['user', 'asAdmin', '_acl', 'links', 'linked', 'linkCount', 'linkCounts', 'tag'])
    ...
    function finish(err, results) {
      ...
      decorate(collection, results, ops, function(err, results) {
        ...
        var scope = {results: results, options: ops}

        async.eachSeries(after, callAfter, send)

        function callAfter(fn, next) {
          fn.call(collection, scope, function(err, newScope) {
            if (err) return cb(err)
        ...
        function send(err) {
          if (err) return cb(err)
          cb(null, scope.results, scope.options)
        }
      })
    }
  }
...
function safeFindById(id, options, cb) {
  ...
  if (tipe.isError(clName)) return cb(perr.badValue(clName))
  if (!clName) return cb(perr.badValue('Unknow collection for id: ' + id))
...
function safeFindByIds(ids, options, cb) {
  ...
  ids.forEach(function(id) {
    var clName = clNameFromId(id)
    if (tipe.isError(clName)) return cb(perr.badValue('Unknow collection for id: ' + id))
    if (!clName) return cb(perr.badValue('Unknow collection for id:', id))
    clMap[clName] = clMap[clName] || []
    clMap[clName].push(id)
  })

  async.eachSeries(Object.keys(clMap), getDocs, finish)

  function getDocs(clName, nextCl) {

    if (clMap[clName].length === 1) {
      // SafeFindOne
      _db[clName].safeFindOne({_id: clMap[clName][0]}, options, function(err, doc) {
        if (err) return cb(err)
      ...
      // SafeFind
      _db[clName].safeFind({_id: {$in: clMap[clName]}}, options, function(err, foundDocs) {
        if (err) return cb(err)
...
function logSlow(time, clName, selector, ops, err, results) {
  ...
  else if (tipe.isNumber(results)) logErr('count: ' + results)
  else if (results && results.length) logErr('count: ' + results.length)
