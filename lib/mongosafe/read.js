...
var linksQuerySpec = {
  ...
  type: 'array', value: {
    type: 'object', value: {
      ...
      docFilter:   {type: 'object', strict: false, default: {}},
      docFields:   fieldSpec,
      sort: {
        type: 'object|array|string',
        default: '-_id',
...
function safeFind(query, options, cb) {
  ...
  validate(collection, query, options, function(err, query, options) {
    ...
    if (tipe.isDefined(options.result)) return processDocs(null, options.result)
    ...
    function processDocs(err, docs) {
      ...
      if (options.links) {
        getLinkedDocs(collection, docs, query, options, finish)
      }
...
function safeFindOne(query, options, cb) {
  ...
  // Call the find validators
  validate(collection, query, options, function(err, query, modifiedOps) {
    if (err) return cb(err)

    if (modifiedOps) options = modifiedOps

    // One of the validators determined the result, skip the query
    if (tipe.isDefined(options.result)) return processDoc(null, options.result)

    // Diagnostics
    if (options.log || options.debug) {
      log('safeFindOne query on ' + collection.collectionName + ':', query)
      log('safeFindOne options:', options)
    }

    // Run the query
    collection.findOne(query, options, processDoc)

    function processDoc(err, doc) {
      if (err) return cb(err)
      if (!doc) return cb(null, null, options)
      ...
      function finish(err, doc) {
        ...
        decorate(collection, doc, options, function(err, doc) {
          ...
          // Delete options that don't make sense for findOne
          delete options.sort
          delete options.limit
          delete options.skip

          if (doc) options.count = 1
          var after = collection.schema.after.read

          // Return results
          if (!after) return cb(null, doc, options)

          // Call the after function with everthing we know
          var scope = {
            doc: doc,
            options: options,
          }
...
function validate(collection, query, options, cb) {
  ...
  function callBefore(before, next) {
    ...
    before.call(collection, query, options, function(err, newQuery, newOptions) {
      ...
      // If a validator populates the result property of options
      // return immediately and send them
      if (tipe.isDefined(options.result)) return finish()
      next()
    })
  }
...
function getLinkedDocs(collection, docs, query, options, cb) {
  ...
  if (tipe.isObject(docs)) doc = docs
    ...
    return async.mapSeries(docs, function(doc, nextDoc) {
      getLinkedDocs(collection, doc, query, options, nextDoc)
    }, cb)
  }
  ...
  function getLinkq(linkq, nextLinkq) {
    ...
    function getLinks(selector, direction, cb) {
      ...
      // If link fields are defined merge in the system fields
      ...
      function nextLink() {
        cursor.nextObject(function(err, link) {
          ...
          // If linkq.docFields is falsey skip fectching the linked document and
          // promote the link as the top-level member of the .linked array. This
          // seems a little too subtle -- we may need a more explict API.
          if (!tipe.isTruthy(linkq.docFields) && tipe.isUndefined(linkq.docFilter)) {
            results.push(link)
          ...
          var docSelector = _.extend(linkq.docFilter, {_id: linkField})
          var findDocOps = {
            user: options.user,
          ...
          if (!_.isEmpty(linkq.docFields)) {
            findDocOps.fields = linkq.docFields
          }

          // Find the linked doc with the docFilter on. If found push on results.
...
function validateLinkQuery(linkq, options) {
  ...
  if (linkq.count) {
    if (!_.isEmpty(linkq.docFilter)) return 'Link count does not support docFilter.'
