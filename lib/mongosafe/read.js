...
function extend(mongo) {
  ...
  Collection.prototype.safeFind = function() { // query, options, cb
    ...
    debug('before scrub', args.query)
    var err = scrub(args.query, querySpec, {db: this.db, log: true})
    if (err) {
...
function safeFind(collection, query, options, cb) {
  ...
  selector = query.filter || query.find || {}       // find is deprecated, use filter
  ...
  function getLinkedDocuments(err, docs) {
    ...
    debug('documents before getting links', docs)
    if (query.links) {
