...
function safeFind(collection, query, options, cb) {
  ...
  function getRefs(docs, cb) {
    ...
    if (docs.length > statics.internalLimit) {
      logErr('getRefs called on collection larger than ' + statics.internlLimit + '. skipping')
      return cb()
    }
    ...
    function getRefDocs(clName, nextCl) {
      ...
      db[clName].safeFind({_id: {$in: ids}}, ops, function(err, refDocs) {
        if (err) {
          if (401 === err.code) return nextCl() // user doesn't have permissions to read the collection, skip
