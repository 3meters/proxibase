...
function safeFind(collection, query, options, cb) {
  ...
  // regular find without links
  if (!options.links) {
    return collection.find(selector, options).toArray(finish)
  }

  // find with links
  findWithLinks(collection, selector, options, finish)


  // We now have our top-level docs.  Decorate and return results to caller
  function finish(err, docs) {
    ...
    if (options.refs) {
      getRefs(collection, docs, options, function(err, docs) {
        if (err) return cb(err)
        else return cb(null, docs, meta)
      })
    }
    else return cb(null, docs, meta)
  }
...
function findWithLinks(collection, selector, options, cb) {
  ...
  // Walk the collection with a cursor adding results until the limit
  // is reached
  collection.find(selector, {batchSize: 100}).each(function(err, doc) {
    if (err) return cb(err)
      ...
      function getLinks(selector, direction, cb) {
        ...
        function nextLink() {
          cursor.nextObject(function(err, link) {
            ...
            if (!linkq.noDocuments) {  // BUG: find another place to remove the doc
