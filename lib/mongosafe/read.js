...
function safeFind(query, options, cb) {
  ...
  validate(collection, query, options, function(err, query, options) {
    ...
    function processDocs(err, docs) {
      ...
      // The query syntax for the links themselves is shared
      options.links = options.links || options.linked

      // Get linked documents or links
      if (options.links) {
        getLinks(collection, docs, query, options, finish)
      }
...
function safeFindOne(query, options, cb) {
  ...
  validate(collection, query, options, function(err, query, modifiedOps) {
    ...
    function processDoc(err, doc) {
      ...
      // Get linked documents or links
      if (options.links) {
        getLinks(collection, doc, query, options, finish)
      }
      else finish(err, doc)
...
function getLinks(collection, docs, query, options, cb) {
  ...
  if (tipe.isObject(docs)) doc = docs
    ...
    return async.mapSeries(docs, function(doc, nextDoc) {
      getLinks(collection, doc, query, options, nextDoc)
    }, cb)
  }
