...
function extend(mongo) {
  ...
  Collection.prototype.safeFind = safeFind
  Collection.prototype.safeFindOne = safeFindOne
  Collection.prototype.safeFirst = safeFirst
  Collection.prototype.safeLast = safeLast
  Collection.prototype.safeEach = safeEach

  // Called by the stats framework
  Collection.prototype.decorate = function(docs, options, cb) {
...
function safeFirst(query, options, cb) {
  options = options || {}
  options.sort = [{_id: 1}]
  this.safeFindOrderedById(query, options, cb)
}
...
function safeFindOrderedById(query, options, cb) {
  ...
  options = options || {}
  options.limit = 1
  debug({query: query, options: options})
...
function safeEach(query, options, fn, cb) {
  ...
  collection.findLast(query, options, function(err, lastDoc) {
    if (err) return cb(err)
    ...
    function findNext(_id) {
      ...
      options.sort = [{_id: 1}]
      options.limit = 1
      collection.safeFind(nextQuery, options, function(err, doc) {
        if (err) return cb(err)

        if (!doc || (doc._id > lastId)) {
          return cb(null, count)  // done
        }

        fn(doc, function(err) {
          if (err) return cb(err)
          count++
          return findNext(doc._id)
        })
      })
    }
  })
...
// validators. Return potentially modified query and options on success
