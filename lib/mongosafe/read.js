...
function extend(mongo) {
  ...
  mongo.Cursor.prototype.safeEach = function(cb, finish) {
    var cursor = this
    safeEach(cursor, cb, finish)
  }
...
function safeFind(collection, query, options, cb) {
  ...
  if (options.limit > 1) options.limit = outerQueryLimit + 1
...
// Asycronously walk an entire cursor, one document at a time
function safeEach(cursor, cb, finish) {

  if (!(tipe.isFunction(cb) && tipe.isFunction(finish))) {
    return console.error('Expected cursor.safeEach(callback, finalCallback)')
  }
