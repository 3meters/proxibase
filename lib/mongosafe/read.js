...
function getLinkedDocs(collection, docs, query, options, cb) {
  ...
  function getLinkq(linkq, nextLinkq) {
    ...
    function getLinks(selector, direction, cb) {
      ...
      function nextLink() {
        cursor.nextObject(function(err, link) {
          ...
          if (linkq.count) {
            counts[cl] = counts[cl] || {}
            counts[cl][direction] = counts[cl][direction] || 0
            counts[cl][direction]++
            return nextLink()
          }
          ...
          // Find the linked doc with the docFilter on. Only add the link to the
          // results if doc is found.
          db[cl].safeFindOne(docSelector, findDocOps, function(err, linkedDoc) {
            if (err) return cb(err)
            if (linkedDoc) {
              if (tipe.isTruthy(linkq.docFields)) link.document = linkedDoc
              results.push(link)
            }
            nextLink()
          })
    ...
    function finishLinkQuery(err) {
      ...
      doc.linkCounts = _.extend(doc.linkCounts, counts)
