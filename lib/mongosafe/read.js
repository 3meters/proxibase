...
var querySchema = {
  ...
  links:      links.linksQuerySchema,
}

function config(options) {
  if (options && tipe.isObject(options.limits)) limits = options.limits
}
...
function extend(mongo) {
  ...
  Collection.prototype.safeFind = function() { // query, options, cb
    ...
    var err = chk(args.query, querySchema, {db: this.db})
    if (err) {
  ...
  Collection.prototype.formatSort = formatSort
...
function safeFind(collection, query, options, cb) {
  ...
  if (query.sort) findOps.sort = formatSort(query.sort)
  if (tipe.isError(findOps.sort)) return cb(findOps.sort)
 ...
 *
 * we also support a singleton object: {field1: 1}
 */
function formatSort(inSort) {
  var err, sort = []

  if (tipe.isString(inSort)) return inSort       // 'users'
  if (tipe.isObject(inSort)) inSort = [inSort]   // {users: 1}
  inSort.forEach(function(elm) {
    if (tipe.isArray(elm) && 2 == elm.length) sort.push(elm)
    else {
      if (!tipe.isObject(elm) || 1 != Object.keys(elm).length) {
        return err = new Error('Invalid sort: \n' + util.inspect(inSort))
      }
      for (var key in elm) {
        if (tipe.isTruthy(elm[key])) sort.push([key, 'ascending'])
        else sort.push([key, 'descending'])
      }
    }
  })
  return (err) ? err : sort
