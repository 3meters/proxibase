...
function getLinkedDocs(collection, docs, query, options, cb) {
  ...
  function getLinkq(linkq, nextLinkq) {
    ...
    function buildToInClause() {
      ...
      if (linkq.count) counts.to = counts.to || {}
      ...
      if (!_.isEmpty(linkq.to)) {
        ...
        for (var key in linkq.to) {
          if (linkq.count) counts.to[key] = counts.to[key] || 0
    ...
    function buildFromInClause(err) {
      ...
      if (linkq.count) counts.from = counts.from || {}
      ...
      if (!_.isEmpty(linkq.from)) {
        ...
        for (var key in linkq.from) {
          if (linkq.count) counts.from[key] = counts.from[key] || 0
    ...
    function getLinks(selector, direction, cb) {
      ...
      function nextLink() {
        cursor.nextObject(function(err, link) {
          ...
          // if linkq.docFields is falsy skip fectching linked document
    ...
    function finishLinkQuery(err) {
      ...
      if (linkq.count) {
        ...
        doc.linkedCount = _.extend(doc.linkedCount, counts)
      }
      else {
        // add links
