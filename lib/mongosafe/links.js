...
// Derive the linkCount spec
var linkCountSpec = _.cloneDeep(linksSpec)
linkCountSpec.value.limit = 0
linkCountSpec.value.type = {type: 'string', required: true, validate: function(v) {
  if (v.indexOf(',') >= 0) {
...
function validateLinkQuery(linkQrys, options) {
  ...
  linkQrys.forEach(function(linkq) {
    ...
    for (key in linkq.to) {
      if (!db.safeCollection(key)) return fail('Unknown collection: ' + key)
...
function get(collection, docs, options, cb) {
  ...
  function getOuterLinkQueries(doc, nextDoc) {
    ...
    function getOuterLinkQuery(linkOpt, nextLinkOpt) {
      ...
      var err = scrub(linkQueries, specs[linkOpt], {strict: true, db: collection.db})
      if (err) {
      ...
      function getLinkq(linkq, nextLinkq) {
        ...
        function getLinksFromDb(selector, direction, cb) {
          ...
          var linkCursorOps = {
            sort: linkq.sort,
            limit: Math.min(read.config().limits.join, linkq.limit) + 1,
            skip: linkq.skip,
          }

          if (linkOpt === 'linkCount') delete linkCursorOps.limit
          ...
          function nextLink() {
            cursor.nextObject(function(err, link) {
              ...
              cLinks++
              if (linkCursorOps.limit && (cLinks >= linkCursorOps.limit)) {
                var corpse = _.assign(selector, _.pick(linkCursorOps, ['skip', 'limit', 'sort']))
                if (linkOpt === 'links') {
                  doc.moreLinks = true
                  doc.moreLinksQueries = doc.moreLinksQueries || []
                  doc.moreLinksQueries.push(corpse)
                }
                if (linkOpt === 'linked') {
                  doc.moreLinked = true
                  doc.moreLinkedQueries = doc.moreLinkedQueries || []
                  doc.moreLinkedQueries.push(corpse)
                }
                cursor.close()
                return cb()
              }
