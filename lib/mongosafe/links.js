...
function get(collection, docs, options, cb) {
  ...
  // When there is only one parent we let the db handle limit and skip
  // When there are multiple parents we handle limit and we disallow skip
  if (docs.length === 1) singleParent = true
  ...
  function getLinkOpts(linkOpt, nextLinkOpt) {
    ...
    function getLinkq(linkq, nextLinkq) {
      ...
      // TODO: PERF:  We aggregate most of these stats live as part of the
      // stats framework.  Detect when those conditions are met and grab them
      // from the stats cache, rather than recalculating them each time.
      ...
      function getLinksFromDb(cb) {
        ...
        db.links.find(selector, linkOps).toArray(function(err, links) {
          ...
          if (cLinks > linkOps.limit) {
            ...
            if (singleParent) more = true // No problem, regular paging
              ...
              // could run a linkCounts query by doc._id to find the offending doc.
              //
