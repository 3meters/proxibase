    ...
    function getOuterLinkQuery(linkOpt, nextLinkOpt) {
      ...
      function getLinkq(linkq, nextLinkq) {
        ...
        if (linkOpt === 'linkCount') {
          ...
          // Run some additional spec checks for linkCount queries
          if (!_.isEmpty(linkq.linkedFilter)) return finish(perr.badRequest('linkCount does not support linkedFilter'))
          if (!linkq.type) return finish(perr.badRequest('linkCount requires type'))
          if (linkq.type.indexOf(',') >= 0) {
            return finish(perr.badRequest('linkCount does not support multiple types. Use an array of linkCount queries.'))
          }
