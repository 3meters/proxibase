...
function get(collection, docs, options, cb) {
  ...
  // Invalid call, bug somewhere upstream
  if (!tipe.isArray(docs)) {
    return perr.serverError('Invalid call to links, expected object or array:', docs)
  }
  ...
  function getLinkOpts(linkOpt, nextLinkOpt) {
    ...
    function getLinkq(linkq, nextLinkq) {
      ...
      function getLinksFromDb(cb) {
        ...
        var linkOps = {
          sort: linkq.sort,
          limit: config.limits.join || 1000,   // results will be loaded into memory
          skip: 0,
        }
        ...
        db.links.find(selector, linkOps).toArray(function(err, links) {
          ...
          // See if the query would retrun more links than we can safely load into memory
          ...
          docs.forEach(function(doc) {
            doc.links = doc.links || []
            if (linkq.more) doc.moreLinks = doc.moreLinks || false
          })

          links.forEach(function(link) {
            var docId = dirTo ? link._from : link._to
            if (docMap[docId].links.length < linkq.limit) {
              docMap[docId].links.push(link)
            }
            else docMap[docId].moreLinks = true
          })
      ...
      function getLinkedDocs(links, more, cb) {
        ...
        docs.forEach(function(doc) {
          ...
          if (linkq.more) doc.moreLinked = doc.moreLinked || more
        })
        ...
        var timer = util.timer()
        db[clName].safeFind(linkedDocsSelector, linkedDocsOps, function(err, linkedDocs) {
          if (err) return finish(err)
          ...
          function graftLinked(err, linkedDocs) {
            ...
            links.forEach(function(link) {
              ...
              if (linkedDoc) {
                if (doc.linked.length < linkq.limit) {
                  doc.linked.push(linkedDoc)
                }
                else if (linkq.more) doc.moreLinked = true
              }
            })
