...
function get(collection, query, options, docs, cb) {
  ...
  async.each(docs, getLinked, finish)
      ...
      function getLinks(selector, direction, cb) {
        ...
        function getNextLink() {
          cursor.nextObject(function(err, link) {
            ...
            // Set docFields to -1 to skip including linked document
            var docFields = {}
            if (tipe.isDefined(linkq.docFields)) docFields = linkq.docFields
            if (tipe.isObject(linkq[direction][cl])) docFields = linkq[direction][cl]
            if (docFields) {
            ...
            function finishGetLink() {
              if (linkq.linkFields) {
                var trimmedLink = {_id: link._id}
                if (link.document) trimmedLink.document = link.document
                for (var key in linkq.linkFields) {
                  trimmedLink[key] = link[key]
                }
                link = trimmedLink
                debug('link', link)
              }
      ...
      function finishGetLinks(err) {
        ...
        if (linkQueryIsObject) {
          if (1 !== doc.links.length) {
            err = new Error('Expected doc.links to have one element')
            err.details = doc.links
            return nextLinkq(err)
          }
