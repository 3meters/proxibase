...
function parseArgs(args, options) {  // args is arguments from the caller
  ...
  if (validTypes.indexOf(tipe(args[0])) < 0) {
    return perr.serverError('mongoSafe expects arugments[0] to be one of: ' + validTypes.join(', '))
  }

  // options
  if (!tipe.isObject(args[1])) {
    return perr.serverError('mongoSafe expects arguments[1] to be an object')
  }

  // Blacklist unsupported options
  if (args[1].upsert) {
    return perr.serverError('options.upsert not supported, use safeUpsert')
  }
  if (args[1].multi) {
    return perr.serverError('multi not supported')
  }

  // Some methods require a document._id
  if (options.idRequired && !tipe.isDefined(args[0]._id)) {
    return perr.serverError('mongoSafe arguments[0] missing required _id')
  }
...
function parseArg(arg, target) {
  ...
  switch (tipe(arg)) {
    ...
    case 'array':
      if ('array' === target) return arg // nothing to do
      else return perr.serverError('Unexpected argument type: array')  // jshint ignore:line
