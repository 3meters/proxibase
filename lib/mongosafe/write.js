...
var util = require('../utils')    // jshint ignore:line
var scrub = util.scrub            // jshint ignore:line
...
function writeDoc(cmd, collection, doc, options, cb) {
  ...
  function execute(err) {
    ...
    // Good place to inspect recursive calls
    /*
    debug('<=== safeWrite ===>')
    debug('cl', collection.collectionName)
    debug('raw doc', doc)
    ...
    */
    ...
    function shapeResults(err, dbResults) {
      ...
      if (tipe.isObject(dbResults)) {
        switch (cmd) {
          case 'insert':
            if (dbResults.ops && dbResults.ops.length) {
              savedDoc = dbResults.ops[0]
            }
            break

          case 'update':
            if (dbResults.result && dbResults.result.ok) {
              savedDoc = doc
            }
            break

          case 'remove':
            if (dbResults.result && tipe.isNumber(dbResults.result.n)) {
  ...
  function finish(err, results) {
    ...
    if (!schema.after[cmd]) return send()

    // Package up state for the after triggers
    // after triggers may modify state
    var state = {
      method: cmd,
      document: savedDoc,
      previous: previous,
    ...
    // Call each after function in series
    async.eachSeries(schema.after[cmd], callAfter, send)

    // Execute an after function. Optionally update state.
    function callAfter(fn, next) {
      fn.call(collection, state, function(err, newState) {
...
function prepareWrite(doc, previous, schema) {
  ...
  // Create a clean copy of the existing document saved in the db, pruning
  // fields that no longer exist in the current schema
  var newDoc = _.pick(_.cloneDeep(previous), Object.keys(schema.fields))

  // Remove nulls and undefined from top level properties
  // and one-level deep properties of objects
  for (key in doc) {
    if (isNull(doc[key]) || tipe.isUndefined(doc[key])) {
      delete newDoc[key]
      delete doc[key]
    }
