...
function extend(mongo) {
  ...
  function(doc, options, cb) {        // jshint ignore:line
    var args = checkArgs(arguments)
    if (tipe.isError(args)) return console.error(args)
  ...
  function(doc, options, cb) {        // jshint ignore:line
    ...
    if (tipe.isError(args)) return console.error(args)
    var self = this
    if (!doc._id) return self.safeInsert(args.doc, args.options, args.cb)
    self.findOne({_id: doc._id}, function(err, foundDoc) {
      if (err) return args.cb(err)
      if (foundDoc) {
        return self.safeUpdate(args.doc, args.options, args.cb)
      }
      else return self.safeInsert(args.doc, args.options, args.cb)
    })
  }
...
function checkArgs(inArgs, idRequired) {
  ...
  var _args = {
    ...
    cb: {type: 'function', default: console.log}
  }

  var err = scrub(args, _args)
  if (err) return err
