...
function writeDoc(cmd, collection, doc, options, cb) {
  ...
  function execute(err) {
    ...
    switch(cmd) {
      ...
      case 'insert':
        ...
        return collection.insertOne(doc, options, handleInsertedDoc)
        break

      case 'update':
        doc = prepareWrite(doc, previous, schema)
        if (tipe.isError(doc)) return cb(doc)
        return collection.updateOne({_id: doc._id}, doc, options, handleUpdatedDoc)
        break

      case 'remove':
        return collection.removeOne({_id: doc._id}, options, handleRemovedDoc)
        break
  ...
  function handleInsertedDoc(err, result) {
    var meta = prepareMeta(result)
    if (err) return finish(err, null, meta)
    var doc = (result && result.ops && result.ops.length) ? result.ops[0] : null
    if (!doc) return finish(err, doc, meta)
  ...
  function handleUpdatedDoc(err, result) {
    var meta = prepareMeta(result)
    if (err) return finish(err, null, meta)
    collection.findOne({_id: doc._id}, function(err, savedDoc) {
  ...
  function handleRemovedDoc(err, result) {
    var meta = prepareMeta(result)
...
function prepareMeta(result) {
  var meta = {errors: []}
  if (!result) return meta
  if (result.result && tipe.isDefined(result.result.n)) meta.count = result.result.n
  else if (tipe.isDefined(result.nRemoved)) meta.count = result.nRemoved
  return meta
}
