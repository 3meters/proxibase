...
function safeExec(collection, method, query, doc, options, cb) {
  ...
  function handleRemoves(err, count) {
    ...
    finish(err, null, meta)    // included deleted doc for post-porcessing?
  }
  ...
  function finish(err, savedDoc, meta) {
    ...
    if (!schema.after) {
      ...
      schema.after.call(collection, err, state, cb)
    }
  }
}
