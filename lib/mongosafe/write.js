...
// Spec for links to be inserted with a newly inserted doc
var insertLinksSpec = {
  type: 'array', value: {
    type: 'object', value: {
      _to:    {type: 'string'},
      ...
      if (!(val._to || val._from)) return 'insert link requires either _from or _to'
      if (val._to && val._from) return 'insert link cannot set both _from and _to'
    }
  }
}
...
function safeInsert(doc, options, cb) {
  ...
  var collection = this
  if (doc.links) {
    options.links = doc.links
    delete doc.links
    err = scrub(options.links, insertLinksSpec)
    if (err) return cb(err)
  }
  run(collection, 'insert', doc, null, options, cb)
}
...
function safeUpsert(doc, options, cb) {
  ...
  // Doc did not include _id, insert
  if (!tipe.isString(doc._id)) {
    return run(collection, 'insert', doc, null, options, cb)
  }
...
function run(collection, method, doc, previous, options, cb) {
  ...
  function handleInserts(err, docs, meta) {
    ...
    if (err || !doc) return finish(err, doc, meta)
    if (!options.links) return finish(err, doc, meta)
    insertLinks(err, doc, meta)
  }


  // Newly inserted documents can include an array of links to insert
  // after the document was successfully inserted.
  function insertLinks(err, doc, meta) {

    doc.links = []
    var linkDbOps = {user: options.user}
    var linkErrors = []
    ...
    function insertLink(link, nextLink) {
      ...
      db.links.safeInsert(link, linkDbOps, function(err, savedLink) {
        if (err) linkErrors.push({link: link, error: err})
        if (savedLink) doc.links.push(savedLink)
        nextLink()
      })
    }

    // If any errors were generated add them to meta
    function finishInsertLinks(err) {
      if (linkErrors.length) meta.errors.push({insertLinkErrors: linkErrors})
      finish(err, doc, meta)
    }
  }
...
// Ensure all methods have a common meta object with an errors array
function prepareMeta(meta) {
  if (!tipe.isObject(meta)) {
    if (tipe.isNumber(meta)) meta = {count: meta}
    else meta = {}
  }
