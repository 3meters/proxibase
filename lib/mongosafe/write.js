...
function safeExec(collection, method, query, doc, options, cb) {
  ...
  // Schema includes a post-processor function with the signiture:  fn(err, cb, savedDoc, meta)
  // Call it, inserting the original callback as the second parameter.
  if (schema.after) {
    var _cb = cb
    cb = function() {
      var cbArgs = Array.prototype.slice.call(arguments)
      var newArgs = [cbArgs[0], _cb]
      if (2 === cbArgs.length) newArgs.push(doc) // remove is arity 2: err, meta.  Insert removed doc as arg2
      newArgs = newArgs.concat(cbArgs.slice(1))
      debug('newArgs', newArgs)
      schema.after.apply(collection, newArgs)
    }
  }
  ...
  function handleInserts(err, docs) {
    var savedDoc
    var meta = {method: 'insert', count: 0, options: options}
    if (docs) {
  ...
  function handleUpdates(err, count) {
    var meta = {method: 'update', count: 0, options: options}
    if (err) return cb(err, null, meta)
    collection.findOne(query, function(findErr, savedDoc) {
      if (savedDoc) meta.count = 1
      cb(findErr, savedDoc, meta)
    })
  }

  function handleRemoves(err, count) {
    var meta = {method: 'remove', count: count||0, options: options}
    cb(err, meta)    // included deleted doc for post-porcessing?
  }
}
