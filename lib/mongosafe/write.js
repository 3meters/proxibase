...
function run(collection, method, doc, previous, options, cb) {
  ...
  function runBefores() {
    ...
    function callWriteBefores(err) {
      if (err) return cb(err)
      async.eachSeries(befores.write, callBefore, callMethodBefores)
    }

    function callMethodBefores(err) {
      if (err) return cb(err)
      async.eachSeries(befores[method], callBefore, execute)
    }
  }
