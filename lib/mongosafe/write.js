...
function extend(mongo) {
  ...
  function(doc, options, cb) {
    ...
    if (tipe.isError(args)) return cb(args)
  ...
  function(doc, options, cb) {
    var self = this
    if (!doc._id) return self.safeInsert(doc, options, cb)
    self.findOne({_id: doc._id}, function(err, foundDoc) {
      if (err) return cb(err)
      if (foundDoc) {
        return self.safeUpdate(doc, options, cb)
      }
      else return self.safeInsert(doc, options, cb)
    })
  }
