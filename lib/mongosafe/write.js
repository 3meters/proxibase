...
function safeWrite(cmd, args) {
  ...
  var docs = util.clone(args[0])
  var options = args[1]
  var cb = args[2]
  if (err) return cb(err)

  var collection = this
  db = db || this.db // jshint ignore:line
  var results = []
  var meta = {}
  options.method = cmd

  if (cmd === 'insert') {
    // doc links override options links
    if (docs.links) {
...
// Dependin on whether it is found, set upserts to insert
...
function writeDoc(cmd, collection, doc, options, cb) {
  ...
  function execute(err) {
    ...
    switch(cmd) {
      ...
      case 'insert':
        ...
        // debug('safe insert', doc)
        return collection.insert(doc, options, handleInsertedDoc)
        break

      case 'update':
        doc = prepareWrite(doc, previous, schema)
        if (tipe.isError(doc)) return cb(doc)
        // debug('safe update ' + doc._id, doc)
        return collection.update({_id: doc._id}, doc, options, handleUpdatedDoc)
        break

      case 'remove':
        // debug('safe remove ' + doc._id)
        return collection.remove({_id: doc._id}, options, handleRemovedDoc)
        break
  ...
  function insertLinks(doc, meta, cb) {
    ...
    function insertLink(link, nextLink) {
      ...
      collection.db.links.safeInsert(link, linkDbOps, function(err, savedLink) {
  ...
  function handleUpdatedDoc(err, meta) {
    meta = prepareMeta(meta)
    if (err) return finish(err, null, meta)
    collection.findOne({_id: doc._id}, function(err, savedDoc) {
      if (!savedDoc) savedDoc = null
  ...
  function handleRemovedDoc(err, meta) {
    meta = prepareMeta(meta)
