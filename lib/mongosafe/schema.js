...
function extend(mongo) {
  ...
  // Return our annotated collection
  Db.prototype.collection = function(collectionName) {
    debug('clname', collectionName)
    debug('this', this, {depth:1})
    if (!(this.safeCollections && this.safeCollections[collectionName])) {
      return new Error('unknown collection ' + collectionName)
    }
  ...
  mongo.createSchema = function(schema1, schema2) {  // can include more
    ...
    var newSchema
    try { newSchema = JSON.parse(JSON.stringify(_schema)) }
  ...
  Db.prototype.initSchema = function(schema, cb) {
    ...
    if (tipe.isDefined(db[schema.name])) {
      return cb(new Error('Invalid schema name ' + schema.name))
    }

    if (tipe.isDefined(db[schema.collection])) {
      return cb(new Error('Invalid collection name ' + schema.collection))
    }
    ...
    function finish(err) {
      ...
      for (var s in db.safeSchemas) {
        if (db.safeSchemas[s].id === schema.id) {
      ...
      debug('init schema', collection.schema.name)
      cb(null, schema)
    }
  }
