...
function createSchema() {  // arguments is an ordered list of schemas
  ...
  var _schema = { type: 'object', required: true, value: {
    ...
    validators: { type: 'object', default: {}, value: {
      init:   functionArray,
      read:   functionArray,
      write:  functionArray,
...
function hotSwap(schemaName, docs, cb) {
  var db = this
  var schema = db.safeSchema(schemaName)
    ...
    db.createCollection(temp, function(err) {
      ...
      db.collection(temp).insert(docs, {safe: true}, function(err) {
        ...
        db.dropCollection(schema.collection, function(err) {
          if (err) ; // continue, may not exist
          db.collection(temp).rename(schema.collection, function(err) {
            if (err) return cb(err)
            // rebuild the indexes
