...
function filterEntityList(entityIds, beaconIds) {
  ...
  db.collection('entities').find(query, {_id:true}).limit(limit + 1).toArray(function(err, entities) {
    ...
    if (entities.length === 0) {
      ...
      var fields = { _id:true, _beacon:true, label:true, location:true, imagePreviewUri:true, linkUri:true }
      methods.getEntities(filteredIds, req.body.eagerLoad, beaconIds, fields, res)
    }
  })
}