...
exports.main = function(req, res) {
  ...
  if (!(req.body.latitude && typeof req.body.latitude === 'number')) {
    return sendErr(res, new Error("request.body.latitude of type number is required"))
  }

  if (!(req.body.longitude && typeof req.body.longitude === 'number')) {
    return sendErr(res, new Error("request.body.longitude of type number is required"))
  }

  if (!(req.body.radius && typeof req.body.radius === 'number')) {
    return sendErr(res, new Error("request.body.radius of type number is required"))
  }
