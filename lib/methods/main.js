...
exports.execute = function(req, res) {
  if (!methods[req.methodName]) {
    return res.error(new Error('Method ' + req.methodName + ' not found'))
  }
...
function find(req, res) {
  ...
  if (!req.body.table) {
    return res.error(new Error('request.body.table is required'))
  }
  if (!gdb.models[req.body.table]) {
    return res.error(new Error(req.body.table + ' is not a valid table'))
  }
