...
function getEntities(entIds) {
  ...
  qry.run(function(err, ents) {
    ...
    getChildEntitiesFromParents(parts.entIds)
  })
}
...
function getComments(entIds) {
  ...
  qry.run(function(err, comments) {
    ...
    getLinks(parts.entIds)
  })
}

function getLinks(entIds) {
  var qry = mdb.models['links'].find().limit(limit + 1)
    .where('_parent').in(entIds)
    .populate('_beacon', beaconFields)
  qry.run(function(err, links) {
    if (err) return sendErr(module.res, err)
    parts.links = links
    buildPayload()
  })
...
function buildPayload() {
  ...
  function getLinks(entId) {
    var links = []
    parts.links.forEach(function(drop) {
      if (drop._parent === entId) {
        drop = drop.serialize()
        // delete drop._parent
        links.push(drop)
      }
  ...
  ents.forEach(function(ent) {
    ...
    ent.links = getLinks(ent._id)
    ent.links = ent.links.length
    populateEnt(ent)
  })
