...
module.exports.getEntities = function(entityIds, eagerLoad, beaconIds, fields, response) {
  ...
  // fields is always either empty or has settings.
  ...
  db.collection('entities').find(query, params.fields.entities).limit(limit + 1).toArray(function(err, entities) {
  ...
  db.collection('links').find(query).toArray(function(err, links) {
    ...
    if (links.length == 0) {
      ...
      if (params.eagerLoad.children) {
        ...
        db.collection('entities').find(query, params.fields.children).limit(limit + 1).toArray(function(err, childEntities) {
          if (err) return params.res.sendErr(err)
...
function buildPayload() {
  ...
  function getChildren(entityId) {
    ...
    parts.childLinks.forEach(function(childLink) {
      if (childLink._to === entityId) {
        ...
        if (allFields || (!allFields && params.fields.children['creator'] == true)) {
            childEntity.creator = getUser(childEntity._creator)
        }

        if (allFields || (!allFields && params.fields.children['commentsCount'] === true)) {
  ...
  parts.entities.forEach(function(entity) {
    ...
    if (allFields || (!allFields && params.fields.entities['commentsCount'] == true)) {
      if (entity.comments) {
    ...
    if (params.eagerLoad.children) {
      ...
      if (allFields || (!allFields && params.fields.entities['childrenCount'] == true)) {
        entity.childrenCount = getChildrenCount(entity._id)
      }
    }

    if (allFields || (!allFields && params.fields.entities['creator'] == true)) {
      entity.creator = getUser(entity._creator)
    }

    if (allFields || (!allFields && params.fields.entities['_beacon'] == true)) {
    ...
    if (allFields || (!allFields && params.fields.entities['location'] == true)) {
