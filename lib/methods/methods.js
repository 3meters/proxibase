...
module.exports.statics = {
  typePicture: 'com.proxibase.aircandi.candi.picture',
  typePost: 'com.proxibase.aircandi.candi.post',
  typeLink: 'com.proxibase.aircandi.candi.link'
}
...
module.exports.getEntities = function(entityIds, entityIdsCursor, eagerLoad, beaconIds, observation, fields, options, more, response) {
  params.entityIds = entityIds
  params.entityIdsCursor = entityIdsCursor
  ...
  db.collection('entities')
    ...
    if (entities.length > 0) {
      ...
      parts.entityIdsCursor = params.entityIdsCursor
      parts.entityIds = []
      parts.userIds = []
      parts.childEntities = []
      parts.childLinks = []
...
function buildPayload() {
  ...
  function getChildCursor(entityId) {
    var entityIdsCursor = []
    for (var i = 0; i < parts.childEntities.length; i++) {
      for (var j = 0; j < parts.childLinks.length; j++) {
        if (parts.childLinks[j]._from == parts.childEntities[i]._id && parts.childLinks[j]._to === entityId) {
          entityIdsCursor.push(parts.childEntities[i]._id)
        }
      }
    }
    return entityIdsCursor;
  }
  ...
  parts.entities.forEach(function(entity) {
    ...
    if (_.isEmpty(params.fields.entities) || params.fields.entities['childCursor'] == true) {
      entity.childCursor = getChildCursor(entity._id)
    }
