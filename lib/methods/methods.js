...
/*
 * The caller includes an extra entityId if needed to ensure that 
 * limit reporting is correct.
 */
module.exports.getEntities = function(entityIds, eagerLoad, beaconIds, fields, options, response) {
  ...
  params.res = response
  more = [] /* this was accumulating between calls. why? */ 
  ...
  db.collection('entities')
    ...
    .limit(params.options.limit + 1)
    .toArray(function(err, entities) {

    if (err) return params.res.sendErr(err)
    if (entities.length > params.options.limit) {
      entities.pop()
      more.push('entities')
    }
  ...
  function getChildren(entityId) {
    ...
    parts.childLinks.forEach(function(childLink) {
      if (childLink._to === entityId) {
        ...
        if (childCount === params.options.limit) return false
      }
    })
    return children
  }
