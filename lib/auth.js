...
module.exports.start = function(config) {
  ...
  authom.app.signinLocal = function(req, res, next) {
    ...
    users.findOne({ email: user.email }, function(err, foundUser) {
      ...
      else return res.error(new HttpErr(httpErr.badAuthCred))
    })

    // Authenticate user by password
    function authenticate(user) {
      if (!user.verifyPassword(req.body.user.password)) {
        return res.error(new HttpErr(httpErr.badAuthCred))
      }
      log('debug: user authenticated')
      user.lastSignedInDate = util.getTimeUTC()
      user.foo = 'bar'
      log('debug user.foo: ', user.foo)
      user.__user = {
        _id: user._id,
        role: user.role
      }
  ...
  authom.app.changePassword = function(req, res, next) {
    ...
    users.findOne({ _id: user._id }, function(err, foundUser) {
      if (err) return res.error(err, 500)
      if (!foundUser) {
        return res.error(new HttpErr(httpErr.bathAuthCred))
      }
  ...
  authom.app.signout = function(req, res, next) {
    ...
    if (!(req.qry.user && req.qry.session)) {
      return res.error(new HttpErr(httpErr.missingParam, ['user', 'session']))
    }
  ...
  function validateSession(req, res, callback) {
    users.findOne({
      ...
      if (err) return res.error(err, 500)
      if (!user) return res.error(new HttpErr(httpErr.badAuthCred))
      sessions.findOne({
        key: req.qry.session
      }, function(err, session) {
        if (err) return res.error(err, 500)
        ...
        if (now > session.expirationDate) {
          return res.error(new HttpErr(httpErr.sessionExpired))
        }
        if (now > (session.modifiedDate + util.statics.session.refreshAfter)) {
          // Bump the modifiedDate and expirationDate to reset the session time to live clock
          session.modifiedDate = now
          session.expirationDate = now + util.statics.session.timeToLive
