...
module.exports.start = function(config) {
  ...
  authom.app.signinLocal = function(req, res, next) {
    ...
    if (!(req.body && req.body.user && req.body.user.name && req.body.user.password)) {
      return res.error('user.name and user.password are required')
    }
  ...
  authom.app.changePassword = function(req, res, next) {
    ...
    var user = req.body.user
    if (!(user._id && user.oldPassword && user.newPassword)) {
    ...
    users.findOne({ _id: user._id }, function(err, foundUser) {
      if (err) return res.error(err, 500)
      if (!foundUser) {
        return res.error('User _id: ' + user._id + ' not found', 404)
      }
  ...
  function validateSession(req, res, callback) {
    users.findOne({
      ...
      if (err) return res.error(err, 500)
      if (!user) return res.error('Session invalid, user ' + req.qry.user + ' not found', 401)
      sessions.findOne({
        key: req.qry.session
      }, function(err, session) {
        if (err) return res.error(err, 500)
        if (!session) {
          return res.error('Session invalid, session ' + req.qry.session + ' not found', 401)
        }
