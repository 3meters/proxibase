    ...
    if (adminUser) return next(null, adminUser)
      ...
      users.collection.insert(newAdminUser, {safe: true}, function(err) {
        ...
        users.findOne({ _id: util.adminUser._id }, function(err, adminUser) {
          if (err) return next(err)
          if (!adminUser) return next(new Error('Could not create admin user'))
          return next(null, adminUser)
        })
      })
    }
  })

  function next(err, adminUser) {
    if (err) throw err
...
function startAppServer() {
  ...
  util.authService = require('./api/auth/auth').start()
  log('Authentication service started', authService)
  ...
  app.use(express.vhost('api.' + config.service.host, require('./api/main').app))
    ...
    .use(express.vhost(config.service.host, require('./api/main').app))
