...
var
  version = '0.02.0000',                 // update in package.json as well
  util = require('./util'),
  fs = require('fs'),
...
function ensureAdminUser() {
  ...
  // Writing through mongoskin to bypass normal mongoose checks
  db.collection('users').findOne({ _id: util.statics.adminId },
    function(err, adminUser) {
    if (err) throw err
    if (adminUser) return finish(adminUser)
    else {
      log('Creating new admin user')
      db.collection('users').save({
        _id:  util.statics.adminId,
        email: 'admin',
        name: 'admin',
        role: 'admin',
        password: gdb.models['users'].hashPassword('admin')
      }, function(err) {
        if (err) throw err
        db.collection('users').findOne({ _id: util.statics.adminId }, function(err, adminUser) {
