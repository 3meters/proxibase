...
config.service.version = semver.valid(version)
assert(config.service.version, 'Invalid version')
...
// Set the client version from the document in the database if it exists
// otherwise from statics
function ensureClientVersion() {
  var cv = util.statics.clientVersion
  util.db.documents.findOne({_id: cv._id}, function(err, doc) {
    if (err) throw err
    if (doc) config.clientVersion = doc.data.version
    else config.clientVersion = cv.data.version
    startAppServer()
  })
}
...
process.on('uncaughtException', function(err) {
  ...
  // setTimeout(function() {process.exit(1)}, 1000) // die
  process.exit(1)
})
