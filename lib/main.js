...
cli
  ...
  .option('-t --test', 'run on test port as defined in config file')
  .option('-p --port <port>', 'port number (overrides test)', Number)
  .parse(process.argv)
...
log('===============================================================')
console.log(Date())
log('Attempting to start ' + config.name + ' using config:\n', config)
log()
...
function startAppServer() {
  ...
  var app = null
  var listenMessage = config.name + ' server listening on https://%s:%d'

  if (config.bypassSsl) {
    config.port = config.bypassSslPort
    app = express.createServer()
    listenMessage = config.name + ' server listening on http://%s:%d'
  } else {
  ...
  console.log(listenMessage, config.host, config.port)
