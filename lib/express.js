...
var pe = require('./error')
var proxErr = pe.proxErr
var ProxErr = pe.ProxErr
var proxErrMap = pe.proxErrMap
var proxErrCodeMap = pe.proxErrCodeMap


// Stash express's original send method
...
express.response.error = function(err, info) {
  ...
  // Selectively log errors to stdErr
  if (!err.status || err.status >= 500 || config.log > 1) {
    logErr('\n==== ERROR on request: ' + req.tag + ', ip: ' + req.ip +
        ', received: ' + req.timer.baseDate().toUTCString())
    logErr(req.method + " " + req.url)
    if (req.method === 'post') logErr(req.body, true, 5)
    logErr()
    logErr('appStack:', body.error.appStack)
    if (config.log > 2) logErr('stack:', err.stack)
  }
