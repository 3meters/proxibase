...
function doInsertEntity(entity) {
  ...
  doc.save(function (err, savedDoc) {
    ...
    if (!savedDoc._id) return res.sendErr(new Error('Insert entitiy failed', 500))
  ...
  if (!req.body.beacon) {
    ...
    db.collection('beacons').findOne({_id:offeredBeacon._id}, function(err, beacon) {
      ...
      if (!beacon) {
        ...
        doc.save(function (err, savedDoc) {
          if (err) return res.sendErr(err)
          if (!savedDoc._id) return res.sendErr(new Error('Insert beacon failed', 500))
...
function updateBeacon(beacon) {
  gdb.models['beacons'].findOne({ _id: beacon._id}, function (err, doc) {
    ...
    doc.save(function(err, updatedDoc) {
      if (err) return res.sendErr(err)
      if (!updatedDoc) return res.sendErr(new Error('Update beacon failed', 500))
      insertLink()
    })
  })
}
...
function insertLink() {
  if (!req.body.link) {
    ...
    doc.save(function (err, savedDoc) {
      if (err) return res.sendErr(err)
      if (!savedDoc._id) return res.sendErr(new Error('Insert link failed', 500))
      insertObservation()
    })
  }
}
