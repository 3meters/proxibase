...
var util = require('util')
  ...
  , _send = express.response.send  // Stash express's send method
...
;(function() {
  ...
  Response.error = function(err) {
    ...
    log('debug http new res.error called')
  ...
  express.send = function(body) {
    log('debug http.send methods extended send called')
    Response.sendfoo(body)
  }

  // Extend Express's res.send with a version that logs response times
  Response.sendfoo = express.sendfoo = function(body) {

    log('debug http.sendfoo: new res.sendfoo called')
  ...
  Response.sendErr = function(err) {
    log('debug http sendErr called')
    Response.error(err)
  }
