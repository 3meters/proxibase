...
express.response.error = function(err, info) {
  ...
  for (key in err) { body.error[key] = err[key] }

  if (err.stack) body.error.appStack = getAppStack(err.stack)
  if (err.stack && config.log > 1) body.error.stack = err.stack
  if (err.stack && config.log > 2) util.logErr(err.stack)
...
// Creates a stack that filters out calls in node_modules and nodes internal event pump
