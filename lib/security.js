...
module.exports.start = function(config) {
  ...
  authom.on('auth', function(req, res, data) {
    ...
    gdb.models['users'].findOne({oauthId: oauthId}, function(err, user) {
      ...
      if (!user) {
        return res.sendErr('User was validated by ' + data.service + ' with id ' + data.id + 
          ', but we could not find a user with those credentials in our system. Valid providers include ' + 
      ...
      user.save(function(err, updatedUser) {
        ...
        if (!updatedUser) return res.sendErr('Update failed for user ' + oauthId, 500)
